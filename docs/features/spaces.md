...
- **Google Calendar** — the data model is per-space (each space can connect its own calendar), but the push connector currently writes everything to a single calendar. The per-space connector is the active in-progress slice of the roadmap. See [`pending/REVIEW-LATER.md`](../../pending/REVIEW-LATER.md) (FEAT-3p9 entry).
