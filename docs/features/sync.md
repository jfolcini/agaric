...
Agaric syncs across your own devices over local WiFi. **No cloud, no accounts, and no third-party servers unless you turn on the internet fallback** (below). Edits converge automatically via CRDT — there is no conflict dialog because there are no merge conflicts to resolve.
...
- **Turn on the internet fallback** (Settings → Sync & Devices, *Local network, with an internet fallback*; off by default) when your devices can see each other but every connection dies — the VPN case in the pitfalls below. With it on, on **both** devices, sync goes through a connection server on the internet (a relay run by iroh's maintainers) whenever the direct path fails; the local network is still used whenever it works. The relay forwards encrypted traffic it cannot read, but it does see which of your devices are talking, from which addresses, when, and how much moves — and the device holds a connection to it for as long as sync is running. It takes effect the next time Agaric starts. It does not help two devices on different networks find each other: discovery is still mDNS-only, so both devices must still be on the same local network (#4549).
...
- **A restored copy of another device's app data *is* that device, and will not pair with it.** The app data directory holds `device-id` and `sync-endpoint.key` next to the vault, so restoring a backup onto a second install brings the first device's identity with it. That identity is not decoration: every operation in the log is hashed under its `device_id` and keyed by `(device_id, sequence number)`, so two installs answering to one id would hand different edits the same slot. Agaric refuses the pair instead: the receiving device answers *"cannot sync with self"* and says nothing to its own user (there is nothing that user did, or can do, about it), and the device that dialled only sees the sync fail — so the symptom is a pair that never completes, right after a restore. **Copying the app data directory is not how you set up a second device**: install Agaric there and pair, and the first sync fills the vault for you. If you already have such a copy and have **not** edited it since the restore, quit it, delete `device-id` **and** `sync-endpoint.key` from its app data directory, and start it again — it mints a fresh identity, leaves your notes alone, and can then be paired like any new device. If you *have* edited it, that no longer works and you must not try it: a new identity only changes who authors ops from now on, while the edits both copies made since the restore are already written under the original device's identity, in the same slots. The pair then completes and every sync after it fails permanently — the merge asks for a snapshot, and the snapshot cannot heal a device that forked its own identity space (#792, #2503). Export those notes out of the copy instead (per-page export or export-all-as-ZIP), then set the device up fresh and import them.
