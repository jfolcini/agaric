...
**Include inherited tags** (off by default) widens the whole query to blocks nested under a tagged block: a tag on a block applies to every block beneath it, however deep, so a tag on a page's first block matches the entire page. Only tags added to a block are inherited — a `#tag` typed in a block's text is not. The page header shows inherited tags as dashed, italic chips; a plain tag query does not match them until the switch is on.
