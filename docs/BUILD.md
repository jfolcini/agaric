...
`#5101` — on vitest 5 the runner's per-mutant test filter matched nothing and every mutant was reported survived. `stryker.vitest.config.mjs` carries the shim that widens that filter; delete it once [stryker-js#6214](https://github.com/stryker-mutator/stryker-js/pull/6214) ships and the two Stryker packages move past it.
...
> **`src-tauri/fuzz/Cargo.lock` is not only a release concern.** A *version bump* is handled for you (step 3 above). But `src-tauri/fuzz` is its own workspace that path-depends on `agaric`, `agaric-store`, `agaric-sync` and `agaric-engine`, so its lock also resolves every requirement declared in the parent manifests: **adding or changing a dependency requirement in any `src-tauri/**/Cargo.toml` invalidates `src-tauri/fuzz/Cargo.lock` too**, and `_validate.yml`'s `verify-lockfiles` job (#4142) reds on every such PR until it is regenerated — even when the diff never touches `src-tauri/fuzz`. No parent-workspace cargo command reaches it, so refreshing `src-tauri/Cargo.lock` alone does not fix it. Regenerate in the same commit with `cd src-tauri/fuzz && cargo metadata --format-version 1 >/dev/null` (any cargo command *without* `--locked` rewrites the lock conservatively, keeping every pin that still satisfies the manifests; `cargo metadata` is the cheapest — it runs no build). **Do not use `cargo generate-lockfile` or a bare `cargo update` there:** they re-resolve the whole graph to the newest compatible versions and lift deliberate holds — measured on this lock, 882 packages relocked in one go. `loro` no longer illustrates the lift (it is pinned exactly in `agaric-engine`, so the resolver now refuses it), but any hold carried by the lock alone still goes silently. Dependabot cannot do this for you: its `/src-tauri/fuzz` entry is `lockfile-only` by design (#3432), so a `/src-tauri` bump that edits a manifest reds here and needs the one-command fix by hand. See [AGENTS.md § Coupled Dependency Updates](../AGENTS.md#coupled-dependency-updates).
