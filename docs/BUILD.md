...
This is a **nightly-only, non-gating** lane (`mutants-frontend` job in `.github/workflows/scheduled-deep-checks.yml`) — surviving mutants are triage signal for occasional audits, not a merge gate; while #5101 is open they are not signal at all, per the note above. See issue #886 for the full evaluation and rationale.
