...
> **Compaction lives in `agaric-sync` but is not sync code.** `compact_op_log` (`src-tauri/agaric-sync/src/snapshot/`) is reached only through the user-facing `compact_op_log_cmd` (`src-tauri/src/commands/compaction.rs`, registered in `src-tauri/src/lib.rs`) and the 24 h maintenance tick — never from the protocol. So the carve-out to invariant #1 ("the op log is append-only *except compaction*") is exercised from `agaric-sync`, not from `agaric-store`, whose own `op_log/bypass.rs` holds only the row-deletion primitives (`truncate`, `prune`) that path calls. The module keeps its `snapshot/` name from the zstd-CBOR blob it used to build; #4699 deleted the blob and everything that read it. Details in [`architecture/crdt-and-recovery.md`](architecture/crdt-and-recovery.md).
...
- **Bindings**: `src/lib/bindings.ts` (specta-generated; checked in; CI fails on drift). Every frontend call site calls `commands.*` on it directly (#2927 retired the hand-written wrapper layer); the handful of helpers that carry real logic live in `src/lib/ipc-helpers.ts`.
