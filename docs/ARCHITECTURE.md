...
4. **Roving editor, one per mounted tree.** Exactly one block hosts a mounted TipTap editor surface at a time — focus is global — and everything else renders static. The `Editor` objects behind it are per `BlockTree`, not per app: the journal week/month/stream views mount one tree (and one lazily-constructed editor) per day, so app-level code reaches the live editor through the focus-published registry `src/editor/active-editor.ts`. See [`architecture/editor-and-content.md`](architecture/editor-and-content.md) § Roving-editor invariant.
...
> **Snapshots live in `agaric-sync`, but they are not sync-only.** `create_snapshot`, `apply_snapshot` and `compact_op_log` (`src-tauri/agaric-sync/src/snapshot/`) are local-durability code — the sync protocol does not consume them; the app crate does, through the user-facing `compact_op_log_cmd` (`src-tauri/src/commands/compaction.rs`, registered in `src-tauri/src/lib.rs`) and boot recovery. So the compaction carve-out to invariant #1 ("the op log is append-only *except compaction*") is reached from `agaric-sync`, not from `agaric-store`, whose own `op_log/bypass.rs` holds only the row-deletion primitives (`truncate` for a snapshot RESET wipe, `prune` for compaction) those paths call. Details in [`architecture/crdt-and-recovery.md`](architecture/crdt-and-recovery.md).
