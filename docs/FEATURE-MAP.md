...
| **Embeds** | `{{embed ((ULID))}}` renders a block's or page's subtree inline on another page, live, in a bordered container with a source breadcrumb, an Open-source route, cycle and depth stubs. Read-only until the per-embed unlock toggle, after which a row's text is editable in place — writing to the page that owns the block — while every structural chord stays off. | [features/tags-and-links.md](features/tags-and-links.md) |
...
| **Views** | Search, Pages browser, Tags browser, Advanced Query (nested filters, sort, group-by, aggregates), Trash, History, Templates, Graph, Status, Settings. | [features/views.md](features/views.md) |
...
| **Leaked empty-block cleanup** | A block left blank and carrying nothing — no children, no task state or dates, no properties or tags, and unreferenced — is removed when you leave it, and never the last block of a page. Blocks already stored that way are soft-deleted into Trash by a cursor-gated boot sweep, so they are recoverable (#4729). Neither delete takes a slot on the undo stack: Ctrl+Z stays yours. | `src/lib/empty-block-cleanup.ts`, `src-tauri/agaric-engine/src/empty_blocks.rs` |
...
- **OS notifications** — _desktop reminders ship; the rest is outstanding_. A maintenance job (`src-tauri/src/reminders.rs`, on the 60 s daemon tick) fires one OS notification per open task on its `due_date`, at the reminder time set in Settings → Notifications, while the app is running (minimised counts; closed does not). A per-`(block, date)` ledger in `app_settings` keeps a tick or a restart from notifying twice; preferences are device-local (`get_reminder_settings` / `set_reminder_settings`, off by default). Dispatch is the fire-now `notify_task` path (`src-tauri/src/commands/notifier.rs`; Linux via notify-rust, macOS/Windows via `tauri-plugin-notification`). Still outstanding on #4554: `scheduled_date` as an anchor, the Android arm (OS alarms), lead offsets and a per-task `reminder` property, the catch-up digest, tap-to-open, snooze.
- **Working two-device sync** — the _transport_ is done: the iroh QUIC port shipped (#78, plan #3464), replacing the WebSocket + TLS + cert-hash-TOFU stack, with mDNS now advertising `_agaric._udp` carrying each peer's `endpoint_id`. The defect that stopped a first-ever pairing being initiated at all (#3502 — daemon policy rather than transport, so the port carried it across unchanged) is fixed. A first-ever pair has now been observed against two live devices (Linux desktop and Android phone, both 0.9.8) over QUIC on a real LAN, retiring the Android half of the old unverified claim. What that run surfaced is that the remaining failure modes are largely environmental — a VPN tunnel capturing a publicly-numbered LAN, and a default-deny host firewall. The pair also exposed three real defects: an initiator-side apply that aborted on `blocks.parent_id` (#4083, the one that broke sync outright); a responder-only device recording no progress, so it reads as never-synced and is marked due every tick (#4084); and every initiator `SyncEvent` carrying an empty `remote_device_id` (#4085). All three are fixed on main; none has been re-verified on hardware. The run is recorded in [`session-1345`](session-log/session-1345-first-live-pair-and-what-it-cost-to-believe-the-tools.md). **QUIC/UDP behaviour on restrictive WiFi is still unverified**, and a full two-way sync end to end remains unobserved (#3507). Known follow-ups from the cutover: #3511 (asymmetric per-peer lock during the pairing window), #3512 (wire compression retired), #3513 (inbound single-homed), #3514 (silent one-time re-TOFU for migrated installs), #3515 (10 s dead-peer dial).
