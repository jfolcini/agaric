...
- **Working two-device sync** — the _transport_ is done: the iroh QUIC port shipped (#78, plan #3464), replacing the WebSocket + TLS + cert-hash-TOFU stack, with mDNS now advertising `_agaric._udp` carrying each peer's `endpoint_id`. The defect that stopped a first-ever pairing being initiated at all (#3502 — daemon policy rather than transport, so the port carried it across unchanged) is fixed. What remains is verification rather than a known break: nothing has been observed against two live devices (#3507), and QUIC/UDP behaviour on Android and on restrictive WiFi is unverified because it needs hardware. Known follow-ups from the cutover: #3511 (asymmetric per-peer lock during the pairing window), #3512 (wire compression retired), #3513 (inbound single-homed), #3514 (silent one-time re-TOFU for migrated installs), #3515 (10 s dead-peer dial).
