...
- **OS notifications** — _partial_. A fire-now `notify_task` command ships (`src-tauri/src/commands/notifier.rs`; Linux via notify-rust, macOS/Windows via `tauri-plugin-notification`), invoked through `commands.notifyTask` (`src/lib/bindings.ts`) from `src/components/settings/NotificationsTab.tsx`. The scheduler, dedupe ledger, snooze semantics, and Settings sub-tab that would auto-fire for due tasks / scheduled events (incl. Android-mobile parity) remain outstanding (#138).
