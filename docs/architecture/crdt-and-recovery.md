...
They were also the sync catch-up vehicle. Since **#2503** production catch-up ships **Loro** snapshots that the receiver *merges* into its own engine, so the CBOR wipe-and-replace path below is reached only by the accept-old compatibility branch talking to a pre-#2503 peer (see [sync-protocol-spec.md](sync-protocol-spec.md) § Snapshot-fallback flow).
...
`apply_snapshot()` is the RESET path a **CBOR** snapshot catch-up runs on the *caught-up* (initiator) device (`sync_daemon::snapshot_transfer::try_receive_snapshot_catchup`). Post-#2503 it is reached only when the peer is a pre-#2503 responder still sending `SnapshotOffer`; the current Loro-merge catch-up is non-destructive. The contract below is what that legacy branch still costs. In one `BEGIN IMMEDIATE` transaction it wipes the core tables **and** `op_log`, `loro_doc_state`, `loro_sync_inbox`, `log_snapshots`, and `block_drafts`, then re-seeds the core tables from the peer's snapshot:
...
Steps 2–3 are gated on this boot's block-recovery signal, plus a persisted retry marker (`engine_reproject_pending`, #2920) so a partial engine reprojection is retried on the next boot instead of being lost permanently.
