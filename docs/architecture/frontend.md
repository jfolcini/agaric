...
This is #2462 off the FE↔BE boundary review, filed after the #2451 audit found every keyboard handler's failure path was dead code (built for a throw-on-failure contract the store never had) and every existing failure test mocked the store as throwing — the #2407 class, where a Backspace-merge permanently deleted a source block whose merged content never actually saved. The regression test for that fix lives in `src/hooks/__tests__/useBlockActionOrchestration.test.ts` under `describe('merge honors edit() resolving false (store contract)')` — it mocks `edit: vi.fn(async () => false)`, never `mockRejectedValue`. **Any new failure-path test must mock the store the same way**: `mockResolvedValue(false)` / `vi.fn(async () => false)`, never `mockRejectedValue` — a rejecting mock tests a contract the store doesn't implement.
