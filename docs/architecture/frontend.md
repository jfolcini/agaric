...
**This is an in-progress migration (#2927), not a settled layering.** Most non-test files already import `bindings.ts` (or `commands.*`) directly; the remainder still go through the wrapper modules. The set of files still importing the legacy surface is frozen by `scripts/tauri-import-baseline.json` and policed by the `tauri-import-baseline` prek hook — the list may shrink, never grow. Read that file for the current state rather than trusting a count here.
