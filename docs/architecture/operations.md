...
Every interactive command is now in the enforced green tier except `revert_ops` (separate `SLO_INCLUDE_REVERT=1` gate, aspirational budget) and `list_projected_agenda` (below). `list_page_links` was confirmed under budget on the nightly lane and promoted (#2178): 84–109 ms @ 100K after the 20K count-then-cap (#2529, was 557–635 ms). `list_projected_agenda` improved to the budget line with the #2069 date-overlap prefilter but straddles it on runner variance (181 / 216 ms on same-day nightly runs), so it stays behind `SLO_INCLUDE_PROBLEM=1` — a hard gate there would coin-flip red — until further #2069 work or a #2298-style budget re-calibration settles it. The graph and projected-agenda paths remain the known scaling frontiers.
