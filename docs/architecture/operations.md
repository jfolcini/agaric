...
Every interactive command is now in the enforced green tier except `revert_ops`, which keeps its separate `SLO_INCLUDE_REVERT=1` gate (aspirational budget). The two former problem paths were confirmed under budget on the nightly lane and promoted (#2178): `list_page_links` at 84 ms @ 100K after the 20K count-then-cap (#2529, was 557–635 ms) and `list_projected_agenda` at 181 ms @ 100K after the #2069 date-overlap prefilter. The `SLO_INCLUDE_PROBLEM=1` gate now covers only measurement probes (the #2508 cache direct-query cases), not budget violations. The graph and projected-agenda paths remain the known scaling frontiers; `list_projected_agenda` sits at ~91% of budget, so runner variance there is the first place to look if the gate reds.
