...
The legacy CBOR path (`SnapshotOffer` / `SnapshotAccept` / `SnapshotReject` → `apply_snapshot()`, which wiped core tables and re-seeded from the blob) was deleted in #3487: the iroh cutover made it unreachable, since `SYNC_ALPN` is negotiated before any application byte moves and a build predating the port cannot open a session at all. `apply_snapshot()` itself now has no production caller — disaster recovery reprojects from `loro_doc_state`, and compaction only creates snapshots — so it survives as the read side of the compaction artifact's format, exercised by tests and benches (#4699). What that destructive restore costs a device is documented in [`crdt-and-recovery.md § What a catch-up RESET costs the caught-up device`](crdt-and-recovery.md), and the wire-level detail is in [`sync-protocol-spec.md § Snapshot-fallback flow`](sync-protocol-spec.md).
