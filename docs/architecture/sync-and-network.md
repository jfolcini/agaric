...
- **QUIC/UDP on restrictive WiFi is unverified.** The Android half of this claim is retired: a first pair completed between a desktop and an Android phone over QUIC on a real LAN ([`session-1345`](../session-log/session-1345-first-live-pair-and-what-it-cost-to-believe-the-tools.md)). What remains is not an Android-specific risk at all — this bullet outlives the Android delta list it sits in. What that run did *not* exercise is a hostile network — AP/client isolation, multicast disabled, or two subnets — and it needed a VPN and a host firewall cleared before it worked at all, which is evidence that restrictive environments are where the remaining risk is. This gates release, not development (#3507).
