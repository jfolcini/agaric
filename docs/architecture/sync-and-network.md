...
| Layer | Choice |
| --- | --- |
| Discovery | mDNS via `iroh-mdns-address-lookup` / `swarm-discovery` (pure Rust, no Avahi/Bonjour daemon required), added to the bound endpoint as an iroh address lookup |
| Pairing | EFF-wordlist 4-word passphrase OR QR code (v2 payload also carries the host's `device_id` + `endpoint_id` + bound `ip:port`, which the joiner dials, #4037); the passphrase proof rides inside the first `HeadExchange` (#855) |
...
mDNS service type `_agaricv1._udp.local.` (`src-tauri/agaric-sync/src/mdns.rs`). Each device publishes its iroh `endpoint_id` as the instance name and its `device_id` as iroh `UserData`, with the bound socket as the address; peers browse and resolve. **No Avahi dependency** — `swarm-discovery` does everything in-process. The lookup is added to the bound endpoint (`mdns::attach`), not to the builder, so `lan_only`'s `clear_address_lookup()` and its guard tests are untouched.

Both fields are load-bearing and neither is trusted. `endpoint_id` (the ed25519 public key) is the only part you can dial — the record supplies the rest of an iroh `EndpointAddr`. `device_id` is the op-log attribution key and what `peer_refs` rows are stored under, so a discovering peer can match a hit against a known row before spending a dial. A record with no `device_id` is ignored rather than surfaced: it addresses a peer without naming it. Anyone on the LAN can forge the `device_id`; the pin is checked against the handshake-authenticated `EndpointId`, so a forged record costs a failed dial, not a session.

Every service-name change is **a silent wire break**: a device on either side does not see one on the other — the browse key differs, so no event fires and no error is raised. `_agaric._tcp` → `_agaric._udp` (#3488, a QUIC endpoint is UDP) and `_agaric` → `_agaricv1` (#3464, the swap from `mdns-sd`; the crate default `irohv1` is shared with every other iroh app) both did this. The ALPN already forbids a cross-release *session*, which is what makes the break harmless, but it happens after the dial and so cannot report this one.
...
This fallback does not exist for a *first-ever* pair: `endpoint_id` is bound by TOFU (`bind_endpoint_id`) only after a successful first sync, a first sync requires a dial, and a dial requires an `endpoint_id`. Both TOFU sites bind only after the session settles — `server.rs` on the responder, `session_supervisor.rs` on the initiator — so the key a dial needs cannot come from the session that dial would open. Before a session exists, the mDNS TXT record is the only carrier of a peer's `endpoint_id` (`mdns.rs`); the QUIC handshake also names one, but only once you have already reached the peer. `lan_only` calls `clear_address_lookup()`, so iroh has no discovery of its own. The pairing dialog's manual mode (`entryMode: 'manual' | 'scan'` in `PairingDialog.tsx`) is passphrase entry vs. QR scan, not an address field. **The QR is the one carrier that does not depend on multicast**, and since #4037 it carries what a dial needs: payload v2 adds the host's `endpoint_id` and its bound `ip:port` candidates alongside the passphrase (`pairing::pairing_qr_payload`). The host resolves them from `SyncScheduler::publish_local_endpoint`, which `daemon_loop` writes the moment the QUIC endpoint binds — after the pairing marker is armed, because arming is what wakes a dormant daemon into binding at all.

**The joiner dials them.** `handleQrScan` keeps the v2 fields alongside the passphrase and hands them to `confirm_pairing`, which validates them into the same `DiscoveredPeer` an mDNS announcement produces (`ScannedPeerCandidate::into_discovered`) and publishes it on `SyncScheduler::publish_scanned_peer`. `daemon_loop` Branch B folds that one candidate into the round `peers_for_change_round` composes, on the same `pairing_pending` terms as an mDNS-discovered unpaired peer, and everything below it — the dial, the TOFU pin, the #855 proof — is unchanged and cannot tell the two sources apart. So the QR is an alternative *discovery source*, not an alternative dial path, and mDNS is untouched: a stale candidate loses a dial that fails in under a millisecond (`a_dial_naming_only_an_endpoint_id_fails_fast_instead_of_hanging`) and the mDNS round proceeds.

This is why the v2 payload carries `device_id` as well as `endpoint_id`. `DiscoveredPeers` is keyed by device id, and `try_sync_with_peer` uses it as both the orchestrator's `expected_remote_id` and the `peer_refs.peer_id` the TOFU pin binds — and `bind_endpoint_id` refuses to re-point a key afterwards, so a synthesised id would be permanent and would lock the real one out. An mDNS-discovered peer supplies its id in the TXT record; a QR-dialled peer has no announcement to read one from, so the QR must carry it. It is normalised on arrival by `accept_stated_device_id`, the same rejecting (never truncating) gate the wire's `sender_device_id` takes, for the same reason: it is untrusted text that becomes a `peer_refs.peer_id`.

The daemon's `discovered` map itself is still written by exactly one thing — a real mDNS event (`daemon_loop` Branch A; `start_with_lifecycle_seeded` is `cfg(test-util)`-gated). The scanned candidate is deliberately kept *out* of it: an entry in that map means "mDNS resolved this peer at this instant", which `mdns_last_seen` / `mdns_is_reaching_us` read to decide whether a dial timeout is a captured LAN (#4299). A QR candidate is evidence of no such thing, so it joins the round without joining the map, and its `seen_at` is the honest `None`.
...
1. EFF-wordlist 4-word passphrase displayed on Device A. The QR encodes that passphrase plus, since #4037, Device A's `device_id`, `endpoint_id` and bound `ip:port` candidates (payload `v: 2`). A device with no bound endpoint to advertise emits the `v: 1` passphrase-only payload instead — the same bytes it always did — and the TS parser reads `passphrase` without checking `v`, so the two versions interoperate in both directions.
...
3. There is no certificate exchange any more. The QUIC handshake authenticates both ends' `EndpointId` (an ed25519 public key) before a single application byte moves, so identity is established by the transport rather than claimed in an application message. The initiator carries `HeadExchange.pairing_proof`; the responder admits the unpaired peer only when that proof matches the one it stored when it armed or confirmed the pairing, and *then* binds the handshake-authenticated key into `peer_refs.endpoint_id` (TOFU — first contact wins, `bind_endpoint_id`). **Which `peer_id` that key is bound to comes from `HeadExchange.sender_device_id` (#4380)** — the joiner stating which device it is. It used to be inferred from the sort order of the joiner's advertised `heads`, which since #2481 carry every device frontier the joiner holds, so a three-device vault bound the joiner's key under another device's id about half the time. A peer too old to state an id is not bound at all when its heads name more than one candidate; the window stays open and the pair completes through the responder's own initiator pass.
...
- **Inbound is single-homed (#3513).** `lan_only` takes **one** bind address, so the daemon accepts on a single interface, with the prefix taken from its netmask. Which interface is `lan_interface::decide`'s call, not "the first RFC 1918 IPv4" — that selection was the #3853 bug (see *Operator notes*). A desktop on both WiFi and Ethernet accepts on whichever `decide` ranks first: same-class candidates are separated by the off-default-route hint before enumeration order is consulted at all, so the picked NIC is usually the one carrying the default route, not the one that happened to enumerate first. The *outbound* half genuinely improved: `EndpointAddr` carries every advertised address at once and iroh races them, so one `CONNECT_TIMEOUT` now covers all candidates where the old sequential loop paid one per address.
...
- **The S-5 lock key was asymmetric during the pairing window (#3511, fixed).** `HeadExchange` carried no `device_id` when this was fixed (it now carries `sender_device_id`, #4380 — read at the post-session bind, never as a lock key), and a fresh joiner with an empty op log advertises no head of its own, so the responder had nothing but the endpoint id to lock on until a binding existed — while the initiator locked on the device id. An inbound and an outbound session with the same device could therefore overlap for exactly the window in which both ends are most likely to dial simultaneously. The old stack did not have this gap: the cert CN supplied a device id unconditionally. **Both roles now key on the peer's `EndpointId`** (`sync_daemon::peer_lock_key`), which is the only identifier both hold unconditionally — the responder from the handshake, the initiator because it cannot dial without one. No wire change; see #3529 for why `EndpointId` is the right key for anything that must work *during* pairing, and why `device_id` stays the durable identity everywhere else.
...
Nothing in this document should be read as saying the QUIC port *by itself* makes pairing work. A first-ever pair has since been observed against two live devices — a Linux desktop and an Android phone, both 0.9.8 — so the evidence is no longer unit tests alone. The run is recorded in [`session-1345`](../session-log/session-1345-first-live-pair-and-what-it-cost-to-believe-the-tools.md). Be precise about what it covers: pairing and the first inbound session, not a verified two-way sync, and only after a VPN on the phone and a default-deny firewall on the desktop were cleared. The device attribution matters: the tunnel that captured the subnet (`bypassable=false`, all uids) was measured on the handset over adb; the desktop's daemon selected `wlp2s0` correctly and was blocked only by `ufw`. The initiator-side apply that aborted on `blocks.parent_id` (#4083) was fixed afterwards and has not been re-exercised on hardware, so a full two-way sync remains unobserved (#3507).
...
The legacy CBOR path (`SnapshotOffer` / `SnapshotAccept` / `SnapshotReject` → `apply_snapshot()`, which wiped core tables and re-seeded from the blob) was deleted in #3487: the iroh cutover made it unreachable, since `SYNC_ALPN` is negotiated before any application byte moves and a build predating the port cannot open a session at all. #4699 deleted the restore, the blob and the `SnapshotAccept` / `SnapshotReject` wire variants with it — nothing read the blob once `apply_snapshot()` lost its last caller, and it duplicated the derived SQL view rather than `loro_doc_state`, the merge truth disaster recovery reprojects from. Compaction survives as a bare op-log purge plus a `compaction_watermark` row (see [`crdt-and-recovery.md § Op-log compaction`](crdt-and-recovery.md)); the wire-level detail of the flow that replaced the CBOR one is in [`sync-protocol-spec.md § Snapshot-fallback flow`](sync-protocol-spec.md).
...
The port is **not stable across launches**: `lan_bind_target` binds `(interface_ip, 0)`, so the OS assigns an ephemeral port each start and the daemon announces whatever it got over mDNS. A firewall rule therefore cannot name a port. On a firewall that can match an application (macOS, Windows) that is what the rule should name; `ufw` and firewalld cannot, so on Linux the workable rule scopes by interface, protocol and source subnet instead — see the pitfall in [`sync.md`](../features/sync.md) for the exact form. `lan_bind_target` also picks **one** interface (#3513). It no longer picks the first RFC 1918 address: that loop was the #3853 bug, and it is worth knowing why, because it skipped a `192.160.160.0/24` LAN — public address space that looks private — and bound a Docker bridge instead. Selection now runs `lan_interface::decide`, which classifies every enumerated address (physical before cellular before virtual, CGNAT deprioritised) and uses the default route only to break ties *within* a class — deliberately not as the primary key, because with a VPN up the default route points away from the LAN and would pick the tunnel, reproducing the bug in a new costume. The loopback fallback survives for the case where nothing usable is found at all, and still binds and accepts nothing from outside.

**QUIC/UDP on restrictive WiFi is unverified**, and this is no longer an Android-specific risk — it used to sit in the Android delta list because Android was the untested half, and a first pair has now completed between a desktop and an Android phone over QUIC on a real LAN ([`session-1345`](../session-log/session-1345-first-live-pair-and-what-it-cost-to-believe-the-tools.md)). What that run did *not* exercise is a hostile network — AP/client isolation, multicast disabled, or two subnets — and it needed a VPN and a host firewall cleared before it worked at all, which is evidence that restrictive environments are where the remaining risk is, on every platform. This gates release, not development (#3507).

The user-facing setup snippets live in [`docs/features/sync.md § Pitfalls`](../features/sync.md).
