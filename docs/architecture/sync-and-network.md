...
| Pairing | EFF-wordlist 4-word passphrase OR QR code (v2 payload also carries the host's `endpoint_id` + bound `ip:port`, #4037); the passphrase proof rides inside the first `HeadExchange` (#855) |
...
**The joiner does not yet dial them.** Scanning extracts only the passphrase (`handleQrScan`), so on a network where mDNS multicast does not reach both devices (AP/client isolation, guest WiFi, multicast disabled, two subnets), a first pair still cannot be completed by any route the UI exposes. The remaining gap is that the daemon's `discovered` map is written by exactly one thing — a real mDNS event (`daemon_loop` Branch A; `start_with_lifecycle_seeded` is `cfg(test-util)`-gated so production cannot acquire a second writer) — and a `DiscoveredPeer` needs a `device_id`, which the QR does not carry. Closing it is a deliberate change to that invariant, tracked on #4037.
