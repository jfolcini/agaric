...
| Release artefacts | **Elevation of privilege** — a malicious updater payload runs at install time with installer privileges. | Low | High | The in-app signature check (minisign against the embedded pubkey) is the gate. SLSA provenance is the verification path for sophisticated users; OS-level code signing is intentionally not in place today (see the no-OS-signing tradeoff in [`ci-and-tooling.md`](ci-and-tooling.md#slsa-provenance-sigstore-and-the-unsigned-binary-tradeoff)). Windows SignPath OSS is in flight per [`PEND-41`](../../pending/PEND-41-ci-tooling-review.md) R3; macOS notarisation is a strict no-go for the current cycle per R11. | Mitigated (in-app) / Accepted (OS-layer first-launch UX) |
...
- **Sigstore-keyless updater signing** — adoption deferred pending upstream Tauri support maturity ([`PEND-41`](../../pending/PEND-41-ci-tooling-review.md) R22). When it lands, the B4 boundary's "leaked `TAURI_SIGNING_PRIVATE_KEY`" row downgrades from Accepted to Mitigated and the rotation procedure in [`SECURITY.md`](../../SECURITY.md#updater-signing-key-rotation) is rewritten around ephemeral Fulcio certificates.
- **Windows code signing (SignPath OSS)** — application in flight per [`PEND-41`](../../pending/PEND-41-ci-tooling-review.md) R3. When it lands, the B4 "first-launch UX" row tightens from Accepted to Mitigated on Windows.
- **macOS notarisation** — explicit no-go for the current cycle ([`PEND-41`](../../pending/PEND-41-ci-tooling-review.md) R11). The row remains Accepted until a downstream packager forces a revisit.
