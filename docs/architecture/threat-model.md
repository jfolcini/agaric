...
| Sync peer trust state | **Elevation of privilege** — an inbound and an outbound session with the same device overlap during the pairing window, because the per-peer lock key is asymmetric (#3511). | Low | Low | **Unresolved.** `HeadExchange` carries no `device_id`, and `get_local_heads` reads only `op_log`, so a fresh joiner with an empty log advertises no head of its own. The responder therefore falls back to locking on the endpoint id while the initiator locks on the device id, and S-5 — which exists to stop exactly that overlap — has a hole in precisely the window where both ends arm and dial simultaneously. It closes the moment a binding exists, and holds for every paired peer. The old stack did not have this gap: the certificate CN supplied a device id unconditionally. The clean fix (add `device_id` to `HeadExchange`, additive with `#[serde(default)]`) is a wire change across 63 construction sites and was deliberately not taken inside the cutover. | Open; tracked by #3511 |
...
- **The S-5 per-peer lock is asymmetric during the pairing window (#3511).** Tracked as a B3 row above. The model has not committed to either fix: adding `device_id` to `HeadExchange` (a wire change) or accepting the overlap.
...
- **`HeadExchange.wire_compression` is a live field that both ends ignore (#3512).** zstd compression retired with the chunking layer. This is not itself a security claim, but a protocol field that does nothing is a trap for the next reader, and the uncompressed 4× JSON expansion of Loro payloads against the 256 MB frame cap puts a hard offer failure at roughly 64 MB of state in one space — a threshold nobody has measured.
