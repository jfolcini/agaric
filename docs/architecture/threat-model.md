...
3. **Sync daemon ↔ LAN peer.** TLS with mTLS between paired devices; self-signed certificates pinned on first observation (TOFU). Pairing state lives in the database; the verification path is in [`src-tauri/agaric-sync/src/sync_daemon/server.rs`](../../src-tauri/agaric-sync/src/sync_daemon/server.rs) (see `verify_peer_cert`). The model assumes peers are the user's own devices; the LAN itself is **not** treated as a trusted medium even though the threat model treats adversarial-peer hardening as out of scope.
