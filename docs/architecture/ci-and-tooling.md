...
Three deliberate choices in every `Swatinem/rust-cache` invocation. First, **per-job-class `shared-key`s** — `rust-lint` (clippy/cargo-deny/sqlx-prepare), `rust-cov` (the llvm-cov instrumented cargo-tests tree), `rust-mcp` (the MCP release + debug-smoke tree), `rust-release` (the Linux desktop `cargo tauri build`, also restored by release.yml's Linux row), and `rust-android` (cross-compile targets, whose `target/` tree and rustup targets are disjoint from the host slot). rust-cache keeps ONE immutable entry per (os, shared-key, lockfile), so the earlier single `rust-deps` key shared across job classes meant only the first finisher's `target/` tree survived and every other class recompiled from scratch each run (issue #813). Second, **`save-if: ${{ github.ref == 'refs/heads/main' }}`** — only `main` writes to the cache; feature branches read it but never save. This stops every feature-branch churn from evicting the canonical baseline. Third, **`cache-bin: 'true'`** persists `~/.cargo/bin/` across runs so cargo-installed plugins (prek, sqlx-cli, tauri-cli) survive between runs of the same job class.

```yaml
save-if: ${{ github.ref == 'refs/heads/main' }}
```

**Cache rescue procedure.** When `Cargo.lock` changes in a way incompatible with cached `target/` artefacts (a corrupted incremental tree, a confusing linker mismatch), bump the suffix of the affected `shared-key` in `_validate.yml` / `ci.yml` (e.g. `rust-cov` → `rust-cov-v2`; if `rust-release` is bumped, update release.yml's matching restore key too). The next main-branch run repopulates from scratch; feature branches start reading from the new key on their next push. Documented here so future contributors don't re-derive it from a wedged pipeline.
