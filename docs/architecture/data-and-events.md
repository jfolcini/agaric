...
- **Fire-and-forget.** `AppHandle::emit` is a plain broadcast; there is no ack, no persistence, and no replay. `TauriEventSink::on_sync_event` (`src-tauri/agaric-sync/src/sync_events.rs:179-203`) and every other emit site log a `tracing::warn!` on failure but never propagate it — a missing/unmounted frontend does not block the backend.
