...
**Conclusion:** the persistent tier is not uniformly disposable. Class (2) (`ApplyOp`/`BatchApplyOps`) is load-bearing for correctness and structurally exercised on every boot with un-replayed local ops; class (1) (the cache rebuilds) is durability/staleness machinery whose removal is not obviously safe today because no general "rebuild on next boot" path exists to fall back on. See #2509 for the full investigation and #2471 for the still-open work of surfacing `bg_dropped`/`bg_dropped_global` (and, per this section, `fg_apply_dropped`/`fg_apply_dropped_persisted`) in the UI.
