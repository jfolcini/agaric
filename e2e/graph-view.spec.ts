...
test.describe('Graph view', () => {
  ...
  test('clicking a node navigates to that page', async ({ page }) => {
    ...
    await expect(nodeGroup).toBeVisible({ timeout: 5_000 })
    ...
    await expect(page.locator('[aria-label="Page title"]')).toBeVisible({ timeout: 5_000 })
  })
  ...
  test('graph view eventually renders after loading', async ({ page }) => {
    ...
    await expect(nodeGroups.first()).toBeVisible({ timeout: 5_000 })
