...
| --- | --- | --- | --- | --- | --- |
| **H1** — Canonical `BlockRow` SELECT clause repeated 34× across 15 files | HIGH | M (4-7 h) | low | medium-high | ⚠ deferred — `sqlx::query_as!()` is a proc-macro that does NOT expand `concat!(macro!(), "…")` arguments before parsing the SQL string (`expected string literal` error). The plan's "analogous to `descendants_cte_purge!()`" guidance applies only to dynamic `sqlx::query()` sites, not the compile-time `query_as!` family used at all 18 production sites. Closing H1 properly requires either (a) migrating all 18 sites to dynamic `sqlx::query_as::<_, BlockRow>(format!(…))` (loses compile-time SQL checking) or (b) deciding the duplication trade-off favors compile-time safety and keeping the canonical column list duplicated. Session 672 attempted the macro path, hit the proc-macro constraint, and reverted. Reopen with explicit migration strategy. |
| **H2** — `apply_op_tx` is a 387-line per-op dispatcher with deeply nested arms | HIGH | M (4-8 h) | low | medium | ✅ done session 672 — `apply_op_tx` body shrunk to a thin dispatch (~30 lines); 12 per-variant `apply_*_tx` private async helpers extracted in the same `materializer::handlers` module (`apply_create_block_tx`, `apply_edit_block_tx`, `apply_delete_block_tx`, `apply_restore_block_tx`, `apply_purge_block_tx`, `apply_move_block_tx`, `apply_add_tag_tx`, `apply_remove_tag_tx`, `apply_set_property_tx`, `apply_delete_property_tx`, `apply_add_attachment_tx`, `apply_delete_attachment_tx`); pure cut-and-paste extraction with no behavior change; the 100-line `PurgeBlock` body now owns its own helper. |
...
| **M2** — `set_property_in_tx` is ~258 lines mixing validation, op-log append, and materialization | MEDIUM | S (2-4 h) | low | medium | ✅ done session 673 — `validate_property_value(payload, declaration) -> Result<(), AppError>` private helper extracted in `commands/blocks/crud.rs` (~115 LOC, runs the 5-step validation pipeline: alphabet, ISO date, reserved-key field-shape, declared-type vs payload-shape, BUG-20 select-options membership); `set_property_in_tx` body shrunk from 243 → 153 total lines (132 code-only); dual-write to `blocks` columns + `BlockRow` return-construction kept inside per AGENTS.md hot-path-properties rule. Return type `Result<(), AppError>` (not `ValidatedPropertyValue`) — there's no kind+value pair consumed downstream; introducing a ceremonial struct would carry no information. New `PropertyDeclaration` struct (private, 2 fields) lifted out of the implicit row shape so the validator stays sync. 15 new unit tests cover the type × payload-shape matrix + reserved-key + select-options BUG-20 cases. |
...
| **M4** — `enqueue_background_tasks` is ~174 lines of imperative per-op cache-invalidation routing | MEDIUM | S (2-3 h) | low | low-medium | ✅ done session 672 — new `invalidations_for_op(record, hint) -> Result<Vec<MaterializeTask>, AppError>` helper expresses the per-op-type matrix as data; `enqueue_background_tasks` body shrunk to a thin loop over the returned Vec; 20 new tests pin the matrix (one per op type + edge cases like serde-error propagation, content-hint vs typed-hint branching, Arc sharing). |
