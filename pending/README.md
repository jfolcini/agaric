...
| block-history-sheet-fix-2026-05-14 | Block-history side panel is too narrow (`sm:max-w-sm`), filter bar wastes vertical chrome, inner ScrollArea sizing broken (LoadMoreButton falls off the bottom) — widen to `sm:max-w-lg`, compact the filter bar, add `SheetBody` slot mirroring the Dialog plan | S (~2 h) | ready |
