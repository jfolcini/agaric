...
| PEND-58c | **Pages compound filters — deferred follow-ups (from PEND-58b / PR #48).** Residual work punted out of the review remediation: gate `total_count` on the first page only (perf); add UI controls for path-exclude + property `ne`/`notExists` (P1-H option 2); implement the `Ref`/`Ne` `HasProperty` SQL (currently validation-rejected); the e2e-strengthening pass (orphan-grooming / zero-result / default-on / opt-out); a one-line `summaryProperty` simplification. No migrations, no IPC signature changes. | S-M (~7-10 h) | scoped; none blocking release |
