...
**Search post-review follow-ups (PEND-61 / 62 / 66).** Remaining work after the post-autonomous review session. **PEND-60** (caret autocomplete) shipped 2026-05-18. **PEND-61** (palette refactor + `search_blocks_partitioned` IPC) builds on the cmdk wrapper now shipped at `src/components/ui/command.tsx`. **PEND-62** (mobile unified search) depends on PEND-61. **PEND-66** is watch-and-act. Suggested order: 61 → 62.
