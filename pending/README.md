...
| PEND-56b | **Pages view materialisation + SortKeyset extraction (PEND-56 follow-up).** Round 2 perf review measured MostLinked / MostContent latency cliff at 20k pages (not 100k). Materialise `inbound_link_count` / `child_block_count` into `pages_cache`; extract a SortKeyset descriptor so the materialised-vs-computed swap is a one-line change per sort mode. | M (~6-10 h) | scoped; measurements in plan |
...
| PEND-62 | **Unified mobile search UX (one icon, scope toggle).** Collapses PEND-50 + 51 + 52 into one mobile sheet with `In this page` / `Across all pages` segments, context-aware default. Pure-touch users finally get in-page find. PEND-61 (cmdk palette refactor + partitioned IPC) is now shipped — this is unblocked. | M (~9-12 h) | scoped; 4 open Qs |
...
| PEND-68 | **Tablet / hardware-keyboard detection for the mobile search sheet.** Extends PEND-62's `< 768 px` icon-gate with a first-keydown probe so iPad-portrait without a keyboard shows the mobile sheet, while iPad-with-keyboard stays on Cmd+K. | S (~2-3 h) | scoped; 3 open Qs resolved; one acceptance matrix |
...
**Search post-review follow-ups (PEND-62 / 66).** Remaining work after the post-autonomous review session. **PEND-60** (caret autocomplete) shipped 2026-05-18. **PEND-61** (palette refactor to cmdk + `search_blocks_partitioned` IPC + multi-mode shell) and **PEND-67** (palette power-user polish — all 8 phases) shipped 2026-05-19. **PEND-62** (mobile unified search) is unblocked now that PEND-61's `CommandPalette` lives. **PEND-66** is watch-and-act.
