...
| PEND-60 | **Caret-anchored autocomplete on cmdk.** Ships the popover PEND-54 deferred. When typing `tag:` / `state:` / `prop:` etc., a popover anchors next to the caret showing value suggestions. | M (~6-8 h) | scoped; 3 open Qs |
| PEND-61 | **Refactor Cmd+K palette to cmdk + multi-mode growth + `search_blocks_partitioned` IPC.** Replace hand-rolled PEND-51 palette with a cmdk shell; opens the door for Linear-style multi-mode (search / commands / nav / spaces / agents). Backend gains `search_blocks_partitioned` (one FTS scan instead of two). | M-L (~10-14 h) | scoped; 3 open Qs |
| PEND-62 | **Unified mobile search UX (one icon, scope toggle).** Collapses PEND-50 + 51 + 52 into one mobile sheet with `In this page` / `Across all pages` segments, context-aware default. Pure-touch users finally get in-page find. Depends on PEND-61. | M (~9-12 h) | scoped; 4 open Qs |
...
**Search post-review follow-ups (PEND-60 → 66).** Remaining work after the post-autonomous review session. **PEND-60** (caret autocomplete) and **PEND-61** (palette refactor + `search_blocks_partitioned` IPC) build on the cmdk wrapper now shipped at `src/components/ui/command.tsx`. **PEND-62** (mobile unified search) depends on PEND-61. **PEND-66** is watch-and-act. Suggested order: 60 → 61 → 62.
