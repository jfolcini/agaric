...
| PEND-40 | Supply-chain hardening follow-ups (SLSA build provenance, `--locked` audit sweep, `cosign verify`, threat-model doc) | S-M (~6-10 h) | partial-shipped (SHA-pin all actions + Dependabot, `npm audit signatures`, `cargo audit` hook, `.nsprc` expiry, `npx --yes` removed); 4 stretch items outstanding |
