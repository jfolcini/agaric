...
| PEND-58d | **Pages view hardening — deep-review findings + full test/e2e coverage.** From a 5-perspective review + adversarial verification of the whole feature. P1: `PathGlob` full-scans the cache & is documented backwards & untested; materialised `inbound_link_count` counts same-page links (corrupts Orphan/MostLinked/badge); default sort only orders the loaded page; cost-reorder/bind-renumber untested. Plus P2/P3 robustness/UX/maintainability fixes, and a comprehensive e2e suite covering **every** Pages-view feature (filters, sort, density, pagination, CRUD, count chip, a11y). Supersedes PEND-58c. One migration backfill (inbound count); one possible IPC type reshape. | M-L (~28-43 h) | **P1+P2+P3 shipped** (Sessions 810–811: D1–D27 incl. HasProperty `predicate` reshape, perf gate, count-basis, clear-all, date validation, Orphan/inbound same-page exclusion; all backend/frontend unit + tauri-mock tests). **Remaining:** comprehensive e2e suite + deferred D23a (fold-`<mark>` bounding) |
