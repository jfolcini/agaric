...
| PEND-67 | **Command Palette power-user polish (commands mode v2).** Closes the gap to Linear / Raycast / Notion / VSCode parity: inline per-command shortcuts, recent commands, `#` / `?` quick-action prefixes, pinned recents, Tab-opened action menu, per-mode persistent query, numeric prefix jumps (1-9), run-last-command (`⌘.`). 8 small phases, each independently shippable. | M-L (~14-20 h) | scoped; 4 open Qs |
...
**Search post-review follow-ups (PEND-62 / 66 / 67).** Remaining work after the post-autonomous review session. **PEND-60** (caret autocomplete) shipped 2026-05-18. **PEND-61** (palette refactor to cmdk + `search_blocks_partitioned` IPC + multi-mode shell) shipped 2026-05-19. **PEND-62** (mobile unified search) is unblocked now that PEND-61's `CommandPalette` lives. **PEND-66** is watch-and-act. **PEND-67** (palette power-user polish) is the optional best-in-class follow-up scoped after PEND-61's senior CR; pick any phase independently. Suggested order: PEND-67 phases 1, 2, 6, 7 (cheap wins) → PEND-62 → PEND-67 phases 3, 4, 5, 8.
