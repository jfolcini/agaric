...
| PEND-77 | **Property-test coverage gaps.** **Tier A shipped** (2026-05-24): `word_diff::compute_word_diff` reconstruction invariants + `space_filter_canonical::normalize` idempotence / whitespace-equivalence. **Remaining — Tier B** (DB-bound; needs a seeded-tree fixture harness): `reverse::compute_reverse` (inverse law), `dag` walk/`find_lca` (termination/cycle/commutativity), `soft_delete` cascade/restore, block-descendant/position CTEs (closure). Test-only. | B: M-L (harness is the bulk) | Tier A done; Tier B deferred |
...
2. ✅ **PEND-78 / PEND-79 / PEND-77 Tier A** — quick wins shipped together on `pend-78-79-77-quick-wins`: recent-strip cross-space leak fix, AppImage Linux desktop self-integration, and `word_diff` + `space_filter_canonical` property tests. PEND-77 **Tier B** (DB-bound property tests behind a seeded-tree fixture harness) stays deferred.
