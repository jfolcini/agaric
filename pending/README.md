...
| PEND-63 | **Wire `not-state:` / `not-priority:` proper inversion.** PEND-53 ships visual-only chips; this PEND wires them to backend with `(state IS NULL OR state NOT IN (…))`. Small backend SQL + tests. | S (~2-3 h) | scoped; 2 open Qs |
| PEND-64 | **`prop:KEY=VALUE` matches all four typed columns.** PEND-53 ships `value_text` only; this PEND extends to `value_num` / `value_date` / `value_ref` via type-coerced bind variants. Small backend SQL + tests. | S (~2-3 h) | scoped; 3 open Qs |
| PEND-65 | **MCP `search` tool: thread full `SearchFilter` through to agents.** PEND-54+53 hard-coded empty filter fields at the MCP boundary; agents can only search by query string today. Add structured `filter` arg to the MCP tool schema. | M (~3-4 h) | scoped; 3 open Qs |
...
**Search post-review follow-ups (PEND-59 → 66).** The post-autonomous review session decided to adopt cmdk and fix six smaller gaps. Dependency graph: **PEND-59** (cmdk foundation) unblocks PEND-60 (caret autocomplete), PEND-61 (palette refactor + `search_blocks_partitioned` IPC), and PEND-62 (mobile unified search). **PEND-63 / 64 / 65** are independent small backend fixes. **PEND-66** is watch-and-act. Suggested batching: ship PEND-63 + 64 + 65 as one small backend cycle; ship PEND-59 then 60 + 61 in series; PEND-62 last because it depends on both.
