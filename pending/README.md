...
| PEND-67 | **Command Palette power-user polish (commands mode v2).** Closes the gap to Linear / Raycast / Notion / VSCode parity. Phases **1, 2, 6, 7 shipped 2026-05-19** (inline shortcut chips, recent commands strip, per-mode persistent query, numeric prefix jump). Remaining: phases 3, 4, 5, 8 (`#` / `?` prefixes, pinned recents, right-rail action menu, run-last-command `⌘.`). | M-L (~14-20 h) total; ~7-10 h remaining | partial; 4 open Qs |
