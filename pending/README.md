...
| PEND-29 | Frontend robustness review — second pass (post-validation) | mostly trivial; one S (B-1 Option A) | ready (1 MEDIUM + 8 LOW; complementary to PEND-22, zero overlap — both validation passes confirmed PEND-22's fixes are in place at HEAD. Headline: B-1 `BulletList` extension registered but no serializer / parser → real (toast-warned) data loss; recommend Option A removal. ~30 hallucinations + ~20 nits rejected by validation, all listed in body) |
...
- PEND-29 (Frontend robustness review — second pass) — independent of PEND-22 (zero overlap). B-1 (`BulletList` extension) is the only MEDIUM and is the headline; needs a product decision (Option A remove vs Option B implement) before it can land. B-2..B-10 are all surgical low-severity fixes (~80–120 LOC across 9 files) that can ship as a single bundle commit ~1.5-2.5h.
