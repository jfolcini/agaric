...
| gcal-oauth-connect-wiring-2026-05-13 | "Connect Google account" silently fails (frontend invokes the `begin_gcal_oauth` Tauri command that doesn't exist). Wire up the desktop loopback OAuth transport so the existing connector picks up freshly-keyringed tokens. | M | ready |
...
| block-history-sheet-fix-2026-05-14 | Block-history side panel is too narrow + crowded — widen Sheet to `sm:max-w-lg`, compact filter bar, add `SheetBody` slot mirroring the Dialog plan, **and** drop the non-reversible lock chip onto its own row (today it competes for width with date+device on every create_block) | S (~2.5 h) | ready |
