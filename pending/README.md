...
| PEND-56 | **Pages view foundation — density rows + extended sort modes.** New `list_pages_with_metadata` sibling IPC adds per-page metadata columns (last-modified, inbound-link count, child-block count, has-property flags); cursor v1 → v2 for new keysets; sort dropdown extends from 3 to 7 modes; density toggle (compact / regular / expanded) gated by a `pageBrowser.densityV1` flag. Foundation for PEND-57 / PEND-58. | M-L (~22 h) | **partially shipped**: Phase 1 backend IPC (`c88b4d26`) + Round 1 fixes (`16adad2f`) + Phase 2 sort/density hooks (`fcd7b08a`); Phase 3 PageBrowser integration remains. |
...
| PEND-58d | **Pages view hardening — deep-review findings + full test/e2e coverage.** From a 5-perspective review + adversarial verification of the whole feature. P1: `PathGlob` full-scans the cache & is documented backwards & untested; materialised `inbound_link_count` counts same-page links (corrupts Orphan/MostLinked/badge); default sort only orders the loaded page; cost-reorder/bind-renumber untested. Plus P2/P3 robustness/UX/maintainability fixes, and a comprehensive e2e suite covering **every** Pages-view feature (filters, sort, density, pagination, CRUD, count chip, a11y). Supersedes PEND-58c. One migration backfill (inbound count); one possible IPC type reshape. | M-L (~28-43 h) | **P1+P2+P3 shipped** (Sessions 810–811: D1–D27 incl. HasProperty `predicate` reshape, perf gate, count-basis, clear-all, date validation, Orphan/inbound same-page exclusion; all backend/frontend unit + tauri-mock tests). Comprehensive e2e suite shipped (Session 812: `e2e/pages-view.spec.ts`, 55 tests). **Remaining:** deferred D23a (fold-`<mark>` bounding) + the triaged deep-review findings (presented to the maintainer; not yet actioned). |
| PEND-58e | **Pages view — deep-review findings (post-verification).** Second 5-perspective review of the whole Pages view after 58d, with a 3-verifier adversarial round (each finding CONFIRMED/OVERSTATED/REFUTED against the code). **P1:** Priority facet offers `A/B/C` while data is `1/2/3` → matches nothing out of the box; `docs/PAGES.md` self-contradicts the shipped D24 controls. **P2:** `LastEdited` custom Range drops end-day edits (data loss); cross-page `MoveBlock` leaves `child_block_count` stale; `tag:` is id-only but invites a name; filter pills never truncate; count chip miscounts in namespaced/starred vaults; `filters.md` shows the pre-D8 `HasProperty` shape. **P3 (~13):** test-efficacy gaps (EXPLAIN-of-a-copy, parity shared-shape blind spot), tauri-mock fidelity gaps, perf-gate realism, UX/a11y/doc nits. Refuted/downgraded items recorded so they aren't re-litigated. No migrations. | M-L (~12-20 h) | triaged + verified; not yet actioned (several P1/P2 are pre-existing from PEND-58) |
...
**Search overhaul — fully shipped.** PEND-60 (caret autocomplete) shipped 2026-05-18. PEND-61 (palette refactor to cmdk + `search_blocks_partitioned` IPC + multi-mode shell), PEND-67 (palette power-user polish), and **PEND-62 (mobile unified search)** all shipped 2026-05-19 via the cmdk PR. **PEND-66** is the only remaining search-area item — watch-and-act on `execCommand` deprecation, no immediate action.
