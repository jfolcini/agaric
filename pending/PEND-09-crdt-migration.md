...
| **Tree-op conflict resolution semantics are loss-prone under LWW** (two devices reparent the same block to different parents → one device's intent is silently dropped) | Spike evaluated `LoroTree` (Day 5 — same LWW one-parent-wins state-level semantics as LoroMap+scalar). Phase 1 stays on `LoroMap` + scalar parent_id; the LWW resolution rule (property + move) is documented in `ARCHITECTURE.md` §12 "LWW resolution rule (property + move conflicts)". Optional "remote-overrode-your-move" awareness log is logged as Phase-4 polish. |
