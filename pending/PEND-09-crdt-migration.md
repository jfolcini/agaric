...
| **Tree-op conflict resolution semantics are loss-prone under LWW** (two devices reparent the same block to different parents → one device's intent is silently dropped) | Spike must evaluate `LoroTree` (if stable). If staying on `LoroMap` + scalar parent_id, define and document the LWW resolution explicitly so users aren't surprised. Consider keeping a short audit log of "your move was overridden by remote" events to preserve awareness. |
