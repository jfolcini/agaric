...
This doc captures the project's **Last-Writer-Wins (LWW) rule** for two specific kinds of concurrent operation: `set_property` writes to the same `(block_id, key)` pair, and `move_block` ops on the same block. It is the design-doc artefact called for by `pending/PEND-09-crdt-migration.md` Q5 (line 201) and the Phase-1 readiness checklist item 9 in the archived spike report (`git tag pend-09/spike-archive`; verdict summarised in `SESSION-LOG.md` Session 698).
...
This is the same UX surface as Phase 0 + Phase 1 of the Loro migration: `LoroMap` + scalar value uses LWW with key-write timestamps, and produces no user-visible conflict event for property writes. The user's only signal that the rule fired is the absence of a property value they thought they set — and even that is only visible if they happen to look. (The archived spike notes' day-3 corpus, preserved at `git tag pend-09/spike-archive`, confirms ~17 of the 53 `merge/tests.rs` cases exercise LWW-tiebreak paths and all converge silently — see `SESSION-LOG.md` Session 698 Phase 0 day-3 entry.)

A future "remote-overrode-your-move" awareness log is logged as a Phase-4 polish in `pending/PEND-09-crdt-migration.md:121` and the archived spike report's §5 LWW-tree-op risk row (`git tag pend-09/spike-archive`); it is **not** part of the Phase-1 surface.
...
The archived Phase 0 spike report (`git tag pend-09/spike-archive`) §3 Q5 is the load-bearing reference (excerpt below; spike verdict summarised in `SESSION-LOG.md` Session 698 Phase 0 days 2-7):
...
So: `LoroMap` + scalar value uses **LWW with key-write timestamps**. In normal cases (archived spike report §2 row 2: bucket A = byte-identical or B = Loro better — see `git tag pend-09/spike-archive`), Loro's per-key LWW resolves to the same winner the SQL dispatcher does, because the timestamps Loro stamps on key writes are derived from the same underlying clock the `created_at` column carries. The day-3 parity port (archived spike report §3 Q5 + spike `tests/parity_corpus.rs`, all preserved at `git tag pend-09/spike-archive`) confirms this: 13/15 cases A+B, two C cases documented as expected CRDT semantics (RGA-CRDT identical-edit doubling and Lamport-vs-wallclock LWW tiebreak — neither a property/move LWW disagreement).
...
- `git tag pend-09/spike-archive` (archived Phase 0 spike report §3 Q5, §5 LWW-tree-op risk row) — the spike-side equivalence; verdict summary in `SESSION-LOG.md` Session 698.
