...
- **Phase 5 — Test gaps (S, ~3 h).** NFC normalisation, mid-emoji surrogate, all-operator queries, FTS-drift `verify_fts_consistency` integration test, one desktop palette e2e. **Status: PARTIAL — SHIPPED 2026-05-20** (T1b mid-emoji surrogate; T1c all-operator pin-current-behaviour test; T1d FTS-drift helper + two integration tests; T2 three desktop palette Playwright specs). T1a (NFC normalisation) DEFERRED — depends on B3 (NFC normalisation implementation) which is also deferred; pair both together when a real NFC/NFD bug surfaces.
...
| B8 | `src-tauri/src/fts/search.rs:500`; `src-tauri/src/fts/toggle_filter.rs:541` | `(SELECT COUNT(DISTINCT bt.tag_id) FROM block_tags … IN (…)) = ?N` per result row. Should be `EXISTS`-per-tag. **FINAL DEFER 2026-05-20** per the plan's "measured, no-op" escape. Total cost (~6 h): build a runnable 10k-block bench harness; implement the dynamic per-tag EXISTS rewrite across two call sites; measure both shapes; decide. The win is data-shape-dependent (scales with the number of AND'd tags, the cardinality of `block_tags` per block, and the read pool's cache state) and may not exceed the plan's 5% threshold. Both call sites are READ-only and not user-visibly slow today. Revisit when a real user-facing slowdown surfaces and the bench harness is justified by debugging that slowdown. |

### Frontend still open

| # | File:line (HEAD) | Issue |
|---|---|---|
| ~~U1~~ | ~~`CommandPalette.tsx:482, :1540`; `useAutocompleteSources.ts:123`~~ | ~~IPC failures `logger.warn` only — no toast, no inline status.~~ **SHIPPED 2026-05-20** — `useFailedOnce` hook (in-memory `Set<string>` keyed by surface) added; three palette catch sites now `surfaceFailureOnce('palette:search' / 'palette:tags' / 'autocomplete:tags', () => notify.error(t('search.failed')))`. Cancellation half is still swallowed pre-toast via `isCancellation`. `usePaginatedQuery` already toasts on every onError-tagged failure (no gating needed there — it's the search-panel's existing user-visible signal). |
| ~~U2~~ | ~~`SearchHistoryDropdown.tsx:72-74`~~ | ~~History rows `role="option" aria-selected={false} tabIndex={-1}` — no roving focus, no `aria-activedescendant`.~~ **SHIPPED 2026-05-20** — `useSearchHistoryCycling` now exposes `activeIndex` (-1 = typing, ≥0 = browsing); `SearchHistoryDropdown` takes `listboxId` + `activeIndex` props, sets the listbox `id`, gives each row a stable `${listboxId}-opt-${idx}` id, and reflects active state via `aria-selected={idx === activeIndex}`. `SearchPanel`'s `inputComboboxAttrs` wires `aria-controls` + `aria-activedescendant` to the history listbox when no autocomplete is active (the two are mutually exclusive by visibility gate). 4 new tests assert the listbox/row id format and the aria-selected propagation. |
...
| ~~U8~~ | ~~`src/stores/useCommandPaletteStore.ts:135-136` + `CommandPalette.tsx:181`~~ | ~~`open$()` captures `activeElement` but not `Range`/`Selection`; `[[page]]` insertion re-reads `selectionStart` at insert time.~~ **SHIPPED 2026-05-20** — `previousSelectionRange: Range \| null` added to the store; `open$()` clones the current selection range (try/catch guards jsdom + sandboxed-iframe edge cases); `close()` clears it. `insertPageLinkInto` accepts the snapshot and restores it before `execCommand('insertText')` on contenteditable targets so the link plants at the user's original caret even after palette focus stole the selection. Native input/textarea path unchanged (those have direct `selectionStart`/`End`). |
| U9 | `useSearchSheetStore` + `useCommandPaletteStore.queryByMode` + `useInPageFindStore.lastQuery` | Three mode-memory layers; segment switches don't carry query into new mode's memory. **FINAL DEFER 2026-05-20** — per the plan's open question 3 ("if Phase 3.U9 grows beyond ~2 h, defer"). Actual scope ~4-5 h: sheet store would need a `paletteQueryByMode: Record<PaletteMode, string>` field, bridge would need bi-directional mirror, palette store would need a `setQueryByModeFromSheet(map)` method, plus tests for every cross-store flow. The marginal UX win (palette mode memory survives a sheet round-trip) doesn't justify the cross-store coupling cost. Bridge already mirrors the VISIBLE query through `useSearchSheetBridge`; only per-mode palette memory beyond the current mode is lost. Revisit only if a user reports the missing-memory friction. |
...
| ~~M3~~ | ~~`CommandPalette.tsx:422, :1512`; `useAutocompleteSources.ts:83`~~ | ~~Three implementations of the generation-counter race-discard pattern.~~ **SHIPPED 2026-05-20** — new `src/hooks/useGenerationGuard.ts` exports `{ next, isCurrent }` (memoised so the returned object is dep-stable). All three sites migrated; 7 unit tests added. Autocomplete tag-debounce path keeps this even after the eventual `AbortController` migration (the debounce is its own race, not the IPC). |
...
| ~~M6~~ | ~~`CommandPalette.tsx:319-332`~~ | ~~10 individual store selectors.~~ **SHIPPED 2026-05-20** — collapsed the 8 individual `useCommandPaletteStore((s) => s.*)` calls in `PaletteBody` into a single `useCommandPaletteStore(useShallow((s) => ({ … })))` selector. Matches the existing `SearchSheet.tsx:44` pattern. zustand bails out at the top of the selector when no watched field changed (vs. the previous 8 separate equality checks per commit). The 2 selectors in the outer wrapper component (`open`, `closeStore`) are left as-is — small enough that bundling adds noise. |
...
| ~~R2~~ | ~~`useSearchSheetBridge.ts:102-106`~~ | ~~Bridge write-back relies only on `q !== query` check. Future writer triggers ping-pong.~~ **SHIPPED 2026-05-20** — `lastWroteRef` tracks the most recent value the bridge wrote into the segment store; the subscription's listener suppresses the echo of our own seed write before falling through to the existing `q !== sheet.query` guard. Structural rather than incidental defence; ref is cleared on the next non-echo update so subsequent real keystrokes mirror through normally. |
...
| ~~R4~~ | ~~`src/lib/tauri.ts`~~ | ~~Zero `AbortController` / `AbortSignal` hits. Backend `CancellationToken` is never signalled from the UI — PEND-70's cancellation is one-sided.~~ **SHIPPED-AS-LIBRARY 2026-05-20** — `cancelledError(reason)` + `withAbort(promise, signal?)` exported from `lib/tauri.ts`. The rejected value flows through `isCancellation()` cleanly (same `{ kind: 'cancelled', message }` shape the backend emits). Tauri 2's `invoke()` itself doesn't honour the signal server-side (the IPC stays open until the Rust future resolves) — so this is a CLIENT-side stop-waiting primitive. Existing consumers NOT migrated: `useGenerationGuard` already discards stale responses correctly and the user-visible win was already paid by the cancellation swallow in Phase 2. New consumers can reach for `withAbort` from day 1 for cleaner code. 7 unit tests cover the resolve/reject/already-aborted/mid-flight-abort/undefined-signal/isCancellation-predicate matrix. |
...
| ~~T2~~ | ~~One desktop palette Playwright spec: `Cmd+K` open → type → arrow → Enter → navigate; plus `[[page]]` insertion happy path.~~ **SHIPPED 2026-05-20** — `e2e/palette-desktop.spec.ts` with three specs: happy-path navigate, Escape closes without nav, palette opens from editor-view-with-sidebar-focused (PEND-51 context-gating guard). `[[page]]` insertion already covered by `inner-links.spec.ts` + `autocomplete.spec.ts`; not duplicated. |
