...
| # | File:line (HEAD) | Issue |
|---|---|---|
| U1 | `CommandPalette.tsx:482, :1540`; `useAutocompleteSources.ts:123` | IPC failures `logger.warn` only — no toast, no inline status. |
| U2 | `SearchHistoryDropdown.tsx:72-74` | History rows `role="option" aria-selected={false} tabIndex={-1}` — no roving focus, no `aria-activedescendant`. |
| U3 | `SearchResultBlockRow.tsx:107-113` | Dead `onKeyDown` on `<li>` that nothing focuses. |
| U4 | `CommandPalette.tsx:362-364`; `SearchPanel.tsx:482-484` | `useEffect` autofocus → one-frame flash on slow mounts. |
| U5 | `SearchPanel.tsx:851-856` | `window.setTimeout(() => setInputFocused(false), 150)` to defer blur. |
| U6 | `docs/SEARCH.md:21-24` | Says palette fires "two parallel `searchBlocks` calls". Code: one `searchBlocksPartitioned` since `f665452f`. |
| U7 | `docs/SEARCH.md:325-326` | Claims `<CommandPalette variant="embedded" />`. Code mounts `<PaletteBody>`; no `variant` prop exists. |
| U8 | `src/stores/useCommandPaletteStore.ts:135-136` + `CommandPalette.tsx:181` | `open$()` captures `activeElement` but not `Range`/`Selection`; `[[page]]` insertion re-reads `selectionStart` at insert time. |
| U9 | `useSearchSheetStore` + `useCommandPaletteStore.queryByMode` + `useInPageFindStore.lastQuery` | Three mode-memory layers; segment switches don't carry query into new mode's memory. |
| U10 | `SearchPanel.tsx:324-332` | localStorage failure swallowed silently — filter-syntax toast re-fires every mount in private-mode browsers. |
| M3 | `CommandPalette.tsx:422, :1512`; `useAutocompleteSources.ts:83` | Three implementations of the generation-counter race-discard pattern. |
| M4 | `src/components/SearchPanel/SearchResultList.tsx` (88 LOC) | Unused since SearchResultGroups took over. |
| M5 | `src/components/SearchPanel.tsx:825` | `void tokenKey` of an unused import. |
| M6 | `CommandPalette.tsx:319-332` | 10 individual store selectors. |
| M7 | `AutocompletePopover.tsx:144-147` | `useEffect(() => { syncAriaIds() })` no-deps — runs after every commit. Redundant with `SelectedItemBridge`. |
| R1 | `src/stores/search-history.ts:70-74` | `version: 1` with no `migrate` fn — future bump silently wipes history. |
| R2 | `useSearchSheetBridge.ts:102-106` | Bridge write-back relies only on `q !== query` check. Future writer triggers ping-pong. |
| R3 | `src/lib/bindings.ts:828-831` | `AppError = { kind: string, message: string }` — no `Cancelled` literal narrowing. Backend variant exists at `src-tauri/src/error.rs:144` but frontend can't discriminate. |
| R4 | `src/lib/tauri.ts` | Zero `AbortController` / `AbortSignal` hits. Backend `CancellationToken` is never signalled from the UI — PEND-70's cancellation is one-sided. |
| P1 | `SearchResultBlockRow.tsx` | No `React.memo`. Every focus move re-renders every visible row. |
| P2 | `SearchPanel.tsx:752` + `:237` | `groupResultsByPage(results, pageTitles)` memo invalidates on every breadcrumb fetch — `pageTitles` is a `useState`'d `Map` rebuilt with `new Map(...)`. |
