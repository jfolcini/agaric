...
This doc captures the **front-end ↔ back-end edit-coordinate boundary**: who owns the in-progress edit while a user is typing, what happens when a remote sync arrives mid-edit, and where the authority transfers. It is the design-doc artefact called for by the Phase 0 spike report's §6 readiness checklist item 10 — "Confirm the FE edit-coordinate space" — and by spike notebook Q10 (spike day-2). The spike's notes + final report are preserved at `git tag pend-09/spike-archive`; the Phase 0 verdict is summarised in `SESSION-LOG.md` Session 698.
...
In Phase 2.5 / Phase 3, **a future revisit may want richer merge semantics on the FE.** Specifically, `LoroText` supports character-level merge (the spike's day-2 demo, preserved at `git tag pend-09/spike-archive`, demonstrates two peers converging on `"The slow brown dog"` from divergent edits to `"The quick brown fox"` — see `SESSION-LOG.md` Session 698 Phase 0 day-2 entry). If we wanted the user to see the remote's in-band insertions appear in their in-progress edit (Notion-style live merge), we would need to:

- Subscribe the FE editor to Loro doc updates (a `subscribe()` callback on the per-space `LoroDoc`).
- Bridge Loro's USV-coordinate edits (`LoroText::insert/delete/splice` — see the archived spike's day-2 demo at `git tag pend-09/spike-archive` plus notebook Q10) into TipTap's UTF-16 ProseMirror transactions; TipTap / contenteditable typically emits UTF-16, Loro speaks USV.
...
This is the **flagged-for-Phase-2 work** that the archived spike report's §6 item 10 calls out (the spike report is preserved at `git tag pend-09/spike-archive`; verdict in `SESSION-LOG.md` Session 698 Phase 0 day-7 entry). It is **not** Phase 1 scope. Phase 1's deliverable is "the FE coord-space contract is documented and the BE shadow-engine doesn't disturb it" — both true today.

## See also

- `git tag pend-09/spike-archive` (the Phase 0 spike report's §6 readiness item 10) — the open question this doc answers; verdict summary in `SESSION-LOG.md` Session 698.
- `git tag pend-09/spike-archive` notebook Q10 — the USV vs UTF-16 coordinate-coercion question for the eventual FE-side Loro bridge.
- `pending/PEND-09-lww-resolution-rule.md` — companion doc covering property + move LWW.
- `git tag pend-09/spike-archive` day-2 LoroText demo — motivates Phase 2's live-merge possibility (summarised in `SESSION-LOG.md` Session 698 Phase 0 day-2 entry).
