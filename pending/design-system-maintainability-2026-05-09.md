...
| ~~`src/components/BlockTree.tsx`~~ | ~~790~~ → 697 | **closed Session 727** — the "recursion + DnD overlay" framing in the audit was already addressed across earlier sessions (`getDragDescendants` lives in `src/lib/tree-utils.ts`; collapse / zoom tree-flattening lives in `useBlockCollapse` + `useBlockZoom`; the DnD overlay shipped as `block-tree/BlockDndOverlay.tsx`; the DnD wiring itself sits in `useBlockDnD.ts`). The remaining 802-LOC file was glue: 30+ hook calls held together by cross-callback ref indirections (`useRovingEditor` ↔ `useBlockNavigateToLink` ↔ `handleFlush`) where strict creation order matters. Three additional seams carved out into `src/components/block-tree/`: (a) `use-block-flush.ts` — the `handleFlush` callback (parser-driven multi-block split + checkbox-syntax → todo-state persistence + optimistic `pageStore.setState`); (b) `use-block-auto-create-first-block.ts` — the H-9 "empty page gets an empty content block" effect with its per-page idempotency ref; (c) `use-block-tree-context-bags.ts` — memoised `BlockActions` (14 callbacks) + `BlockResolvers` (4 resolvers) bags published via `BlockActionsProvider` / `BlockResolversProvider`. Each hook's parameter shape mirrors the previous inline implementation 1:1 so memo dep arrays stay observable at the call site. Public exports unchanged (`BlockTree`, `processCheckboxSyntax`, `guessMimeType`). Perf-critical pieces flagged in the task brief — `useRovingEditor` / `useViewportObserver` return-value memos, `MeasuringStrategy.WhileDragging`, `SortableBlockWrapper`'s `React.memo`, `BlockListItem.metadata` typed primitives — all left in place untouched. 209/209 BlockTree tests + 9746/9746 full suite green; `npx tsc -b --noEmit` clean. |
