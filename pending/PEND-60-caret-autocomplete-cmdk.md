...
> **Status (2026-05-18):** Phase 1 shipped on branch `pend-60-phase1-caret-autocomplete`. Caret-anchor utility (`src/lib/caret-anchor.ts`), popover component (`src/components/search/AutocompletePopover.tsx`), SearchPanel wire-up, and static value lists for `state:` / `priority:` / `due:` / `scheduled:` are landed with 24 new frontend tests (7 unit + 9 component + 8 integration). Phases 2 (dynamic sources for `tag:` / `prop:` / `path:`) and 3 (Playwright e2e + `docs/SEARCH.md` update) remain open. UX review flagged one follow-up: ARIA combobox wiring on the input (`role="combobox"` + `aria-expanded` / `aria-controls` / `aria-activedescendant`) — non-blocking, fold into Phase 2 a11y polish.
>
