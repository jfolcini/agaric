...
> **Status (Session 678):** Phase 0 + Phase 1 both LANDED. `SpaceId` newtype + `SpaceScope` tagged enum live in `src-tauri/src/space.rs` (production-shape: full `Display`/`AsRef<str>`/`From<SpaceId> for String` + 6 `PartialEq<&str>/<str>/<String>` and reverse impls, mirroring `ActiveBlockId`). Spike probe removed; 17 inline tests cover from_string/from_trusted/serde/sqlx/as_filter_param/PartialEq. Specta + sqlx round-trip validated in Phase 0; `SpaceId`/`SpaceScope` are not currently emitted in `bindings.ts` (no command takes them yet — they reappear in Phase 3 when real commands take `SpaceScope`). **Next:** Phase 2 migrates `_inner` functions in domain order (agenda → blocks → pages → tags → history → queries → pagination → backlink → FTS → MCP) — `space_id: Option<String>` parameter becomes `scope: SpaceScope`, SQL stays byte-identical, `scope.as_filter_param()` at the bind site replaces `space_id`. Phases 2.5/3 unchanged from the plan body below.
