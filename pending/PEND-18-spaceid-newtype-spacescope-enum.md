...
> **Status (Session 678):** Phase 0 + Phase 1 + Phase 2 all LANDED. `SpaceId` newtype + `SpaceScope` tagged enum live in `src-tauri/src/space.rs` with full impl surface (`Display`, `AsRef<str>`, `From<SpaceId> for String`, 6 PartialEq impls mirroring `ActiveBlockId`). Phase 2 migrated all ~16 production `_inner` functions across `commands/{agenda,blocks/{crud,queries},queries,pages,tags,history}.rs` + `mcp/tools_ro.rs` + `mcp/tools_rw.rs::handle_create_page` + `gcal_push/connector.rs` from `space_id: Option<String>` to `scope: &SpaceScope`. Tauri command wrappers translate IPC-side `Option<String>` → `SpaceScope` at entry; `_inner` SQL bind sites use `scope.as_filter_param()` (returns `Option<&str>`, byte-identical to pre-migration). 6 parity tests assert `&SpaceScope::Global` ≡ pre-migration `None` for the most-used `_inner` per domain (agenda, blocks, queries, pages, tags, history). Pagination/backlink/fts/tag_query helpers stay on `Option<&str>` per plan — Phase 2 is type-safety at the IPC + commands layer; helpers are an optional follow-up. **Next:** Phase 3 (IPC boundary + frontend `tauri.ts` wrappers — atomic commit with `bindings.ts` regen) lifts `space_id: Option<String>` IPC-side to `SpaceScope`. ~2-3h.
