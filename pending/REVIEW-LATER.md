...
> **Last updated:** 2026-05-04 (Session 667 — closed PEND-28b H1 + H2 + H3 + H4 + H5 + H6 + M2 + M3 + M4 + M5 + M7 in one batch (11 sub-items: ALL 6 HIGH + 5 MEDIUM): primitive `max-h-[calc(100dvh-Nrem)]` on Dialog/AlertDialog/Popover (propagates everywhere), BlockPropertyDrawer `w-3/4 sm:w-80`, BlockPropertyEditor portal viewport cap, AddFilterRow `[@media(pointer:coarse)]:h-11`, FormattingToolbar 3 popovers explicit cap (defense-in-depth — they were already inheriting), DuePanelFilters min-w, AgendaResults priority text-sm, AgendaSortGroupControls text-sm, PageAliasSection sm:w-32, DiffDisplay flex-wrap. +66 frontend tests. PEND-28b status: 0 HIGH + 8 MEDIUM (M1, M6, M8-M13) + 1 LOW remaining. 9456+/9456+ vitest + 3510/3510 nextest + `prek run --all-files` clean.)
