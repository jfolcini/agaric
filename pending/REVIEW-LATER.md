...
> **Last updated:** 2026-05-05 (Session 676 — review pass over session 675's PEND-17 Part B + PEND-33 Layer B work via 4 parallel review subagents (2 technical + 2 UX). **3 critical fixes applied by reviewers**: (1) **P0** — `BlockHistoryItem` "Compared to current" `useEffect` self-cancellation race in `HistoryListItem.tsx`. The dep array included `comparedDiff`/`comparedLoading`/`comparedFailed` — state the effect itself sets — so calling `setComparedLoading(true)` re-ran the effect, fired the cleanup (`cancelled = true`), and the resolving fetch's `.finally` skipped `setComparedLoading(false)` — spinner stuck forever in production (synchronous test mocks masked it). Removed the self-set state from deps + biome-ignore + cleanup defensive reset; new `HistoryPanel.compare-diff-async.test.tsx` regression guard with async setTimeout-backed mock. (2) **P0** — `ToggleGroupItem` lacked the `[@media(pointer:coarse)]:min-h-11` touch-target floor (~22 px tall, well below the 44 px AGENTS.md minimum). Added the variant. (3) **P1** — `aria-haspopup="menu"` on the FormattingToolbar overflow trigger should be `"dialog"` to match the underlying Radix Popover semantics (Popover content has `role="dialog"`, children are plain `<Button>` elements, NOT `role="menuitem"` inside `role="menu"`). Updated trigger + matching test assertion. **6 MAINT entries filed (217-222)** for deferred items: diff direction inversion in "Compared to current" view, multi-device correctness in `compute_block_vs_current_diff_inner`, keyboard nav focus management double-fire bug, missing Lock icon for non-restorable rows, nested-popover integration tests, redundant ScrollArea around FormattingToolbar. Suite: 9579/9579 vitest (was 9577 — +2 from the new regression-guard file's 1 test + 1 a11y audit, both orchestrator-added) + 3556/3556 nextest + `prek run --all-files` clean (after orchestrator added the missing `axe(container)` audit to the new regression-guard test file per `axe-presence` hook).)
