...
> **Last updated:** 2026-05-05 (Session 678 — PEND-18 Phases 0 + 1 + 2 ALL LANDED in three commits. Phase 0 (`81f86302`): validates `SpaceId` newtype + `SpaceScope` tagged enum work cleanly with specta + sqlx; throwaway IPC probe + reviewer canonical-form fix. Phase 1 (`0b89c8d2`): probe removed, 6 PartialEq impls added mirroring `ActiveBlockId`, module header production-shape. Phase 2 (uncommitted in working tree, this commit): mass migration of ~16 production `_inner` functions across `commands/{agenda,blocks/{crud,queries},queries,pages,tags,history}.rs` + `mcp/tools_ro.rs` + `mcp/tools_rw.rs::handle_create_page` + `gcal_push/connector.rs` from `space_id: Option<String>` to `scope: &SpaceScope`. Tauri wrappers translate IPC-side `Option<String>` → `SpaceScope` at entry; SQL bind sites use `scope.as_filter_param()` (returns `Option<&str>`, byte-identical to pre-migration). 29 files modified, +1505 / -423 LOC, +6 parity tests asserting `&SpaceScope::Global` ≡ pre-migration `None` for the most-used `_inner` per domain. Pagination/backlink/fts/tag_query helpers stay on `Option<&str>` per plan (commands/* `_inner` adapts via `as_filter_param()`). Phase 3 (atomic IPC boundary + frontend `tauri.ts` wrappers — `bindings.ts` regen, ~2-3h) next. Suite: 3586/3586 nextest after Phase 2 (was 3580 — +6 from parity tests), `prek run --all-files` clean.)
