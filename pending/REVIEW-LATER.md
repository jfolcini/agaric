...
> **Last updated:** 2026-05-05 (Session 678 — PEND-18 Phase 0 spike LANDED. Validates `SpaceId` newtype + `SpaceScope` tagged enum work cleanly with specta (TS discriminated union `{ kind: "global" } | { kind: "active"; space_id: SpaceId }`) and sqlx (`#[sqlx(transparent)]` round-trips through `query_as!` / `query_scalar!` column-cast position with `as "x: SpaceId"`). New `src-tauri/src/space.rs` (320 LOC: types + 14 inline tests covering from_string/from_trusted/serde/sqlx/as_filter_param) + 3-LOC delta to `lib.rs` (`pub mod space;` + probe registration). Throwaway `pend18_spike_probe` Tauri command registered in `agaric_commands!` for the IPC round-trip validation — Phase 1 deletes it. Reviewer flagged 2 P1s; fixed P1#1 (`from_string` switched from `s.to_ascii_uppercase()` to `parsed.to_string()` to match `BlockId::from_string` precedent), deferred P1#2 (the 6 `PartialEq<&str>/<str>/<String>` and reverse impls from the `ActiveBlockId` template) to Phase 1 since the spike's role is the specta+sqlx validation gate. PEND-18 plan file remains open with a Session-678 status note. Suite: 3572/3572 nextest, 14 new `space::tests` all green, `prek run --all-files` clean.)
