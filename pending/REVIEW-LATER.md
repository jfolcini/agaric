...
> **Last updated:** 2026-05-05 (Session 678 — PEND-18 Phase 0 + Phase 1 both LANDED. Phase 0: validates `SpaceId` newtype + `SpaceScope` tagged enum work cleanly with specta (TS discriminated union `{ kind: "global" } | { kind: "active"; space_id: SpaceId }`) and sqlx (`#[sqlx(transparent)]` round-trips through `query_as!` / `query_scalar!` column-cast position with `as "x: SpaceId"`); throwaway `pend18_spike_probe` Tauri command + reviewer-applied `parsed.to_string()` canonical-form fix to match `BlockId::from_string` precedent. Phase 1: removes the probe + its `KNOWN_UNWRAPPED` allowlist entry, adds the 6 `PartialEq<&str>/<str>/<String>` and reverse impls (mirroring `ActiveBlockId` byte-for-byte) + 3 new tests, and rewrites the `space.rs` module header to production-shape. Type module is now permanent. `SpaceId`/`SpaceScope` correctly disappear from `bindings.ts` (no command references them yet) — they reappear in Phase 3. Phase 2 (callsite migration: `space_id: Option<String>` → `scope: SpaceScope` across ~30 production sites in domain order agenda → blocks → pages → tags → history → queries → pagination → backlink → FTS → MCP, ~4-6h) is next. Two-commit session: spike commit `81f86302` + Phase 1 polish commit. Suite: 3572/3572 nextest after spike, 17 `space::tests` all green after Phase 1, `prek run --all-files` clean both times.)
