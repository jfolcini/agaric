...
19 open items in the summary table; 25 detail entries (FE-* sub-tables don't appear in the summary).
...
| MAINT-207 | MAINT | Frontend hygiene bundle (5 low-impact items): (a) `Input` and `Textarea` duplicate identical focus-visible + aria-invalid class strings — already partly fixed (`SHARED_INPUT_CLASSES`, Sessions 711+). (b) `MonthlyView` accepts `onNavigateToPage` and `onAddBlock` only to ignore them via `_`-prefixed renames — drop both from the API + caller. (c) `limit: 50` literal repeated in 10+ production sites (SearchPanel, LinkedReferences, TrashView, DonePanel, HistoryPanel, TagFilterPanel, PageBrowser, HistoryView, useDuePanelData) — define `PAGINATION_LIMIT` in `src/lib/constants.ts`. (d) `SearchPanel.tsx:99-130` carries ~22 useState slices — collapse filter/popover state into a `useReducer` or two extracted hooks. (e) `SearchInput.tsx:69-72` synthesizes a partial `ChangeEvent` via `as unknown as` for the clear-button — expose an explicit `onClear` callback instead, or construct a fuller event. | S-M | — |
...
- **Locations:**
  ...
  - **(c)** `limit: 50` literal repeated in 10+ production sites — `src/components/SearchPanel.tsx:151`, `LinkedReferences.tsx:89`, `TrashView.tsx:67`, `DonePanel.tsx:66,108`, `HistoryPanel.tsx:51`, `TagFilterPanel.tsx:113`, `PageBrowser.tsx:72`, `HistoryView.tsx:82`, `src/hooks/useDuePanelData.ts:204,255`. (ConflictList.tsx was deleted by PEND-09 Phase 5.)
...
- **Status:** Partial — **(a), (b), (c) resolved 2026-05-06.** `SHARED_INPUT_CLASSES` exported from `input.tsx` and consumed by `textarea.tsx`; `MonthlyView` props cleaned + callers/tests updated; `PAGINATION_LIMIT = 50` lives in `src/lib/constants.ts` and is imported by all 11 production sites. **(d), (e) remain open.**
