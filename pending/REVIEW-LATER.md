...
> **Last updated:** 2026-05-05 (Session 674 — Tier-2 features batch closing PEND-11 + PEND-17 Part A + PEND-33 Layer A (3 parallel frontend subagents): Space indicator redesign (Alt A) — new `SpaceTopStripe` (3 px accent stripe, top of webview, fixed) + deleted `SpaceStatusChip` + i18n cleanup; `SpaceAccentBadge` kept for collapsed-rail mode (PEND-11, plan deleted); diff-nav fix in `DiffDisplay` — active hunk visually highlighted with `ring-2 ring-ring/60` + `data-hunk-active` attribute on every span of the active hunk, `scrollIntoView` skipped when target is fully visible in scrollable ancestor, nav hidden for single-hunk diffs, counter moved to the left of prev/next buttons (PEND-17 Part A); new `SelectionBubbleMenu` component renders 5 mark toggles + External Link via TipTap's `<BubbleMenu>` (from `@tiptap/react/menus`), visibility predicate `({ state }) => !state.selection.empty`, dependency `@tiptap/extension-bubble-menu@^3.22.4` added matching the existing TipTap stack pin; always-visible `FormattingToolbar` reduced from 23 → 17 buttons (PEND-33 Layer A). **PEND-11 fully closed — plan file deleted. PEND-17 + PEND-33 plans remain open** with status notes pinning what's done; Part B / Layer B respectively need user approval before proceeding. +1 frontend test (SpaceTopStripe a11y audit + visibility) + 4 new DiffDisplay tests + ~20 mark/link tests moved to new `SelectionBubbleMenu.test.tsx` + 39 total new tests in the bubble-menu file. +0 backend tests. Net frontend LOC: ~−170 (PEND-11 deletion) + ~+225 (PEND-17 + PEND-33 net). 11 plan files left in `pending/`. 9698+ vitest + 3553/3553 nextest + `prek run --all-files` clean (after biome auto-fix on 3 format issues + 1 a11y mock fix where biome couldn't statically verify a `role` prop is "toolbar" — switched mock to hardcode `role="toolbar"` since real `BubbleMenu` always does).)
