...
> **Last updated:** 2026-05-05 (Session 677 — closed PEND-28a H1 (Option 2: accept duplication, add parity test). **PEND-28a fully closed — plan file deleted.** Pending folder: 9 → 8 plan files. New `src-tauri/src/pagination/block_row_columns.rs` (273 LOC) with two `#[cfg(test)] pub(crate) const`s (`BLOCK_ROW_CANONICAL_SELECT` string + `BLOCK_ROW_CANONICAL_FIELDS` field-list) and 2 parity tests: Test A asserts the SELECT string ↔ field list agreement (catches drift between the two consts); Test B asserts all 18 production `query_as!(BlockRow, ...)` sites use the canonical (catches drift between any site and the const). Drift detection verified: temporary edits to a production site OR the canonical produce informative error messages with file paths + actual-vs-expected diffs. Audit corrected the plan's site list — the plan listed `backlink/query.rs`, `tag_query/query.rs`, `snapshot/create.rs` as canonical sites (they're not — runtime `query_as::<_, BlockRow>` form OR different row type) and missed `commands/journal.rs`, `pagination/agenda.rs`, `pagination/tags.rs`, `pagination/undated.rs` (4 real canonical sites). Net: 18 sites still total, 11 files (plan said 10). Reviewer found a `clippy::unnecessary_map_or` polish (`map_or` → `is_none_or`) in the new test file; fixed inline. **3 new MAINT entries filed (223-225)**: runtime-`query_as` BlockRow sites left uncovered, Test B failure UX missing line numbers, pre-existing `gcal_push/models.rs:684` clippy warning surfaced during review. Suite: 3558/3558 nextest (was 3556 — +2 from the parity tests) + `prek run --all-files` clean.)
