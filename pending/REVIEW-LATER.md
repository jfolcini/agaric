...
| MAINT-172 | MAINT | Space-filter SQL fragment inlined across 16 production sites in 12 files (`pagination/{hierarchy.rs ×2, agenda.rs ×2, links.rs, undated.rs, tags.rs, properties.rs, trash.rs, history.rs}` + `backlink/{grouped.rs ×2, query.rs}` + `commands/{agenda.rs ×3, blocks/queries.rs}`). Real maintenance hotspot — typo at any one site causes silent space leakage. **PEND-12 (build.rs `OUT_DIR` codegen) was REJECTED session 679**: the Phase 0 spike conclusively proved sqlx 0.8.6 `query!()`/`query_as!()` parse their first arg as a `LitStr` (`syn::LitStr`, see `sqlx-macros-core-0.8.6/src/query/input.rs:55,61`), and `include_str!(concat!(env!("OUT_DIR"), …))` is a macro-invocation token tree that's rejected before any expansion happens. `query_file!` doesn't help either (same `LitStr` constraint + path-resolution rejects absolute `OUT_DIR` paths). Upstream tracking issue: [sqlx#3388](https://github.com/launchbadge/sqlx/issues/3388) (open, no PR linked, no ETA). **Pivot:** instead of source-level DRY, ship a drift-detection parity test mirroring PEND-28a H1 Option 2's pattern (session 677, `src-tauri/src/pagination/block_row_columns.rs`): one `#[cfg(test)] pub(crate) const SPACE_FILTER_CANONICAL: &str = "..."` + a Rust parity test that scans all 16 sites via `include_str!` of each source file, regex-matches every space-filter occurrence, normalises whitespace, and asserts byte-equality with the canonical const. Catches drift across the 16 copies without requiring source consolidation. Cost: M (mirror PEND-28a H1's pattern; ~250 LOC test module + per-site verification of which copies are real production queries vs comments / fixtures). Revisit when sqlx#3388 lands. | M | — |
...
| MAINT-217 | MAINT | PEND-17 Part B "Compared to current" diff direction inverts user intent. Backend `compute_block_vs_current_diff` returns `compute_word_diff(historical, current)` — direction historical → current. So `Insert` spans (rendered green via `bg-status-done`) = "added since historical" = **would be REMOVED on restore**; `Delete` spans (red strikethrough) = "removed since historical" = **would be RESTORED on restore**. The user's intent in this panel is "what changes if I click Restore?" — green should mean "this comes back", red should mean "this disappears", but today the colours map to the inverse. Plan flagged this risk; shipped without mitigation. Cheapest fix: one-line legend under the toggle ("Green = added since this version (will be removed on restore); Red = removed since this version (will be restored)"). Cleaner fix: flip the diff direction in `comparedToCurrent` mode so colours match user intent. | trivial-S | — |
...
| MAINT-223 | MAINT | PEND-28a H1 follow-up — runtime `sqlx::query_as::<_, BlockRow>(...)` sites are not parity-checked. The parity test in `src-tauri/src/pagination/block_row_columns.rs` covers the 18 `query_as!(BlockRow, ...)` macro sites only. Three runtime sites embed the canonical 13-column list as a `&str` but slip past the regex (`backlink/query.rs:327, 340`, `tag_query/query.rs:77`). Drift would be caught at runtime by sqlx's row-mapping, not silently corrupt data, but a static check would be cleaner. Two options: (a) extend the regex to also match runtime forms; (b) extract a shared `pub(crate) const BLOCK_ROW_RUNTIME_SELECT: &str` and reference it from those 3 sites. Option (b) is cleaner but requires touching the runtime sites. | S | — |
| MAINT-224 | MAINT | PEND-28a H1 follow-up — Test B failure message lacks line numbers when multiple drifted sites share a file. Currently shows `commands/blocks/crud.rs` repeatedly without locating each offender. Compute byte→line offsets via `src[..match.start()].matches('\n').count() + 1` to print exact lines. Minor UX-of-failure improvement; not worth fixing unless we hit a real "rename across all sites" scenario where multiple per-file drifts need triage. | trivial | — |
| MAINT-225 | MAINT | Pre-existing `clippy::cast_possible_wrap` warning at `src-tauri/src/gcal_push/models.rs:684` (visible only under `cargo clippy --tests -- -D warnings`, NOT under prek's `cargo clippy -- -D warnings`). Surfaced during PEND-28a H1 Option 2 review when the new test file was checked. Pre-existing — not introduced by recent work. Worth a 1-line fix (`as i64 as` cast, or `i64::try_from(...).unwrap()`) for clean clippy. | trivial | — |
| MAINT-227 | MAINT | `prek run --all-files` does NOT verify `.sqlx/` cache consistency. Surfaced session 679 when `cargo sqlx prepare -- --tests` for the PEND-15 Phase 0 audit binary discovered 5 silently-drifted cache entries from PEND-18 work in session 678 (production queries from `commands/history.rs:847`, `space.rs:337/352`, etc. that compiled via online sqlx but were never written to the cache because earlier `cargo sqlx prepare` runs didn't pass `--tests`). Cache absorption was folded into the Phase 0 commit, but the underlying gap (commits with stale `.sqlx/` caches still pass prek) is structural. Fix: add `cargo sqlx prepare --check -- --tests` as a prek hook (separate from `cargo sqlx prepare` if that's already wired). It's read-only and fast (~12s after a warm build). Cost: trivial-S — one entry in `prek.toml`. Risk: low — `--check` is non-mutating. Impact: medium — closes a real silent-drift surface that already bit this codebase once. | trivial-S | — |
