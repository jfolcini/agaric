...
> **Last updated:** 2026-05-04 (Session 672 — closed PEND-28a H2 + M4 (2 sub-items): `apply_op_tx` 387-line dispatcher split into 12 per-variant `apply_*_tx` private helpers — pure cut-and-paste extraction with zero behavior change (PEND-28a H2); `enqueue_background_tasks` 174-line imperative match flattened to a per-op-type table via new `invalidations_for_op(record, hint) -> Result<Vec<MaterializeTask>, AppError>` helper, with 20 new tests pinning the per-op-type matrix (PEND-28a M4). PEND-28a now down to 2 items (M2 ready, H1 deferred). **H1 attempted and reverted** — `sqlx::query_as!` is a proc-macro that does NOT expand `concat!(macro!(), "…")` arguments before parsing, so the planned macro extraction across 18 sites doesn't compile; the plan's "analogous to `descendants_cte_purge!()`" guidance applies only to the dynamic `sqlx::query()` API, not the compile-time `query_as!` family used at the affected sites. Plan body updated with a migration-strategy note for re-opening H1 properly. M2 (`set_property_in_tx` validation extraction) was bundled into the same subagent as H1 but deferred when H1's compile failures forced a revert; ready to ship in a future batch. +20 backend tests. 3538/3538 nextest + `prek run --all-files` clean (after `cargo fmt` auto-fix).)
