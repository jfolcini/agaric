...
> **Last updated:** 2026-05-04 (Session 671 — closed PEND-23 M6 + L2 + L9 + PEND-28a M1 + M3 in one 4-subagent batch (5 sub-items): focus-ring `@utility focus-ring-visible` extraction across 31 source files + 16 test files (PEND-23 M6, includes L12 fold-in); `MenuPopoverContent` wrapper for canonical `w-64 max-w-[calc(100vw-1.5rem)]` migrated across 6 menu popovers + 1 missing-clamp fix in `GraphFilterBar` (PEND-23 L9); URL validation extracted to `src/lib/url-validation.ts` with 40 tests (PEND-23 L2); `spawn_orphan_drafts_sweeper` boot-one-shot + hourly tick wired in `lib.rs::run` mirroring `materializer::retry_queue::spawn_sweeper` (PEND-28a M1); `pub(crate) struct SyncSessionContext<'a>` lifted 7 session-wide fields out of `try_sync_with_peer`, dropped from 9 args to 3 (PEND-28a M3). **PEND-23 fully closed — plan file deleted.** PEND-28a down to 4 items (H1, H2, M2, M4). +1 backend test + 4 frontend tests + 40 URL-validation tests + 30 flipped focus-ring assertions. 9527+/9527+ vitest + 3518/3518 nextest + `prek run --all-files` clean (after biome auto-fix on 2 format issues + 1 unsafe-fix on a pre-existing `findIndex`→`indexOf` nit in `BlockPropertyEditor.tsx`).)
