...
> **Last updated:** 2026-05-04 (Session 670 — closed PEND-21 (both items) + PEND-24 H1 + H3 + M1 in one 3-subagent batch (5 sub-items): icon-only breadcrumb hover-chip + 3 px focus ring (PEND-21 item 1) and dropped touch-only `<X /> Exit zoom` button (PEND-21 item 2); foreground `ApplyOp` retry-exhausted now persists via new `RetryKind::ApplyOp{device_id, seq}` variant + `__APPLY_OP__` sentinel + sweep-time `try_reenqueue_apply_op` + new `fg_apply_dropped_persisted` counter (PEND-24 H1); `record_failure_with_retry` helper retries once + new `retry_queue_persist_errors` counter (PEND-24 M1); GCal connector pause-on-revoked-refresh-token via `gcal_settings.reauth_required` flag + typed `gcal:reauth_required` Tauri event with `ReauthRequiredPayload { account_email }` + migration `0046_gcal_reauth_flag.sql` (PEND-24 H3 backend; frontend banner deferred → MAINT-216). **PEND-21 fully closed — plan file deleted. PEND-24 fully closed — plan file deleted.** +5 backend tests + 12 flipped frontend assertions. 9489+/9489+ vitest + 3517/3517 nextest + `prek run --all-files` clean.)
