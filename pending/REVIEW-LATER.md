...
| TEST-E2E-1 | TEST | **Local-only** e2e flakiness cluster (34 tests) in `agenda-advanced.spec.ts` (26 — DuePanel never reaches visible state), `features-coverage.spec.ts` (3 — journal view modes), `keyboard-shortcuts.spec.ts` (3 — Alt+Left/Right/T), `settings.spec.ts` (1 — Keyboard tab shortcut list), `templates.spec.ts` (1 — `/template` slash command). **Key finding (session 737):** 0.1.21 SHIPPED on 2026-05-13 with a green `validate / validate` job in CI on `headSha=9fcd820`. Same SHA fails locally today. So the failure mode is environmental, not a code regression — likely chromium binary version (locally `chromium-headless-shell-1217` v147.0.7727.15) interacting with jsdom-less Vite-dev viewport / journal-page hydration timing. **Diagnostic trail:** failure mode is "due-panel locator not found after 5s"; page snapshot shows `<main>` contains ONLY `JournalControls` (Today / Agenda / Open-calendar buttons), no `DaySection`. `JournalPage:183` gates the `DailyView` on `!loading`. `loading` is owned by `useCalendarPageDates`, which fires `listJournalPagesInRange` with the current space id. Mock handler `tauri-mock/handlers.ts:123` filters by `space.value_ref === spaceId`; seed sets `value_ref='SPACE_PERSONAL'`; `list_spaces` mock returns `[{id:'SPACE_PERSONAL',…}]`. So the chain should resolve. Suspect: the `useEffect([currentSpaceId])` re-runs as `currentSpaceId` flips `null → 'SPACE_PERSONAL'` AFTER first paint, and a stale `loading=true` from the first fetch (with spaceId='') sticks long enough that `findByTestId` times out. Worth instrumenting the `loading` state machine + adding a `data-testid="journal-content-ready"` marker for e2e to wait on. Needs a dedicated agenda/journal-page investigation session. | M-L | — |
