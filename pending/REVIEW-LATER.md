...
> **Last updated:** 2026-05-05 (Session 673 — cherry-pick batch closing PEND-16 + PEND-28a M2 (2 sub-items, 2 parallel subagents): daily-journal double-block race fixed via Option 1a — `BlockTree.autoCreateFirstBlock` is now the single owner for fresh-page first-block creation, the no-template `else { createBlock(...) }` fallback in `useJournalBlockCreation.handleAddBlock` deleted, the `setCreatedPages/onPageCreated/useResolveStore.set` triple moved to AFTER the template/no-template branch resolves so `BlockTree` sees the seeded blocks (or empty page) before mounting (PEND-16); `validate_property_value(payload, declaration) -> Result<(), AppError>` private helper extracted in `commands/blocks/crud.rs` (~115 LOC, 5-step validation pipeline), `set_property_in_tx` body shrunk from 243 → 153 lines (132 code-only), 15 new unit tests cover the type × payload-shape matrix + reserved-key + select-options BUG-20 (PEND-28a M2). **PEND-16 fully closed — plan file deleted.** PEND-28a down to 1 item (H1 deferred). +1 frontend integration test (`JournalPage.integration.test.tsx`) using the real `BlockTree` — proves the race is dead by asserting exactly 1 `create_block` call. +1 a11y audit (orchestrator-added per `axe-presence` hook). +15 backend tests. 9528+ vitest + 3553/3553 nextest + `prek run --all-files` clean (after `cargo fmt` auto-fix on M2's helper).)
