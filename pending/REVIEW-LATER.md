...
> **Last updated:** 2026-05-05 (Session 675 — Tier-2 features follow-up batch closing PEND-17 Part B + PEND-33 Layer B (2 parallel subagents, mixed Rust + frontend): block history restore-with-preview redesign — `BlockHistoryItem` row click expands a panel with primary "Restore this version" button + read-only `RichContentRenderer` of the historical content + `ToggleGroup` defaulting to "Compared to current"; new backend Tauri command `compute_block_vs_current_diff` returns `Vec<DiffSpan>` for the cumulative diff; in-panel restore drops the ConfirmDialog (toast-with-Undo is the safety net); keyboard browse via ↓/↑/Enter/Esc; new `ToggleGroup` UI primitive added; frontend integration via 5 new i18n keys + new mock handler + new `tauri.ts` wrapper (PEND-17 Part B); new `useToolbarOverflow` hook + `MoreHorizontal` overflow popover render the FormattingToolbar's 17 always-visible buttons via priority-driven width-fit (Undo/Redo priority 100 always visible, Discard priority 30 drops first); ResizeObserver-driven re-measurement, off-screen sentinel pre-rendering for per-button widths, +24 px reserve for popover-trigger variable inner content; nested popovers handled (heading/code-block popovers anchor at the overflow row when they overflow); separators only render when both adjacent groups have visible buttons (PEND-33 Layer B). **PEND-17 fully closed — plan file deleted. PEND-33 fully closed — plan file deleted.** +3 backend tests (compute_block_vs_current_diff matrix incl. soft-deleted edge case → graceful NotFound) + 23 frontend tests (15 hook + 8 toolbar overflow integration) + 17 redesigned BlockHistoryItem tests + 31 redesigned HistoryPanel tests (full rewrite with command-name-routed IPC mock pattern). 9 plan files left in `pending/`. 9577 vitest + 3556/3556 nextest + `prek run --all-files` clean (after cargo fmt + biome --write x2 + 4 manual a11y fixes: `<section>` for the preview region, biome-ignore consolidation for the row-as-button pattern, `useExhaustiveDependencies` ignore on the items-as-trigger pattern in the hook, removal of stale unused-suppression comments).)
