...
| CI-R15 | PERF | Vitest pool A/B benchmark — `forks` (default) vs `threads`. Happy-dom suites typically run 1.3-1.8× faster on threads but threads can leak module state. ADOPT if measured speedup >30% (run one CI experiment on each branch); document either way. | S (one CI run + decision) | Opt-in benchmark cycle |
