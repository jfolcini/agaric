...
| PEND-48 | MAINT | Verify reproducible builds end-to-end — build the release matrix twice from a clean state, diff hashes, identify and eliminate sources of non-determinism (timestamps, sort order, embedded build env). Flips `build_reproducible` to Met on bestpractices.dev Silver tier. Greenfield work: `release.yml` has zero `SOURCE_DATE_EPOCH` / reproducibility hooks today, so the cost is the multi-week investigation across desktop + Android build steps, not a quick verification pass. | L (multi-week) | — |
