...
+ **Component decomposition backlog.** ~13 files exceed AGENTS.md's 500-LOC threshold; worst: ~~`BlockTree.tsx` (790)~~ — closed Session 727: dropped 802 → 697 LOC after extracting `useBlockFlush` (parser-driven flush + checkbox/todo persistence), `useBlockAutoCreateFirstBlock` (H-9 first-block-on-empty-page effect), and `useBlockTreeContextBags` (memoised `BlockActions` + `BlockResolvers` published via context — MAINT-118) into `src/components/block-tree/use-block-*.ts`; the audit's "recursion + DnD overlay" framing was already addressed across earlier sessions (`getDragDescendants`, `useBlockCollapse`, `useBlockZoom`, `useBlockDnD`, `BlockDndOverlay`). Perf wins from Sessions 717-720 (`useRovingEditor` / `useViewportObserver` memoed returns, `MeasuringStrategy.WhileDragging`, `SortableBlockWrapper`'s `React.memo`) untouched; public exports (`BlockTree`, `processCheckboxSyntax`, `guessMimeType`) unchanged. ~~`RichContentRenderer.tsx` (659)~~ — RichContentRenderer split Session 716 (per-mark renderers extracted to `src/components/RichContentRenderer/marks/*.tsx`; main file now 53 LOC dispatcher; public API unchanged). ~~`sidebar.tsx` (1078, 22 sub-components) is fat-but-cohesive — splitting `useSidebarState` and edge-swipe/keyboard handlers out is the cheapest win.~~ — sidebar split Session 717: dropped 1078 → 865 LOC after extracting `useSidebarState`, `useSidebarKeyboard`, `useSidebarEdgeSwipe`, and `useSidebarRailDrag` into `src/components/ui/sidebar/*.ts`; public API (22 sub-components + `useSidebar` context) unchanged; hooks each have a sibling test file. (ConflictList.tsx no longer applicable — deleted by PEND-09 Phase 5.)
...
+ ~~**List rendering primitive zoo.** 7 distinct row primitives across list views; `TemplatesView.tsx:186-239` and `DuePanel.tsx:343-389` (projected entries) write raw `<li>` and bypass the design system entirely.~~ — closed Session 726 with scope **(b)**, narrow. Inventory pass found the "7 primitives" framing was misleading: only 5 are real, intentional specializations — `BlockListItem` (HTML5 drag + virtualizer + typed metadata primitive props for `React.memo`), `HistoryListItem` (`<div role="row">` inside an ARIA **grid** with `gridcell` children + `aria-selected` + shift-select), `BlockHistoryItem` (expand-to-reveal restore panel + diff-mode `ToggleGroup`), `PeerListItem` (a `<div>` card that isn't in a list at all), `AlertListRow` (CVA `destructive`/`pending` bordered alert row) — plus the shared `ui/list-item.tsx` base (already in use by `TagList`, `PropertyDefinitionsList`) and the two raw-`<li>` violations cited above. Forcing a unified `ListRow` over the 5 specializations would either invalidate HTML (PeerListItem isn't a list item), break ARIA-grid semantics (HistoryListItem), or layer a wrapper around `BlockListItem` for no benefit. The real audit miss was just the 2 raw-`<li>` blocks, which now use `ListItem` from `@/components/ui/list-item`: `TemplatesView.tsx` template row, `DuePanel.tsx` projected-entry row (chrome overrides via `cn()` + `tailwind-merge`). Both now inherit `[@media(pointer:coarse)]:min-h-11`, `focus-ring-visible`, and `transition-colors` from the base. Tests: full TemplatesView (26) + DuePanel + DuePanelFilters (67) green; full suite + `npx tsc -b --noEmit` clean (the unrelated single `PageBrowser.test.tsx:232` Load-More failure pre-exists on `main`).
