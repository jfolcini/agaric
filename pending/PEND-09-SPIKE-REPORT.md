...
> **Archive note (Phase-2 day-8, Gate 8).** This report originally lived
> at `src-tauri/crates/loro-spike/SPIKE-REPORT.md`. The spike crate was
> archived in Phase-2 day-8 — the directory is gone, but the final
> source is preserved at the annotated git tag `pend-09/spike-archive`
> (commit `3e8a1267`). All in-doc references to
> `src-tauri/crates/loro-spike/...` paths point at archived files; recover
> via `git show pend-09/spike-archive:src-tauri/crates/loro-spike/<path>`.
> Sibling notebook moved to `pending/PEND-09-SPIKE-NOTES.md`.
...
running notebook in `pending/PEND-09-SPIKE-NOTES.md` and to the seven daily
commits.
...
| 3 | **100K-op replay < 10 min wall-clock + < 2 GB peak heap** | **PASS** (large margin) | **1.677 s wall-clock** (358× under the 10-minute floor), **144.32 MiB peak RSS** (14× under the 2 GiB floor), final snapshot 6.40 MiB for ~25K alive blocks (≈260 B / alive block); RSS grew almost-linearly from 4.4 MiB across the full apply loop with no perf cliff at any 10K checkpoint. _(**Phase-2 day-5:** the day-4 bench's Linux-only `/proc/self/statm` parse swapped for the cross-platform `memory-stats` crate (`1.2`, MIT OR Apache-2.0); RSS is now sampled identically on Linux (`/proc/self/statm`), macOS (Mach `task_info`), and Windows (`GetProcessMemoryInfo`). Bench output gains a machine-readable `---- machine-readable summary ----` stanza with `peak_rss_kib=NNN`, `wall_clock_seconds=N.NNN`, `snapshot_bytes=NNN`, `alive_blocks=NNN` so a CI script can grep without parsing the prose. Rerun on Linux confirmed the verdict unchanged (`peak_rss_kib=147788` ≡ 144.32 MiB, 1.82 s wall-clock); macOS / Windows numbers are now obtainable in one command ahead of the Phase-2 cutover toggle. New smoke test `replay_bench::tests::current_rss_kib_returns_positive_reading` asserts the API returns a positive reading on whichever platform CI dispatches it on (45 → 46 spike tests). Default agaric build byte-identical to commit `62d3790c` — `memory-stats` lives only in `crates/loro-spike/Cargo.toml` and does not enter the agaric build.)_ | 4 | `e72c4d01` |
...
1. ✅ **Add the dep.** `src-tauri/Cargo.toml` gains `loro = "1.12"` (caret-1, matching the spike). Also `xxhash-rust = "0.8"` for the peer-id hash (replaces the spike's `std::hash::DefaultHasher`, see notebook Q13). _(Day-1: `loro = "1.12"` landed as `optional = true`, gated behind the new `loro-shadow` cargo feature so the default build is byte-identical to today's.  `xxhash-rust` swap deferred — Phase-1 day-1 inherits the spike's `DefaultHasher`-based `peer_id_from_device_id` unchanged; the swap becomes load-bearing once peer ids start persisting across process lifetimes, i.e. Phase 2.  **Day-7:** `xxhash-rust = "0.8"` landed as `optional = true`, added to the `loro-shadow` feature's enable list so default builds remain unaffected.  `peer_id_from_device_id` (in `src-tauri/src/loro/engine.rs`) now hashes via `xxh3_64` — deterministic (seed 0), spec-pinned, stable across Rust toolchain upgrades.  Two regression tests added (3769 → 3771 with `--features loro-shadow`): `peer_id_from_device_id_is_deterministic` (same input → same output, distinct inputs → distinct outputs) and `peer_id_from_device_id_is_stable_against_known_values` (locks `peer_id_from_device_id("01ARZ3NDEKTSV4RRFFQ69G5FAV") == 0x11e7_9683_b730_ff1f`; updating this expected value is a coordinated-migration event since every device's already-stored op history would be re-credited to a different peer).  The function's docstring now carries a `# Stability contract — DO NOT CHANGE WITHOUT A COORDINATED MIGRATION` block.  Spike crate's `peer_id_from_device_id` left untouched per Day-7 plan — the spike is throwaway post-Phase-0 and we don't compare hash outputs across crates.)_
2. ✅ **Sample the real op-log histogram.** Run the user's actual `notes.db` op_log through a one-off counting script; confirm the 30/50/10/5/5 spike proxy is in the right ballpark, OR re-run the day-4 replay bench with the real distribution to confirm the kill-criterion margin is robust to it (notebook Q12). _(**Phase-2 day-4:** closes plan Q9 + Phase-2 Gate 4 (`pending/PEND-09-PHASE-2-CUTOVER-PLAN.md` §2 Gate 4 + §3 day 4).  `src-tauri/src/bin/op_log_histogram.rs` (~700 LOC including tests) lands a stand-alone read-only diagnostic the maintainer runs against their real `notes.db`.  Opens the DB with `SqliteConnectOptions::filename(...).read_only(true)` so SQLite refuses any write at the engine level — safe to run on a live DB.  Runs `SELECT op_type, COUNT(*) FROM op_log GROUP BY op_type ORDER BY count DESC, op_type ASC` (deterministic tie-break on op_type so the output is reproducible) and pretty-prints a table with thousands-separated counts and one-decimal-place percentages.  Compares each row against the spike's `30/50/10/5/5` proxy and emits informational flags for op types whose share is `>2x` over or `<0.5x` under their proxy share (strict inequalities so the spec wording reads naturally), plus `[not-in-proxy]` for op types the spike never measured (≥1% share threshold so the report doesn't drown in tiny-share rows) and `[absent-in-actual]` for proxy-listed op types missing from the histogram entirely.  No color codes — output may be piped to a file; ASCII rule lines (`-`) instead of U+2500 box drawing for grep-friendliness.  The proxy block is suppressed entirely when the op_log is empty (the comparison would be meaningless).  Errors flow through `AppError`; no `unwrap()` / `expect()` outside the `#[cfg(test)]` module.  Registered as a `[[bin]]` in `src-tauri/Cargo.toml` alongside `agaric-mcp` and `audit_cross_space_refs`.  15 new tests (3 required by the spec — `histogram_groups_by_op_type`, `histogram_handles_empty_op_log`, `histogram_compares_against_proxy` — plus 12 covering CLI parsing, deterministic SQL ordering, the empty-histogram short-circuit, the not-in-proxy / absent-in-actual edge cases, and the formatter's no-color / ASCII-rule contract); test count 3734 → 3749 default and 3782 → 3797 with `--features loro-shadow`.  Default-build behaviour byte-identical to commit `9d780586` apart from the new bin's compilation — the binary depends only on `agaric_lib::error::AppError` + `agaric_lib::db::init_pool` (test-only) and is feature-gate-free.  Maintainer runs `op_log_histogram /path/to/notes.db` once and pastes the output into the Phase-2 day-4 commit message — the wide kill-criterion-#3 margin (358× wall-clock, 14× heap) means this gate is about _knowing_, not gating.)_
...
4. ✅ **Define the `loro_batch` payload envelope.** Includes `loro_version: u32` (mitigates kill-criterion-4's YELLOW flag), `payload_version: u32`, `original_op_type: Option<OpType>` (dispatcher routing — plan Q7), and the Loro-exported batch bytes. _(Day-3: `src-tauri/src/loro/envelope.rs` lands the `LoroBatch` schema with `loro_version: u8` (= 1), `payload_version: u8` (= 1), `original_op_type: String`, and `payload: serde_json::Value` (kept as `Value` so future op-type variants can ride through unmodified).  Field widths narrowed `u32 → u8` because we don't anticipate >= 256 versions before this code is rewritten or removed.  `Loro-exported batch bytes` deferred — Phase 1 dual-writes the typed payload; the shift to opaque Loro-exported bytes happens at Phase 2 cutover, at which point this schema gains a `loro_bytes: Vec<u8>` column.  A `TryFrom<&OpRecord>` impl wraps a typed op_log row as an envelope; 4 unit tests (roundtrip, OpRecord conversion, malformed-JSON rejection, typed `OpPayload` round-trip-through-Value).  No call sites yet — day-4 / day-5 territory.  **Phase-2 day-3:** schema bump landed (closes Phase-2 Gate 3).  `LoroBatch` gained a fifth field `loro_bytes: Vec<u8>` carrying Loro's `LoroDoc::export(ExportMode::*)` output for the op(s) in the row; `CURRENT_LORO_VERSION` bumped 1 → 2.  Field is `#[serde(default, skip_serializing_if = "Vec::is_empty")]` so the bump is wire-format BACKWARD-compatible: a v1 row's JSON (no `loro_bytes` key) deserialises into the v2 struct with `loro_bytes: Vec::new()`, and a v2 envelope built with empty bytes serialises identically to v1 — proven by the new `loro_batch_v1_decodes_into_v2_struct_with_empty_bytes` test, which builds a `V1Envelope` mirror struct (field set + order + types matching the Phase-1 day-3 source per `git show 16d369d2:.../envelope.rs`), serialises it through `serde_json` to get a real v1 byte sequence, and feeds that through the v2 deserialiser.  New `LoroBatch::with_loro_bytes(...)` constructor for the cutover writer (Phase-2 day 9); existing `LoroBatch::new(...)` and `TryFrom<&OpRecord>` paths produce empty `loro_bytes` (the Phase 1 shape).  Plain `Vec<u8>` rather than `serde_bytes::ByteBuf` — JSON has no native byte-string type, so `serde_bytes` would still encode as a JSON array; switching is a non-breaking encoder swap if a non-JSON wire format ever ships.  Four new unit tests (`loro_batch_v2_roundtrip_with_bytes`, `loro_batch_v1_decodes_into_v2_struct_with_empty_bytes`, `loro_batch_v2_with_empty_bytes_serialises_as_v1`, `loro_batch_with_loro_bytes_constructor_populates_field`); test count 3778 → 3782 with `--features loro-shadow`.  Default-build behaviour byte-identical (3734 unchanged) — the envelope module is gated `#[cfg(feature = "loro-shadow")]`.  No call sites yet; the cutover toggle (day 9) starts populating `loro_bytes`.)_
...
6. ✅ **Design the parity-logging sink.** Either a new SQLite table (`merge_parity_log`) or an in-memory ring buffer flushed periodically; columns / fields = (op_id, diffy_result_hash, loro_result_hash, bucket_a/b/c/d classification, free-text divergence). Decide retention policy (e.g. 30 days rolling). _(Day-1: in-memory ring buffer landed in `src/loro/parity.rs` as `ShadowParitySampler` (default capacity 1024).  `ParityEvent { op_id, diffy_result, loro_result, match, timestamp }` matches the day-1 spec.  **Day-4:** persistent SQLite sink landed.  Migration `0051_pend_09_merge_parity_log.sql` creates the `merge_parity_log` STRICT table with columns `(id, op_id, space_id, op_type, diffy_result, loro_result, matched, bucket, created_at)` + three indexes (`created_at` for retention purge + tail-N debug, `op_id` for per-op history, `matched` for the divergence-rate query and the day-6 classifier scan).  `ParityEvent` extended with `space_id: String` + `op_type: String` so the SQLite row can be reconstructed without re-parsing the payload; `merge::shadow_apply` threads them through; timestamp unit clarified to ms-since-Unix-epoch (was already ms in `shadow_apply`, the day-1 doc said "seconds").  `src/loro/parity_sink.rs` adds `flush_to_sqlite(pool, sampler) -> usize` (drains the ring + multi-row chunked INSERT under `FLUSH_CHUNK_ROWS = 100`, well below the 999-bind limit) and `purge_old(pool, before_ts_ms) -> usize` (single DELETE covered by `idx_merge_parity_log_created_at`).  Default retention is 30 days (`DEFAULT_RETENTION_DAYS = 30`); helper `default_retention_cutoff_ms()` computes the cutoff.  Periodic flush wiring is **not** yet in place — that's day-5 (flush) and day-7+ (purge); day-4's deliverable is the data layer + the standalone tested functions.  6 new unit tests + 1 drain test on the sampler (3750 → 3757 with `--features loro-shadow`).  Bucket A/B/C/D classification + free-text divergence remain on day-6.  **Day-5:** periodic flush + purge wiring landed.  `src/loro/flush_task.rs` adds `pub async fn run_periodic_flush(pool, &'static ShadowState, flush_interval, purge_interval)` — runs forever, ticks on a `tokio::time::interval` (so it cancels cleanly when the runtime drops the future), counts flush ticks to fire `purge_old` every `ticks_per_purge = ceil(purge / flush)` ticks, wraps both calls in `if let Err(e) = …` + `tracing::warn!` and continues so a transient SQL error never aborts the task.  `pub const FLUSH_INTERVAL_SECS = 30` and `PURGE_INTERVAL_SECS = 3600` are the production defaults; tests pass shorter values.  `crate::run`'s `setup` closure now spawns the task via `tauri::async_runtime::spawn` immediately after `crate::loro::shared::init()`, gated on `feature = "loro-shadow"`; the `pools.write` clone happens before `app.manage(WritePool(...))` so the task's `SqlitePool` handle outlives setup.  Three new tests (3757 → 3760 with `--features loro-shadow`): `run_periodic_flush_flushes_events_after_first_tick` (push 3 events, sleep 100 ms over a 20 ms cadence, assert all 3 rows in SQLite + ring drained), `run_periodic_flush_continues_after_flush_error` (close the pool mid-task, push another event, assert the JoinHandle is still `!is_finished()` after several flush ticks against the closed pool), `run_periodic_flush_invokes_purge_at_purge_interval` (seed two 31-day-old rows + one fresh row directly via SQL, run with `flush=10ms`/`purge=20ms`, assert only the fresh row survives).  **Day-6:** bucket A/B/C/D classifier landed.  `src/loro/classifier.rs` adds `pub enum Bucket { A, B, C, D }` with `as_str()`, `pub fn classify(diffy_result, loro_result, matched) -> Bucket` (pure), and `pub async fn classify_unbucketed(pool) -> Result<ClassifyStats, AppError>` (selects rows with `bucket IS NULL` in 4 000-row chunks, classifies them in-process, batches per-bucket UPDATE statements at 200 ids per chunk to stay under SQLite's 999-variable limit, one transaction per SELECT chunk).  Rules: `matched=true` → A; `loro_result.starts_with("error:")` → D; `diffy_result.starts_with("conflict:") && !loro_result.starts_with("conflict:")` → B (forward-compat — production `diffy_summary_for` does not currently emit `conflict:`); else C.  Conservative-by-default per kill-criterion #2 ("when in doubt use C, not D").  Wired into `flush_task::run_periodic_flush` after the `flush_to_sqlite` call on the same flush tick — chosen over a separate cadence because the `WHERE bucket IS NULL` filter makes the empty-pending path a single index probe and chaining the cadences avoids a window where flushed-but-unclassified rows accumulate.  Idempotent: re-running on already-classified rows is a no-op; mutates only the `bucket` column.  9 new tests (3760 → 3769 with `--features loro-shadow`): 6 pure-function (A/B/C/D rules, matched-true short-circuit, error-short-circuits-conflict edge case) + 3 DB-backed (`classify_unbucketed_updates_only_null_rows`, `classify_unbucketed_returns_correct_stats`, `classify_unbucketed_recognises_conflict_resolved_by_loro_as_b`).  Free-text divergence column still deferred — it would require schema change + payload reconstruction; the bucket alone is the kill-criterion proxy.  **Phase-2 day-7:** read-only diagnostic landed — closes Phase-2 Gate 7 (`pending/PEND-09-PHASE-2-CUTOVER-PLAN.md` §2 Gate 7 + §3 day 7) and the §7 item 2 follow-up that flagged the 7-day shadow-mode soak as "tedious to observe without a diagnostic command".  `src-tauri/src/bin/parity_report.rs` (~1100 LOC including tests) gives the maintainer a `parity_report /path/to/notes.db` command that prints the bucket A/B/C/D histogram (with NULL bucket rendered as `unclassified` so unprocessed rows are visible at a glance), the per-op-type breakdown, the date range, and a tail of the most recent 20 bucket-C / bucket-D events (the headline data the user actually cares about during soak).  Same `SqliteConnectOptions::filename(...).read_only(true)` opening as `op_log_histogram` — the binary cannot write to the user's data even if SQLite tried to issue an incidental WAL checkpoint.  Detects a missing `merge_parity_log` table via a `sqlite_master` lookup and exits non-zero with the message "merge_parity_log table not found; was this DB built with shadow mode?" rather than auto-running migrations on the maintainer's data — important because the bin compiles in default builds (no `loro-shadow` gate on the binary itself), so a maintainer who has rolled back to a build without shadow mode can still inspect rows accumulated when shadow mode was on, but the bin must not silently create the table on a DB that never had it.  Every query has explicit `ORDER BY` with a tiebreaking key (bucket query: `ORDER BY bucket` so NULL sorts first; op-type query: `count DESC, op_type ASC`; recent-divergent query: `created_at DESC, id DESC` so two events sharing the same millisecond timestamp still produce stable output) — the output is byte-stable across runs against the same DB.  Verdict line keys off the bucket-D count: `D = 0` → `Verdict: OK — kill-criterion #2 holds (D = 0 in this DB).`; `D > 0` → `Verdict: ALERT — D-bucket sightings present (N rows); investigate before cutover.`.  No color codes; ASCII rule lines (`-`) instead of U+2500 box drawing for grep / pipe friendliness.  Summaries truncated to 36 chars with a single-character ellipsis (`…`) and column padding done by char-count rather than byte-length so the ellipsis row aligns visually.  Hand-rolled days-from-civil timestamp formatter (`fmt_timestamp_ms`) keeps the binary's compile graph from pulling in a new date crate; round-trip tested against epoch + a 2026-05-09 anchor.  Errors flow through `AppError::InvalidOperation` (negative-COUNT guard) or sqlx's `From<sqlx::Error>`; no `unwrap()` / `expect()` outside the `#[cfg(test)]` module.  Registered as a `[[bin]]` in `src-tauri/Cargo.toml` alongside `op_log_histogram`.  19 new tests (5 spec-required — `parity_report_groups_by_bucket`, `parity_report_handles_empty_log`, `parity_report_lists_recent_divergent` (LIMIT 20 cap with 25-row seed + DESC ordering verified), `parity_report_verdict_on_zero_d`, `parity_report_verdict_on_d_present` — plus 14 covering CLI parsing, the `sqlite_master` probe (both present + dropped-table cases), the date formatter on epoch / known dates / non-midnight times, the `pad_right_chars` char-count vs byte-length contract, the truncation behaviour, the empty-DB output shape, and tiebreak determinism for the op-type and divergent tables); test count 3749 → 3768 default and 3807 → 3826 with `--features loro-shadow`.  Default-build behaviour byte-identical to commit `88323c8e` apart from the new bin's compilation — the binary depends only on `agaric_lib::error::AppError` + `agaric_lib::db::init_pool` (test-only) + sqlx and is feature-gate-free.  The `sqlx::migrate!` macro embeds the `0051_pend_09_merge_parity_log.sql` migration regardless of feature flag (same caveat as Phase-2 days 3 + 6); no logic changes in default builds.  Sample output verified end-to-end against a synthetic 68-row fixture (40 A / 8 B / 15 C / 2 D / 3 unclassified): the bucket table shows percentages with one-decimal-place precision, the op-type breakdown sorts by count DESC then op_type ASC, the divergent tail shows D rows first (newest timestamps) then C rows in `created_at DESC` order, the verdict reads `ALERT — D-bucket sightings present (2 rows)`, and the bin exits 0.)_
...
9. ✅ **Document the LWW resolution rule** (concurrent-reparent + concurrent-property-write) in the Phase-1 design doc per plan Q5 + notebook Q12. Decision: Lamport-keyed LWW; loser's intent dropped from materialised state; LoroTree intent-preservation is NOT exposed in Phase 1. _(Day-9: design doc landed at [`pending/PEND-09-lww-resolution-rule.md`](PEND-09-lww-resolution-rule.md). Cites the production rule's actual timebase — wallclock UTC RFC 3339 via `now_rfc3339()` in `src-tauri/src/lib.rs:288-290`, **not** Lamport — corrects the day-1 wording above. Tiebreak: `created_at` → `device_id` lex → `seq` (`src-tauri/src/merge/resolve.rs:223-297` for property; `src-tauri/src/sync_protocol/operations.rs:699-822` inline for move). Loser's intent is dropped from materialised state silently (no conflict copy, no toast); recoverable only by walking `op_log`. Loro-side equivalence cited from §3 Q5: LoroMap+scalar uses LWW with key-write timestamps, bucket A in normal cases. LoroTree intent-preservation deferred to a hypothetical Phase-4 polish per §5 risks row.)_
10. ✅ **Confirm the FE edit-coordinate space.** TipTap / ProseMirror typically emit UTF-16; Loro speaks USV. Phase 1 design doc documents the bridge — either coerce at the FE → backend boundary or use Loro's `splice_utf16` variants (notebook Q10). _(Day-9: design doc landed at [`pending/PEND-09-fe-edit-coordinate-space.md`](PEND-09-fe-edit-coordinate-space.md). Phase-1 verdict: the bridge does **not** need to exist yet. In shadow mode the FE never reads Loro state — TipTap continues to emit UTF-16 ProseMirror transactions, the FE flushes UTF-8 markdown via `editBlock` IPC at blur, and the BE materialises into `blocks.content` exactly as today. The FE coord-space contract is "FE owns the focused block in-progress; BE owns persistence + DAG truth; flush boundary is `unmount()` at blur." Focus-preserving sync reload at `src/stores/page-blocks.ts:246-256`. The USV ↔ UTF-16 coercion only becomes load-bearing at Phase 2.5+ if we wire FE editors to Loro `subscribe()` callbacks for live remote-edit merging — flagged as future work, **not** Phase 1 scope.)_
