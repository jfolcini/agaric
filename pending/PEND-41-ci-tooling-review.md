...
**Batch 2 leftovers landed in session 759 (2026-05-16)** — R5 (`cargo audit` promoted warn → block; `deny.toml [advisories].ignore` becomes the explicit waiver path), R13 (central `unsafe-allowlist.txt` + `scripts/check-unsafe-allowlist.sh` + prek hook — every new `#![allow(unsafe_code)]` carve-out is now audit-gated against the single-entry allowlist for `sync_daemon/android_multicast.rs`), R17 (vitest shard 1 runs with `--coverage`; `json-summary` reporter added so a follow-up step renders the aggregated totals as a markdown table on the job step-summary page — no Codecov dependency), R19 (`depcheck` retired in favour of knip alone; the matching `.nsprc` lodash waivers `1115806` + `1115810` removed as the transitive dep disappears). Same session also fixed three pre-existing CI failures the maintainer flagged in the latest run: zizmor `impostor-commit` × 4 on `actions/attest-build-provenance@<v3-tag-object>` (the pin was the *tag object* SHA, not a commit — bumped to the commit SHA + `# v3.0.0` comment), zizmor `ref-version-mismatch` × 2 on `taiki-e/install-action` (`# v2` → `# v2.78.1` to match the SHA), and the lychee prek hook (CI cache had `v0.24.2` while prek pinned `v0.23.0`, leaving the install script unable to locate the binary — bumped the rev to align).

**Batch 3a (security workflows) landed in session 760 (2026-05-16)** — R9 (`anchore/sbom-action` generates SPDX-JSON + CycloneDX-JSON SBOMs per platform — desktop matrix and Android — files included in the existing `actions/attest-build-provenance` subject-path and uploaded to the draft release alongside the bundles via `gh release upload --clobber`), R10 (new `.github/workflows/scorecard.yml` — OpenSSF Scorecard weekly cron + push-to-main + manual; uploads SARIF to Code Scanning and publishes the public score), R23 (GPL-3.0-or-later compatibility comment block added at the top of `src-tauri/deny.toml [licenses]` + a matching cross-reference comment on the npm `license-checker` hook in `prek.toml`), R29 (build-time secret scan: `strings` + `grep -E` over Linux bundles for canonical secret signatures — AWS keys, GitHub tokens, OpenAI keys, PEM private keys — runs as a new step in `release.yml` before the attest step; non-Linux platforms deferred to a TruffleHog follow-up), R30 (new `.github/workflows/branch-protection-assert.yml` — daily cron + manual; asserts the current `rules/branches/main` ruleset still contains the expected rule types: `deletion`, `non_fast_forward`, `pull_request`, `required_signatures`; reports diff via `jq` set-difference on drift).

**Batch 3b (docs + release-flow polish) landed in session 761 (2026-05-16)** — R4 (`CONTRIBUTING.md` gained a Bootstrap section with `cargo install --locked prek` + `prek install` and a soft-landing for prek-less contributors), R12 (the maintainer-TODO blockquote in `docs/architecture/ci-and-tooling.md` §13 was replaced with the verified ruleset shape: 4 rule types, 1 admin bypass actor, `required_approving_review_count: 1`, signed-commit requirement — all sourced from `gh api repos/jfolcini/agaric/rules/branches/main`), R18 (new `finalize-release-notes` job in `release.yml` that appends a copy-pasteable SLSA-verify recipe — per-asset `gh attestation verify` examples plus an `slsa-verifier` air-gapped alternative — to the draft release notes and mirrors it to the workflow step-summary), R22 (new "Updater signing-key rotation" section in `SECURITY.md` with annual cadence, generate / replace-secret / rotate-pubkey / publish-and-notify procedure, the implicit revocation model, and a deferred-Sigstore note; cross-referenced from `ci-and-tooling.md` §9 and `docs/BUILD.md`). Plus one fix-forward for session 759's R17 — vitest shard 1's coverage check was failing because the global thresholds in `vitest.config.ts` (80% lines / 75% branches) apply to the partial shard; CI command now overrides every threshold to 0 inline so partial coverage stays informational instead of a gate.

**Batch 4 (process + perf tweaks) landed in session 762 (2026-05-16)** — R2 (`.github/dependabot.yml` extended with npm + cargo ecosystems plus 8 + 9 grouping rules respectively; the coupled-dep stacks from AGENTS.md — Tauri / React / TipTap / Radix / Vitest / Playwright / Sqlx / Tokio / Loro / Rustls — each get their own group; specta + tauri-specta are explicitly ignored to preserve the exact `=2.0.0-rc.24` pin), R14 (Playwright workers bumped from `1` → `process.env.CI ? 2 : '50%'`; the audit of all 28 e2e specs identified one additional state-sharing file — `history-revert.spec.ts` — tagged `test.describe.configure({ mode: 'serial' })`; the other three pre-existing serial tags stay), R24 (`src-tauri/Cargo.toml [profile.release]` gained `panic = "abort"` after verifying zero `catch_unwind` call-sites in `src-tauri/src/`; `lto = "fat"` rejected as not worth the build-time cost). **R15 explicitly DEFERRED** — switching vitest from `forks` → `threads` should be measurement-driven per the "measure-don't-imagine" rule; no current data shows the >30% speedup the spec requires for adoption. Logged as a deferred follow-up.

**Remaining:** long-horizon (R3, R11). Both already documented as deferred in the decisions doc and PEND-41 itself; no further action this cycle.
