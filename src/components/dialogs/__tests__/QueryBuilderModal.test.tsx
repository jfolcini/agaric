...
describe('QueryBuilderModal', () => {
  ...
  describe('property-key validation', () => {
    ...
    // and never crashes.
    it('falls back safely (empty datalist, no warning) when listPropertyDefs rejects', async () => {
      mockedInvoke.mockImplementation(async (cmd: string) => {
        if (cmd === 'list_property_defs') return Promise.reject(new Error('boom'))
        return null
      })
      ...
      // Datalist stays empty (the rejected load left knownPropertyKeys = []).
      await waitFor(() => {
        const datalist = screen.getByTestId('qb-prop-key-list')
        expect(datalist.querySelectorAll('option').length).toBe(0)
      })

      // Typing any key produces NO validation warning (fail-safe: zero known
      // keys suppresses the unknown-key warning) and never crashes the modal.
      const keyInput = screen.getByLabelText(/property key/i) as HTMLInputElement
      await user.type(keyInput, 'anything')
      expect(screen.queryByText(/not yet defined/i)).not.toBeInTheDocument()
      expect(keyInput).toHaveAttribute('aria-invalid', 'false')
      expect(keyInput).toHaveValue('anything')
    })
