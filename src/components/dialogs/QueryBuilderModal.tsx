...
export function QueryBuilderModal({
  ...
  useEffect(() => {
    ...
    if (structured) {
      // oxlint-disable-next-line react/set-state-in-effect -- a changed `initialExpression` rehydrates the builder form; a plain derive would discard the user's edits; the guarded adjust is out of scope; see #4407
      setMode('advanced')
