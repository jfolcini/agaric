...
export interface CollapsibleGroupListProps<G extends GroupItem> {
  ...
  /**
   * Phase 1 — optional `role` for the inner `<ul>`. Defaults to
   ...
   * `aria-activedescendant`-driven listbox per the a11y model.
   */
  listRole?: 'listbox' | undefined
  /**
   * Phase 1 — `aria-activedescendant` for the inner `<ul>`,
   * resolved per-group by the caller. Threaded only when `listRole` is
   * `"listbox"`.
   */
  listAriaActiveDescendant?: (group: G) => string | undefined
  /**
   * Phase 1 — `tabIndex` for the inner `<ul>` (default `undefined`).
  ...
  /**
   * Phase 1 — `onKeyDown` for the inner `<ul>`. Search wires the
  ...
  /** Phase 1 — `data-testid` for the inner `<ul>`, for test selectors. */
  listDataTestId?: (group: G) => string | undefined
  /**
   * Phase 1 — override the per-group count label rendered next
  ...
  /**
   * Full override for an expanded group's list body.
  ...
  return (
    <>
      {groups.map((group) => {
        ...
        // Bug 1: references group headers render LEAF + breadcrumb,
        ...
        return (
          <div key={group.page_id} className={groupClassName}>
            ...
            {isExpanded &&
              (renderGroupList ? (
                // The override owns the entire `<ul>`
