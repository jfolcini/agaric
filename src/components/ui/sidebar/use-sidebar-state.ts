...
export const SIDEBAR_WIDTH_STORAGE_KEY = PREFS.sidebarWidth.key
...
export function useSidebarState({
  ...
  // Sidebar width state — persisted in localStorage. `getPref` returns the
  // `-1` sentinel (see PREFS.sidebarWidth) when the key is absent/corrupt,
  // which always fails the `>= SIDEBAR_WIDTH_MIN` check below and falls
  // through to `SIDEBAR_WIDTH_DEFAULT`, same as the pre-registry code.
  const [sidebarWidth, _setSidebarWidth] = React.useState(() => {
    const stored = getPref(PREFS.sidebarWidth)
  ...
  const setSidebarWidth = React.useCallback((width: number) => {
    ...
    setPref(PREFS.sidebarWidth, clamped)
  }, [])
