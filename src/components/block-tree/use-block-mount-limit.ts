...
export function useBlockMountLimit(
  ...
  useEffect(() => {
    ...
    // oxlint-disable-next-line react/set-state-in-effect -- resets the mount ceiling when `pageKey` changes; `expandMountLimit`/`revealIndex` also write it, so it is not derivable; see #4407
