/**
 * useGraphSimulation — d3-force simulation lifecycle (MAINT-57).
 *
 * Encapsulates the worker ↔ main-thread-fallback split for GraphView's
 * force simulation. Responsibilities:
 *
 *   1. SVG / d3 selection setup (nodes, edges, interactions, zoom).
 *   2. WebWorker path with runtime-failure recovery (BUG-45): if the
 *      worker dispatches an `error` / `messageerror` event we terminate
 *      it, flip a `workerFailed` state flag, and rerun the effect to
 *      take the main-thread path. Users never see a frozen graph.
 *   3. Main-thread force simulation fallback (no-Worker environments
 *      and post-failure recovery).
 *   4. Drag, zoom, keyboard-zoom-shortcut lifecycle.
 *
 * Returns imperative zoom handlers for the component's zoom buttons.
 */
...
type LinkSel = Selection<SVGLineElement, GraphEdge, SVGGElement, unknown>
type NodeSel = Selection<SVGGElement, GraphNode, SVGGElement, unknown>
...
function setupZoomBehavior(svg: SVGSVGElement, g: GSel): ZoomBehavior<SVGSVGElement, unknown> {
  const svgSel = select(svg)
...
function createApplyPositions(link: LinkSel, node: NodeSel): () => void {
  return () => {
    link
...
export function useGraphSimulation({
  ...
  useEffect(() => {
    ...
    const rendered = renderGraphElements(svg, nodes, edges, navigateToPage)
    const applyPositions = createApplyPositions(rendered.link, rendered.node)

    const zoomBehavior = setupZoomBehavior(svg, rendered.g)
