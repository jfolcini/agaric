...
describe('handleBlockKeyDown', () => {
  ...
  // MAINT-185: refs-bag pattern — handleKeyDown identity must be stable
  // across re-renders so the keydown listener is not detached/reattached
  // when parent components recreate callback closures.
  describe('refs-bag stability (MAINT-185)', () => {
    ...
    it('keydown listener is not re-attached when callback identities change across re-renders', () => {
      const { editor, cleanup } = makeRealEditor()
      const container = editor.view.dom.parentElement as HTMLElement
      const addSpy = vi.spyOn(container, 'addEventListener')
      const removeSpy = vi.spyOn(container, 'removeEventListener')

      const { rerender, unmount } = renderHook(
        ({ callbacks }: { callbacks: BlockKeyboardCallbacks }) =>
          useBlockKeyboard(editor, callbacks),
        { initialProps: { callbacks: makeCallbacks() } },
      )

      const attachAfterMount = addSpy.mock.calls.length
      const detachAfterMount = removeSpy.mock.calls.length
      expect(attachAfterMount).toBeGreaterThanOrEqual(1)

      // Rerender with a brand-new callbacks object — every callback has a
      // fresh identity. Under the OLD pattern this would re-create
      // handleKeyDown and force the useEffect to re-attach the listener.
      rerender({ callbacks: makeCallbacks() })
      rerender({ callbacks: makeCallbacks() })

      expect(addSpy.mock.calls.length).toBe(attachAfterMount)
      expect(removeSpy.mock.calls.length).toBe(detachAfterMount)

      unmount()
      cleanup()
    })

    it('latest callback closure is invoked via the refs bag after a re-render', () => {
      const { editor, cleanup } = makeRealEditor()

      const escV1 = vi.fn()
      const escV2 = vi.fn()
      const cbsV1: BlockKeyboardCallbacks = { ...makeCallbacks(), onEscapeCancel: escV1 }
      const cbsV2: BlockKeyboardCallbacks = { ...makeCallbacks(), onEscapeCancel: escV2 }

      const { rerender, unmount } = renderHook(
        ({ callbacks }: { callbacks: BlockKeyboardCallbacks }) =>
          useBlockKeyboard(editor, callbacks),
        { initialProps: { callbacks: cbsV1 } },
      )

      // Swap to the v2 callbacks; the listener identity should not change
      // but the refs bag should now point at escV2.
      rerender({ callbacks: cbsV2 })
      ...
      expect(escV1).not.toHaveBeenCalled()
      expect(escV2).toHaveBeenCalledOnce()
