...
**`Storage.prototype` spies intercept, but only the first one in a file.** This was listed as a second reason to opt into jsdom, on the grounds that happy-dom bypasses the prototype entirely. It does not — `Object.getPrototypeOf(localStorage) === Storage.prototype` — but happy-dom's `Storage` is a `Proxy` whose `get` trap runs `ClassMethodBinder.bind`, which reads the descriptor off `Storage.prototype` **at that moment**, copies it onto the instance as an own bound property, and caches the name so it never rebinds. So interception is first-come: a spy installed before anything touches `localStorage.setItem` works (`useTheme.test.ts`, `useDeepLinkRouter.test.ts`), and every spy installed after that point is invisible, because the instance already owns a bound copy of the real method. jsdom forwards to the prototype on every access, which is why the same file passes there.

Two consequences. A file that spies `Storage.prototype` more than once, or in more than one test, needs jsdom — `preferences.test.ts`, `keyboard-config.test.ts`, `useLocalStoragePreference.test.tsx` and `use-block-collapse.test.ts` are pinned for exactly that, and the last of those is order-dependent: it passes in isolation and fails once a preceding test has frozen the binding. And a `not.toHaveBeenCalled()` behind a later spy goes silently vacuous under happy-dom rather than failing, so it proves nothing.
