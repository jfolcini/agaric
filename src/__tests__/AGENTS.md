...
**`Storage.prototype` spies intercept, but only the first one in a file.** happy-dom's `Storage` is a `Proxy` whose `get` trap runs `ClassMethodBinder.bind`: it reads the descriptor off `Storage.prototype` at that moment, copies it onto the instance as an own bound property, and caches the name so it never rebinds. `Object.getPrototypeOf(localStorage) === Storage.prototype` — the prototype is not bypassed — but interception is first-come. A spy installed before anything in the file has touched `localStorage.setItem` works; every spy installed after that point records nothing, because the instance already owns a bound copy of the real method. `vi.restoreAllMocks()` does not undo it. jsdom forwards to the prototype on every access, which is why the same file passes there. A file that needs more than one such spy, or one in a test that runs after an earlier `localStorage` write, belongs on jsdom.

`vi.spyOn(window.localStorage, …)` is unaffected and needs no pin: an instance spy goes through the proxy's `defineProperty` trap, which calls `preventBinding` and defines straight onto the instance, so the freeze cannot reach it. Prefer it when you are adding a new test rather than pinning a file.

**A dead spy fails silently, so prove it live before trusting it.** A throwing `mockImplementation` that never reaches the code under test leaves the swallow-and-warn path unexercised while the test stays green, and a `not.toHaveBeenCalled()` behind a dead spy passes for the wrong reason. Assert `toHaveBeenCalled()` alongside, or temporarily invert it to `toHaveBeenCalledTimes(99)` and read the observed count — and **run the whole file, never `-t` filtered**. A filtered run skips the earlier test whose write froze the binding, so the spy is live under the filter and dead in CI. That is not hypothetical: it is how the first version of this paragraph came to claim the opposite.
