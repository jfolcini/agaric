...
**Spy the storage instance, not `Storage.prototype`, and restore it yourself.** `vi.spyOn(window.localStorage, 'setItem')` intercepts under happy-dom wherever it sits in a file; a `Storage.prototype` spy only intercepts if nothing has touched that method yet, because happy-dom's `Storage` proxy copies each prototype method onto the instance on first access and never rebinds. Pair the spy with an explicit `spy.mockRestore()` in a `finally` — `vi.restoreAllMocks()` does not reach an instance spy, so a throwing `localStorage` otherwise leaks into every later test in the file. The one exception is a file pinned to jsdom: there the mirror image holds, since a `defineProperty` on jsdom's `Storage` proxy stores a *storage item* rather than replacing the method, leaving the instance spy inert. Such a file uses the prototype spy and says in its header that the pin is load-bearing for it.

**A dead spy fails silently, so prove it live before trusting it.** A throwing `mockImplementation` that never reaches the code under test leaves the swallow-and-warn path unexercised while the test stays green, and a `not.toHaveBeenCalled()` behind a dead spy passes for the wrong reason. Assert `toHaveBeenCalled()` alongside, or temporarily invert it to `toHaveBeenCalledTimes(99)` and read the observed count — before the `mockRestore()`, which clears the history — and **run the whole file, never `-t` filtered**. A filtered run skips the earlier test whose write froze the binding, so the spy is live under the filter and dead in CI.
...
- **Global module mocks:** `@tauri-apps/api/core` (`invoke`, `Channel`, `addPluginListener`), `@tauri-apps/plugin-clipboard-manager`, `sonner`, `@/components/ui/select`, and a `TooltipProvider`-wrapping `@/components/ui/tooltip`. The `invoke` mock is the lowest layer, so it intercepts both the `@/lib/ipc-helpers` floor and direct `@/lib/bindings` calls.
