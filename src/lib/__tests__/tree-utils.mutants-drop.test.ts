...
describe('computeDropIndex mutants (#3142)', () => {
  // Line 534 [ConditionalExpression]: `if (overIdxInWithout < 0)`. Forcing
  ...
  // Line 540 [EqualityOperator]: `overIdxInItems > activeIndex ? +1 : same`.
  ...
  // (survives the mutation run; tree-utils.ts:540:16). The extra case needs
  ...
  // branch above (line 534). Line 540 is never reached with the two indices
  ...
  // Line 546 [ConditionalExpression forced-true] and [LogicalOperator, `??`→`&&`]:
  ...
  // the mutation run; tree-utils.ts:546:23). When `parentId` IS null, forcing
  ...
  // returned directly.
  // NOTE: the comment this replaces claimed the test above also kills line 555
  ...
  // Line 546 [UnaryOperator]: `find(...)?.depth ?? -1` → `?? +1`. The literal
  ...
  // Line 555 [ConditionalExpression]: `item.depth === childDepth` forced to
  ...
  // Line 546 [OptionalChaining]: removing `?.` from `find(...)?.depth` throws
  ...
  // Line 553 [EqualityOperator]: `i < insertAt`. Picks a case where
 ...
 * two equivalent mutants documented above (tree-utils.ts:540, :546) are the
 ...
 * "0 differing inputs" claims (540:16, 546:23) over 352,000 generated inputs,
 * with three known-Killed mutants (534:7, 553:19, 555:50) firing as controls,
 * and the 540 canary confirmed reachable (52,000 hits) but never at the line
