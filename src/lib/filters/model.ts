 ...
 * `{ type: 'After', date: GRAPH_HAS_DATE_SENTINEL }` — i.e. the literal
 * `'__graph-has-date__'`, NOT `''` — is reserved for "has date = true". We keep
 * this mapping internal and total so the round-trip is exact.
 *
 * WHY a sentinel rather than a dedicated variant: the canonical `date` kind
 * deliberately embeds the bindings `DatePredicate` verbatim (see its doc
 * above), and that type is GENERATED from the Rust wire enum by Tauri Specta
 * (`src/lib/bindings.ts` — "Do not edit this file manually"). It has an
 * `IsNull` variant but no `IsNotNull`/`HasDate` counterpart, so expressing the
 * true side properly would mean changing the Rust enum and regenerating the
 * bindings — a cross-boundary change for a client-only concern. The canonical
 * form is never evaluated as a query: `GraphFilterBar` serialises it to
 * localStorage and reads it back (`graphFiltersToCanonical` /
 * `canonicalToGraphFilters` are its only callers), so the sentinel only has to
 * be a value no real date can collide with. Its *comparison* semantics are
 * irrelevant — `predicateToGraphHasDate` below inspects only `.type`.
 */
 ...
 *
 * DELIBERATELY module-private, and its VALUE is deliberately duplicated as a
 * bare literal in the tests rather than imported from here:
 * `filters/__tests__/model.test.ts` ("hasDueDate true encodes the After/sentinel
 * marker") and `filters/__tests__/model.mutants.test.ts` ("value=true maps to
 * the After sentinel predicate") both hardcode `'__graph-has-date__'` in their
 * assertions. That duplication is what kills the StringLiteral mutant on this
 * line — if the tests imported the constant, test and code would mutate
 * together and the mutant would become equivalent, dropping this module off its
 * 100% mutation score. So: changing the STRING here intentionally breaks four
 * assertions across those two files; update them in the same change. Renaming
