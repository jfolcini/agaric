...
- **RichContentRenderer** (`src/components/RichContentRenderer.tsx`): Renders markdown AST as React nodes for static block view. Handles inline tokens (tag_ref, block_link, block_ref, external_link), block-level elements (headings, code blocks, callouts, blockquotes, ordered lists, horizontal rules), syntax highlighting via lowlight, lazy-loaded Mermaid diagrams. Exported `renderRichContent()` function + `CALLOUT_CONFIG`. Extracted from StaticBlock (M-17). Used by StaticBlock.
