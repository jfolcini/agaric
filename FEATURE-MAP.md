...
### Spaces (FEAT-3 Phases 1 + 2 + 3 + 5 + 6 + 7 + 8 + 10 + 11)

Partition pages into user-defined contexts ("Personal" / "Work") with a sidebar switcher. Phases 1 + 2 + 3 ship the data model, bootstrap, `list_spaces` + `create_page_in_space` commands, `SpaceStore`, `SpaceSwitcher`, list/search scoping on `list_blocks` + `search_blocks`, link-picker scoping, atomic new-page creation, a PageHeader "Move to space" action, and per-space tabs + recent pages. Phase 5 (session 500) ships per-space daily journal pages keyed by `(date, space_id)` plus per-space `currentDate` + `mode` slices in `useJournalStore`. Phase 6 ships the manage-spaces UI (rename / accent picker / delete-only-if-empty / create-new-space) with backend defense-in-depth via `delete_block_inner` refusing to delete a non-empty space. Phase 7 (session 500) enforces cross-space link policy: `[[ULID]]` chips whose target lives in a foreign space render via the existing broken-link UX (no auto-navigation, click removes the chip). Phase 8 scopes the History view to the current space by default with an opt-in "All spaces" Switch. Phase 10 adds per-space visual identity — accent color (7 oklch tokens), status-bar chip, collapsed-rail badge, and OS window title prefix — so the active space is undetectable-at-a-glance no longer. Phase 11 wires `Ctrl+1`…`Ctrl+9` (`Cmd+1`…`Cmd+9` on macOS) to instant Nth-space switching with discoverable hint chips on every dropdown row. **The BUG-1 fix (session 499) tightens the `create_block` IPC for `block_type='page'` so pages without a `space` property cannot exist at the IPC boundary anymore — closing the daily-impact bug where journal / templates / welcome / date-picker pages used to disappear from PageBrowser.** Phase 4 still scopes remaining query commands (agenda / graph / backlinks / tags / properties), Phase 5b adds per-space journal templates (deferred), and Phase 9 covers per-space external integrations.
