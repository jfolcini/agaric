...
- **BlockGutterControls** (`src/components/BlockGutterControls.tsx`): Drag handle + history + delete gutter buttons extracted from SortableBlock. `GutterButton` helper with Tooltip→button→icon pattern. dnd-kit attribute/listener props. 23 tests (M-25).
