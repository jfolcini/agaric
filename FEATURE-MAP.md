...
### Spaces (FEAT-3 Phases 1 + 2)

Partition pages into user-defined contexts ("Personal" / "Work") with a sidebar switcher. Phases 1 + 2 ship the data model, bootstrap, `list_spaces` + `create_page_in_space` commands, `SpaceStore`, `SpaceSwitcher`, list/search scoping on `list_blocks` + `search_blocks`, link-picker scoping, atomic new-page creation, and a PageHeader "Move to space" action. Phases 3–6 scope remaining query commands (agenda / graph / backlinks / tags / properties / journal), per-space tabs + recent, keyboard shortcuts, and the space-management UI.
