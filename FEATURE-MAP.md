...
- **Favorites/starred pages** (UX-58 + FEAT-12): Star toggle on each page in PageBrowser. Starred pages always render on top under a "Starred" section header with non-starred pages below under "Other pages"; sort applies independently within each group. Single-page vaults and tree mode (titles with `/`) bypass grouping. localStorage persistence via `starred-pages.ts`. Memoized via `starredRevision` counter.
