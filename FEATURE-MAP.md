...
- **Breadcrumb** (`src/components/ui/breadcrumb.tsx`): Design-system breadcrumb primitive (UX-257). Exports `Breadcrumb`, `BreadcrumbItem`, `BreadcrumbSeparator`, `BreadcrumbHome`. `<nav role="navigation">` outer + `<div role="toolbar">` inner (deviates from `<ol>` because axe's `listitem` rule conflicts with `role="toolbar"`). Chevron separators (canonical glyph), `text-xs` + `min-h-7` baseline, `[@media(pointer:coarse)]:py-2` 44px touch hit. Per-crumb `max-w-[160px]` truncate / `max-w-[320px]` for the final `aria-current="location"` crumb. Overflow at >5 items collapses middle into a Radix `Popover` ellipsis. ArrowLeft/Right + Home/End keyboard nav at the toolbar level. `dataAttributes` opt-in field for caller-specific testid passthrough. Used by BlockZoomBar (zoom trail) and PageHeader (namespace-title `parent/child` breadcrumb).
