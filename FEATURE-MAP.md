...
- **DiffDisplay diff-nav fix** (PEND-17 Part A): the prev/next "change" buttons inside an expanded diff in the block history sheet now visibly respond. (1) Active hunk gets `ring-2 ring-ring/60 rounded-sm` + `data-hunk-active="true"` on every span belonging to the active hunk (was: only the first span had `data-hunk-start` and no styling). The data structure flipped from a single `hunkStarts` array to `{ hunkStarts, hunkOfSpan }` so every span knows its hunk membership. (2) Smart scroll: new `findScrollableAncestor` helper walks up the parent chain looking for `overflow: auto | scroll | overlay`; if the target span's `getBoundingClientRect()` is fully inside the ancestor's, `scrollIntoView` is skipped (was: `scrollIntoView({ block: 'nearest' })` was a no-op when the target was already visible, contributing to the "buttons appear inert" perception). (3) Single-hunk diffs render no nav at all (was: "1 of 1 changes" with two disabled buttons read as broken to first-time users). (4) Counter moved to the left of the buttons. 4 new tests + 2 existing tests adjusted (single-hunk fixtures replaced with 2-hunk fixtures so the nav still renders for the assertion). Part B (restore-with-preview) deferred — has 3 open questions for the user.
- **`SelectionBubbleMenu` component** (PEND-33 Layer A, `src/components/SelectionBubbleMenu.tsx`): 5 mark toggles (Bold/Italic/Code/Strike/Highlight) + External Link button hoisted into a TipTap `<BubbleMenu>` (imported from `@tiptap/react/menus`, the subpath introduced in TipTap 3.x). Visibility predicate is `({ state }) => !state.selection.empty` — the bubble menu only renders when the user has a non-empty selection. Mounted as a sibling to `<FormattingToolbar />` and `<EditorContent />` inside `EditableBlock.tsx`. New i18n key `toolbar.selectionFormatting = 'Selection formatting'` (separate from `toolbar.formatting` to disambiguate two coexisting `role="toolbar"` elements for assistive tech). Dependency `@tiptap/extension-bubble-menu@^3.22.4` added explicitly to `package.json` (was an optionalDependency of `@tiptap/react`); pinned exact `3.22.4` to match the rest of the TipTap stack. Always-visible `FormattingToolbar` reduced from 23 → 17 buttons. ~20 mark/link tests moved from `FormattingToolbar.test.tsx` (1186 → 843 lines) to a new `SelectionBubbleMenu.test.tsx` (690 LOC) + 39 total new tests in the bubble-menu file (visibility predicate, focus preservation, theming tokens, axe a11y audit, mark-toggle dispatch). Layer B (priority + overflow popover with `useToolbarOverflow` hook) deferred per the plan's "Stop here for review" gate — needs explicit user approval.
