...
Agaric is a **single-user, multi-device, local-first** application with **no cloud connectivity by default**. Sync runs over the local network between devices the same user has explicitly paired. There is no maintainer-operated server. The one opt-in exception is the **internet fallback** in Settings → Sync & Devices (#4549): off on every install, and when a user turns it on, on both devices, sync may go through a relay server run by iroh's maintainers (n0) when the local network cannot carry it. The relay forwards end-to-end-encrypted QUIC datagrams by endpoint id; it cannot read notes, but it does see both devices' endpoint ids and public addresses, connection timing and traffic volume, and the device holds a standing connection to it while sync is running. The full threat model is documented in [`AGENTS.md` § Threat Model](AGENTS.md#threat-model); the short version is:

- Notes never leave the user's devices in a form anyone else can read. Note content never leaves them at all unless the internet fallback is on, and then only encrypted end to end.
...
- **Threat-model violations in code** — anything that adds an outbound network call to a server the maintainer doesn't operate, opens a listening port the user didn't ask for, or otherwise widens the attack surface beyond "the user's own paired devices on a local network." (Two documented, bounded exceptions: the opt-in, loopback-only observability exporter — see *Observability telemetry egress* below; a way to bypass its loopback enforcement is in scope — and the opt-in sync internet fallback above; a relay connection made while that setting is off, or a path by which enabling it re-installs iroh's address-lookup services, is in scope.)
...
- **Loopback-only, enforced in code.** The endpoint host must be `localhost`, `::1`, or an address in `127.0.0.0/8`; any other host (LAN IP, public host, DNS name) is **rejected at startup** (`observability::config::validate_loopback_endpoint`) and the app falls back to file-only export with a warning. The only permitted destination is a collector on the same machine. Validating the URL is not sufficient on its own, because two request-time mechanisms can redirect the bytes elsewhere, so the export client disables both: **HTTP redirects** (`reqwest::redirect::Policy::none`), so a `3xx` from the collector cannot bounce a span batch off-host, and **the system proxy** (`.no_proxy()`, #3317) — reqwest installs a proxy matcher from `HTTP_PROXY` / `http_proxy` / `ALL_PROXY` by default and hyper-util exempts only hosts named in `NO_PROXY`, so before #3317 a machine with a proxy exported sent every span batch to that proxy while the loopback validation logged nothing adverse. (Residual: `localhost` is resolved by the OS at request time, so a tampered `hosts`/resolver mapping `localhost` to a non-loopback address could still misdirect — but that presupposes a host already compromised below the app's trust boundary; pin a literal `127.0.0.1` to avoid even that.)
...
- **PII discipline: span attributes are allowlisted (by a scoped scan, not by convention); span events are not.** Span *attributes* carry opaque ids / counts / enums / durations / booleans only — never note content, query strings, tag names, or property values — and since #3317 that is enforced by a workspace-source scan (`commands::observability::tests::span_fields_stay_on_the_pii_allowlist`) rather than by convention. (#3317 found and removed one real violation: the markdown import span recorded the user's page title.) Precisely, the scan covers every `#[instrument(... fields(...))]` key, every `...span!("name", ...)` inline field — the generic `tracing::span!` and all five `{info,debug,trace,warn,error}_span!` shorthands — and every `<receiver>.record("key", ...)` call, whatever the receiver is named (#3712 closed three gaps here: span-macro inline fields, `.record()` receivers other than a binding named exactly `span`, and the generic `span!` the five shorthands expand to). Both scans are **deny-by-default**: the `.record()` scan checks every call site and skips only the `(file, receiver)` pairs on an explicit, currently one-entry `NON_SPAN_RECORD_RECEIVERS` list, each confirmed at that call site not to be a span, so a span held under any name — `s`, `tracing_ctx`, `make_span().record(...)`, a struct field, or even the excluded receiver name in any other file (#3988 replaced a bare-name key that excused it workspace-wide) — is still scanned. It is a text scan, not a type check. Its residual limits are stated in full on the test itself and are worth repeating here, because a list of admitted weaknesses is the artefact a reviewer trusts when deciding how hard to look at a new span: **under-flags** — a real field the scan does not see — are three, all in the `.record(` marker: `.record_all(...)` (and any other spelling where `(` does not follow `record` immediately); a `.record()` whose first argument is not a literal at all (`span.record(key_var, ...)`); and the **raw and byte-string key spellings** (`span.record(r"key", v)`, `b"key"`, `br#"key"#`), which are literals but are still not read, so "non-literal key" alone understates the clause; the only **skip** is the one reviewed `(file, receiver)` pair above, and an entry that stops matching a real call site fails the test rather than lingering; and there are two **review questions, not mechanical ones** — a value that turns into user content under an already-allowlisted key, and the fact that the allowlist is keyed on the field NAME, so a shape-generic entry (`kind`, `size`) inherits its allowance at any second site that adopts the name. Neither is something a test can catch; re-reading the emitting sites is the only control over them. The scan also over-flags in the safe direction (a full macro invocation spelled inside a string literal or block comment is read as one, reddening CI at an innocent site). What it no longer depends on is what the span binding is called, which of the three macro delimiters the invocation is spelled with, whether a delimiter or comma sits inside a literal or comment, or whether rustfmt has wrapped a long `.record(` call onto the next line or a comment sits between its paren and its key (#3980, #3988). Span *events* — the `tracing` events `tracing-opentelemetry` folds into the enclosing span, including the `err` line — carry their fields verbatim and are **not** allowlisted, so an OTLP payload can contain free-text log fields and error strings. They reach no local trace file (the file exporter does not serialize events); what keeps them safe is the containment above (loopback + no redirects + no proxy) and the bug-report bundle's deny-by-default redaction, not a claim that the payload is PII-free.
...
| --- | --- | --- |
| *(none yet — this section will populate as advisories are resolved)* | — | — |
...
- **npm advisories** — [`.nsprc`](.nsprc). There are currently no npm waivers: the file is an empty object so every advisory fails closed. If a future exception is unavoidable, it must carry an `expiresOn` no more than 90 days out to force periodic re-triage.
- **Cargo advisories** — [`src-tauri/deny.toml`](src-tauri/deny.toml) `[advisories].ignore`. Every entry is confirmed against the full target-agnostic lockfile view used by `cargo audit`; most are Tauri-transitive GTK3 / unic / paste / proc-macro-error notices that cannot be removed without dropping Linux support, with a smaller Loro-transitive set blocked on its pinned upstream stack. Add an entry only with a `reason =` that names the upstream blocker. A target-pruned `cargo deny` `advisory-not-detected` warning alone does not make an entry stale; remove it only when the full-lockfile audit no longer finds it.
- **Zizmor workflow findings** — [`.github/zizmor.yml`](.github/zizmor.yml). Every release publishing lane is cache-free. In particular, the three dependency-building/generating jobs — `build-and-release`, `android-build-and-release`, and `generate-vex` — neither restore nor save Rust/npm caches. The `cache-poisoning` baseline now contains only four cache sites in `ci.yml`; the single-user, no-adversarial-peers threat model explains that interim posture. `unpinned-uses` is enforced (no baseline entries); every action is pinned to a 40-char SHA and bumped weekly by Dependabot.
