...
- **Loopback-only, enforced in code.** The endpoint host must be `localhost`, `::1`, or an address in `127.0.0.0/8`; any other host (LAN IP, public host, DNS name) is **rejected at startup** (`observability::config::validate_loopback_endpoint`) and the app falls back to file-only export with a warning. The only permitted destination is a collector on the same machine. The exporter also **disables HTTP redirects** (`reqwest::redirect::Policy::none`), so a `3xx` from the collector cannot bounce a span batch off-host — nothing can leave the machine. (Residual: `localhost` is resolved by the OS at request time, so a tampered `hosts`/resolver mapping `localhost` to a non-loopback address could still misdirect — but that presupposes a host already compromised below the app's trust boundary; pin a literal `127.0.0.1` to avoid even that.)
...
- **Zizmor workflow findings** — [`.github/zizmor.yml`](.github/zizmor.yml). The `cache-poisoning` baseline is deliberately partial: `android-build-and-release` and `generate-vex` are cache-free, so their findings are fixed. The two remaining `build-and-release` entries stay baselined and are additionally tracked in [#3573](https://github.com/jfolcini/agaric/issues/3573); cache sites in `ci.yml` also remain in the baseline. The single-user, no-adversarial-peers threat model explains the interim posture. `unpinned-uses` is enforced (no baseline entries); every action is pinned to a 40-char SHA and bumped weekly by Dependabot.
