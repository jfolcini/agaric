...
- **Pre-push:** `verify-ci-equivalent` (`scripts/verify-ci-equivalent.sh`) runs everything `.github/workflows/_validate.yml` runs, parallelized across cores (≈3-4 min wall on a warm cache). Catches CI failures before the push reaches GitHub. `SKIP_CI_VERIFY='<reason>' git push` is the escape hatch for docs-only typo fixes — the value must be a real reason (≥8 chars); a bare truthy flag (`SKIP_CI_VERIFY=1`) is hard-rejected. Use e.g. `SKIP_CI_VERIFY='docs typo, no source change' git push`. Release pre-flight bundle build is opt-in via `scripts/verify-release-build.sh` (not in pre-push — too slow for daily cadence).
