...
6. **sqlx compile-time queries** — `query!` / `query_as!` / `query_scalar!`. `.sqlx/` cache committed. Run `cargo sqlx prepare -- --tests` after SQL changes (the `-- --tests` is required so test-only queries are cached — the verifier runs `cargo sqlx prepare --check -- --tests`).
...
- **SQLx + `.sqlx/` cache.** The `sqlx` crate version and the committed `.sqlx/` query cache must match. If `sqlx` is bumped, regenerate `.sqlx/` with `cargo sqlx prepare -- --tests` in the same commit — do not land a version bump with a stale cache.
...
- **Pairing does not cryptographically bind the typed passphrase to the pinned certificate (accepted residual — #1559, #855).** `confirm_pairing` proves only that the joiner entered the right passphrase; the peer's cert is pinned later, trust-on-first-use, from whichever `agaric-*` certificate is observed first. The passphrase-confirmed identity and the pinned key are therefore not provably the same device, so on a shared LAN a racing or CN-spoofing `agaric-*` device could in principle be pinned in the legitimate device's place. This is deliberately **not** fixed reflexively: it follows from the "peers are the user's own trusted devices" model above, and the obvious binding fix — carrying the host's cert hash in the pairing QR so the responder can verify it — was rejected to keep the QR payload minimal (`{v, passphrase}` only; mDNS owns discovery/address resolution). Revisit only if the threat model tightens to distrust LAN-local devices.
...
- **Error handling:** `AppError` enum (variants include `Database`, `NotFound`, `PoolTimedOut` (kind `pool_busy`), `Conflict` (kind `conflict`), `Migration`, `Io`, `Json`, `Ulid`, `InvalidOperation`, `Channel`, `Snapshot`, `Validation`, `NonReversible`, `Cancelled` (kind `cancelled`)) serializes to `{ kind, message }` for Tauri 2 IPC. Specta-derived TS bindings. See `src-tauri/src/error.rs` for the current variant set.
...
- **Commands:** Tauri command handlers in `src-tauri/src/commands/` (split across files by domain: `blocks/`, `pages/`, `tags.rs`, `properties.rs`, `agenda.rs`, `attachments.rs`, `history.rs`, `journal.rs`, `queries.rs`, `sync_cmds.rs`, `compaction.rs`, `drafts.rs`, `link_metadata.rs`, `logging.rs`, `advanced_query.rs`, `block_cleanup.rs`, `bug_report.rs`, `mcp.rs`, `notifier.rs`, `recovery.rs`, `spaces.rs`). Each command has an `inner_*` function taking `&SqlitePool` for testability.
...
- **Pre-commit:** `prek.toml` — broad file-type-aware hook surface at pre-commit, plus pre-push hooks (`no-commit-to-branch` + the `verify-ci-equivalent` umbrella below). Coverage includes builtin file checks; secret + workflow security (gitleaks, zizmor); actionlint; oxlint + oxfmt + tsc + vitest (frontend lint/format/test); cargo fmt/clippy/nextest/deny/machete (Rust); sqruff + sqlx prepare + migrations-immutable (DB); typos + shellcheck + taplo + markdownlint + lychee (cross-cutting); repo-specific guards (no-hsl-rgb-var-wrap, no-legacy-react-apis, no-ui-store-imports, tauri-mock-parity, tauri-command-sanitize, ipc-error-path-coverage, snapshot-redaction, axe-presence, test-file-naming, md-link-targets); npm audit + license-checker + knip + audit.toml-in-sync. See `prek.toml` for the current hook list. Hook-tool config for typos / taplo / zizmor lives in `_typos.toml`, `.taplo.toml`, and `.github/zizmor.yml` respectively. The `migrations-immutable` hook enforces invariant #1 from the [invariants list](#key-architectural-invariants) at commit time.
- **Pre-push:** `verify-ci-equivalent` (`scripts/verify-ci-equivalent.sh`) is a fast-feedback subset of CI, **not** a full CI mirror (the hook ID is kept for stability). It runs sequentially: **Phase A** = every pre-commit hook against the whole tree; **Phases B–D** = vitest + cargo nextest scoped to the pushed commit range (`@{upstream}..HEAD`); **Phase D2** = `cargo test --doc` when any `.rs` changed (nextest skips doc-tests); **Phase E** = `cargo sqlx prepare --check` when any `.rs` changed in range; **Phase F** = `agaric-mcp` release build + MCP UDS smoke, only when MCP paths change; **Phase G** = warn-only `cargo audit` + `npm audit signatures`. Deliberately **not** run locally (CI only): Playwright e2e, the full `vitest run` / `cargo nextest run --profile ci` suites, the coverage-ratchet and bundle-budget gates, and the desktop bundle / cross-OS / SLSA build. A green pre-push is therefore **not** CI-equivalent — if you touched interaction-heavy code, run `npx playwright test` manually before pushing. `SKIP_CI_VERIFY='<reason>' git push` is the escape hatch for docs-only fixes — the value must be a real reason (≥8 chars); a bare truthy flag (`SKIP_CI_VERIFY=1`) is hard-rejected. Use e.g. `SKIP_CI_VERIFY='docs typo, no source change' git push`. Release pre-flight bundle build is opt-in via `scripts/verify-release-build.sh` (not in pre-push — too slow for daily cadence).
- **CI:** `.github/workflows/_validate.yml` — split into focused jobs after `lint`, `vitest` (sharded matrix), `playwright` (sharded matrix), `cargo-tests`, `validate-all` (aggregate). See `.github/workflows/_validate.yml` for the current job graph and shard counts. `ci.yml` consumes the workflow + adds the desktop build matrix (`ubuntu-24.04`, `windows-2025`, `macos-15`) and Android job. `release.yml` adds the bundle build + SLSA attest + release upload on tag pushes.
