...
6. **sqlx compile-time queries** — `query!` / `query_as!` / `query_scalar!`. `.sqlx/` cache committed. Run **`just gen-sqlx`** after SQL changes, then commit the regenerated cache (the `-- --tests` scope is required so test-only queries are cached — the verifier runs `cargo sqlx prepare --check -- --tests`). **Multi-crate workspace caveat:** the workspace has several query-bearing crates (`agaric`, `agaric-core`, `agaric-store`, and the bin-only `agaric-diagnostics`). The CI `lint` job compiles them all *offline* (`cargo clippy --workspace --all-targets`, no `DATABASE_URL`), so every crate's queries must be in a committed `.sqlx/`. A bare `cargo sqlx prepare` only builds the default member (`agaric`) + its deps and **silently drops leaf crates nothing depends on** (e.g. `diagnostics`), reddening `lint` on the next non-docs PR. `just gen-sqlx` passes cargo's `--workspace` through (`cargo sqlx prepare --workspace -- --workspace --tests`) so leaf-crate queries are captured — always regenerate via the recipe, never the bare command. `agaric-store` additionally keeps its own crate-local `.sqlx/` (checked by a second CI lane); `just gen-sqlx` refreshes the workspace-root cache, and the store cache is regenerated from within `src-tauri/agaric-store` when its queries change.
...
- **SQLx + `.sqlx/` cache.** The `sqlx` crate version and the committed `.sqlx/` query cache must match. If `sqlx` is bumped, regenerate `.sqlx/` with `just gen-sqlx` in the same commit — do not land a version bump with a stale cache. (`just gen-sqlx` runs the workspace-wide `cargo sqlx prepare --workspace -- --workspace --tests`; see invariant #6 for why the bare command silently drops leaf-crate queries.)
