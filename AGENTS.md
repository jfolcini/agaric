...
- **Pre-commit:** `prek.toml` is the hook list — read it rather than trusting any summary here. Broadly it covers builtin file checks; secret + workflow security (gitleaks, zizmor, actionlint); frontend lint/format/typecheck/test (oxlint, oxfmt, tsc, vitest); Rust (cargo fmt/clippy/test/deny/machete); SQL + schema (sqruff, migrations-immutable, migrations-strict-tables); cross-cutting text checks (typos, shellcheck, taplo, markdownlint, lychee); supply chain (npm-audit, license-checker, knip); and a large set of repo-specific guards that mechanically enforce the invariants in this file. Most guards ship a paired `*-self-test` hook so the guard itself can't silently rot. Hook-tool config lives in `_typos.toml`, `.taplo.toml`, and `.github/zizmor.yml`. The `migrations-immutable` hook enforces invariant #1 from the [invariants list](#key-architectural-invariants) at commit time.
...
- **Rust logs reach `adb logcat` from a RELEASE build too** — `adb logcat | grep RustStdoutStderr` is the first move when diagnosing on-device behaviour, and it needs no debuggable build. `init_logging` (`src-tauri/src/lib.rs`) attaches a stderr `fmt` layer with no `cfg(debug_assertions)` gate, and `tao` 0.35.2's `ndk_glue::create()` unconditionally `dup2`s `STDOUT_FILENO`/`STDERR_FILENO` into a pipe forwarded to `__android_log_write` under the tag `RustStdoutStderr` — gated on neither `isDebuggable` nor the Cargo profile. Verified both by reading `tao` and by observing a shipped 0.9.6 release APK emit its full `tracing` stream. **The version is pinned here on purpose:** `tao` is a crates.io dependency, not vendored, so this module path is unverifiable from a fresh checkout and can move across upgrades — re-check it when bumping `tao` ([Coupled Dependency Updates](#coupled-dependency-updates)). Only `run-as` (reading app-private storage) genuinely requires a debuggable build; the `shouldLog() = BuildConfig.DEBUG` gate in `src-tauri/gen/android/app/src/main/java/com/agaric/app/generated/Logger.kt` (generated, gitignored) belongs to Tauri's Kotlin-side WebView logger, a different pipe that does not carry these lines. #3852 stalled for days on the opposite belief.
