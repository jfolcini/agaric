...
- **Reviews judge impact.** A finding needs a concrete failure and a named victim. Otherwise fix it if trivial, or let it go. Filing an issue is the last resort, not the default. Over-building is a finding too: a helper, option, abstraction, or paragraph the fix did not need is deleted, not defended. On an approved, green PR a non-blocking note never causes a push by itself: fold it into a push that is going out anyway, or reply once, and merge; every push is another review round.
...
4. **A bug that reached a user lands a spec in `e2e-tauri/`.** That lane runs the real backend, so it is the only frontend surface with no second implementation between the test and the truth, and a bug that shipped is by definition one the mock-backed estate did not catch. #3081 is the worked example: a real atomicity defect (a committed tag create, then a swallowed `SetProperty(space)`) that the mock never exhibited because it was still reading the `block_properties(key='space')` rows migrations 0087/0088 retired. Until #4671 lands, that lane is `schedule` + `workflow_dispatch` only, so pair the spec with one that runs per-PR — on its own it will not guard the PR that reintroduces the bug.
