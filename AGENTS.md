...
- **Serialize the gate; parallelize only the work in front of it.** The pre-push verify is the one step that must never run concurrently with itself. Two or more full gates at once get silently killed by `earlyoom` — the branches simply never reach the remote, and nothing reports a failure. Fan out builders and reviewers as wide as the work allows; push **one branch at a time**. Three traps around it, each of which reads as something other than what it is:
  - A heavy gate takes longer than a 10-minute foreground command limit, so it must be backgrounded — and then **left alone**.
  ...
  - A gate that passes can still fail to push, because a rebased branch is no longer a fast-forward. That is the one legitimate `SKIP_CI_VERIFY` case beyond docs: the reason must say the gate already passed on that exact SHA and only the ref update failed.

  In every one of these cases the branch keeps its commits — nothing is lost but the run. **Confirm with `git ls-remote origin <branch>`, never with an exit code**, which reports the wrapper's status rather than the push's.
