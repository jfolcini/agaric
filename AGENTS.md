...
- **The `tao` version above is pinned on purpose.** `tao` is a transitive crates.io dependency, not vendored, so that module path is unverifiable from a fresh checkout and can move across upgrades — re-check it when the Tauri stack moves ([Coupled Dependency Updates](#coupled-dependency-updates)). #3852 stalled for days on the opposite belief.
