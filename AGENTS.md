...
**Self-tests.** A guard gets a self-test **only if it parses source text with its own parser** — only those can fail open silently. Guards that glob filenames, compare exit codes, diff a baseline, or grep a fixed string do not qualify; a defect in those surfaces as a false positive, which is self-announcing. **All self-tests are CI-only** (`stages = ["manual"]`, run by the `--hook-stage manual` step in `_validate.yml`); none run at pre-commit. A script that is not itself a guard hook does not get a self-test hook — if it needs tests it gets a vitest file, like `scripts/check-bundle-budget.test.mjs`. (The restaging and that `_validate.yml` step are Phase 2 of #4556; until they land the surviving self-tests are still `pre-commit`-staged. The rule above governs what may be *added* from now on either way.)
