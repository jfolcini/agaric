...
- **Schema:** application tables + an FTS5 virtual table (`fts_blocks`, trigram tokenizer) + internal/cache tables (`materializer_retry_queue`, `materializer_apply_cursor`, `gcal_settings`, `gcal_agenda_event_map`, `gcal_space_config`, `_op_log_mutation_allowed`); indexes and triggers maintained across the migrations directory. See `src-tauri/migrations/` for the current schema set.
...
- **Error handling:** `AppError` enum (variants include `Database`, `Migration`, `Io`, `Json`, `Ulid`, `NotFound`, `InvalidOperation`, `Channel`, `Snapshot`, `Validation`, `NonReversible`, `Gcal`) serializes to `{ kind, message }` for Tauri 2 IPC. Specta-derived TS bindings. See `src-tauri/src/error.rs` for the current variant set.
...
3. **Filter primitives — the parser is the single source of truth.** PEND-54 introduces an inline filter syntax (`tag:#name`, `path:GLOB`, `not-path:GLOB`) at `src/lib/search-query/`. The query string is the canonical state; chips and IPC fields are derived by `parse()` + `astToFilterProjection()`. Surfaces consuming the AST MUST NOT fork the parser — register new token prefixes through `registerTokenPrefix` (e.g. PEND-53's `state:`, `priority:`, …) and declare their `ALLOWED_KEYS` statically rather than re-parsing the query string. The round-trip invariant `parse(serialize(parse(s))) === parse(s)` for any input `s` is enforced by `fast-check` property tests.
4. **`path:` filters resolve against `pages_cache.title` with `LOWER(...)` for case-insensitive matching.** SQLite native `GLOB` (no regex extension), with brace expansion + substring-wrap applied in Rust before binding. Validation failures surface through `AppError::Validation` with an `InvalidGlob:` message prefix — the frontend keys on the prefix string rather than a new error variant (avoids reshaping the `{kind, message}` wire shape at `error.rs`).
5. **`↑` / `↓` precedence in the search input** — when ambiguity exists, autocomplete-open wins, then history recall (PEND-55), then result-list navigation. Document any new precedence-claiming surface here.
6. **PEND-55 — Search toggle row.** Three additive `bool` fields on `SearchFilter` (`case_sensitive`, `whole_word`, `is_regex`) drive a post-FTS regex pipeline at `src-tauri/src/fts/toggle_filter.rs`. **Regex mode bypasses FTS5 entirely** — FTS5 MATCH cannot accept a regex. The regex path scans the structurally-filtered block set ordered by `b.id DESC` (ULID prefixes are time-sortable; `blocks` has no `created_at` column). Wall-time is bounded by `REGEX_PRE_FILTER_CAP = 1000`. Per-row offset output is capped at `MAX_OFFSETS_PER_BLOCK = 50` so a `.` regex against a long block doesn't balloon the IPC payload. `RegexBuilder::size_limit` and `dfa_size_limit` are both `10 * 1024 * 1024`; pattern length cap is `MAX_PATTERN_LEN = 1024`. Compile failures surface as `AppError::Validation` with the `InvalidRegex:` prefix.
7. **PEND-55 — `MatchOffset` carries UTF-16 code-unit offsets, NOT bytes.** Rust `regex` matches return byte offsets into UTF-8 buffers; JavaScript indexes UTF-16. The conversion happens in Rust before serialising (`fts::toggle_filter::byte_to_utf16_offsets`) so the frontend can slice `row.content.substring(start, end)` directly. `日`/`本`/`語` are 3 bytes / 1 UTF-16 code unit each; `🌟` is 4 bytes / 2 UTF-16 code units. Frontend renderers must NOT re-convert.
8. **PEND-55 — Whole-word boundary is ASCII via `(?-u:\b)`.** CJK content does NOT match the boundary — CJK characters aren't ASCII word characters. Documented as a v1 limitation; a future plan revisits Unicode whole-word. Tests in `fts::toggle_filter::unit_tests` and `commands::tests::toggle_filter_tests` lock the behaviour.
9. **PEND-55 — Search history is per-space, Zustand-persisted at `agaric:search-history`.** PEND-54 query strings can carry space-specific `tag:` / `path:` references; cross-space recall would silently no-match. Cap at `MAX_HISTORY = 20` entries per space; submit-time dedupe moves the existing entry to the front. Toggle state is global and stored at `agaric:searchToggles:v1` (toggle semantics are space-agnostic).
10. **PEND-53 — Property filter `prop:KEY=VALUE` matches `block_properties.value_text` ONLY.** The five typed value columns on `block_properties` (`value_text`, `value_num`, `value_date`, `value_ref`, `value_bool`) are mutually exclusive per migration `0062`'s CHECK constraint. v1 ships against `value_text` to cover the common user-typed property case; do NOT extend `SearchFilter::property_filters` to scan other columns without (a) a follow-up plan, (b) updated docs in `docs/SEARCH.md`, and (c) explicit IPC error messaging when a user-typed `prop:` query hits a typed column. **Property keys are case-sensitive** (`block_properties` PK is `(block_id, key)` with no `COLLATE NOCASE`).
11. **PEND-53 — Date filter resolution lives in Rust at query time.** `due:today` / `due:this-week` etc. are resolved against `chrono::Local::now().date_naive()` inside `fts::metadata_filter::prepare_metadata` so the SQL only ever sees concrete date bounds. Week starts on **Monday** (mirrors the agenda view's convention). Test-injected `today` is threaded through `prepare_metadata_with_today` so date-clock churn doesn't flake the snapshot suite. Bucket-keyword vocabulary is locked at: `today`, `yesterday`, `overdue`, `this-week`, `this-month`, `next-week`, `older`, `none`. Unknown buckets and unparseable dates surface as `AppError::Validation("InvalidDateFilter: …")` — the frontend keys on the prefix.
12. **PEND-53 — `state:none` / `priority:none` route to `IS NULL`.** A literal `none` value (case-insensitive) in `state_filter` / `priority_filter` is split out in `prepare_metadata` and emitted as a `column IS NULL` branch in the SQL disjunction. A custom state named `"none"` is matched the same way — documented in `docs/SEARCH.md`. A real custom state must be named differently (`NONE-LITERAL`, etc.) if the `IS NULL` shadow needs to be avoided.
