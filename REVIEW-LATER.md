...
23 open items — 23 planned work (FEAT/MAINT/PERF/PUB). All frontend test-quality items closed. All five LOW backend cleanup batches (MAINT-148..152) closed. **All INFO/nits closed (last 5 in session 547). All UX-* items closed (last 3 in session 548). 17 backend Medium findings closed + 23 MAINT closed (some partially) across sessions 549-577 — see SESSION-LOG.md for the full session-by-session sequence. Latest progress (sessions 572-577): MAINT-131 fully reduced to residual cleanup; MAINT-162 reduced to 1 component remaining (StaticBlock); **MAINT-125 progress: 41 of ~80 wrappers migrated (~51%)**; **MAINT-124 progress: App.tsx 1444L → 907L (–537L, ~37% reduction), 2 extractions remaining (`useAppDialogs`, `<ViewDispatcher>`)**.

Previously resolved: 831+ items across 544 sessions (per SESSION-LOG.md unique session count; latest is session 577).
...
| MAINT-124 | MAINT | Collapse `src/App.tsx` — was 1444L, then 1139L after session 576's `useAppKeyboardShortcuts()` extraction, now 907L after the `<AppSidebar>` extraction (sidebar JSX + `CollapseButton` + `syncDotClass` → `src/components/AppSidebar.tsx` 284L; `NAV_ITEMS` lifted to shared `src/components/nav-items.ts` so `useHeaderLabel` keeps working without a circular import). 2 extractions remaining: `useAppDialogs()` (modal/dialog state — bug-report, no-peers, shortcuts, quick-capture open/handlers) and `<ViewDispatcher>` (currentView-switching component; partly already extracted as `ViewRouter`). Target ≤500L overall. The non-keyboard FEAT-12 quick-capture global hotkey (storage event + Tauri OS-level `registerGlobalShortcut`) and CLOSE_ALL_OVERLAYS_EVENT bridge stayed in App.tsx — out of scope for the keyboard-shortcuts category. | M |
| MAINT-125 | MAINT | `src/lib/tauri.ts` hand-writes `invoke('<str>', ...)` wrappers for commands already typed by `src/lib/bindings.ts` (auto-generated by Tauri Specta). Every new backend command is added in two places; renames drift silently. Migration in progress — **41 of ~80 wrappers migrated (~51%)** via `unwrap` helper: batch 1 (session 574, 11 block-domain) + batch 2 (session 576, 15 list/agenda/property/page-meta) + batch 3 (session 577, 15 trash/tags/properties/property-defs/listUnlinkedReferences: `restoreAllDeleted`, `purgeAllDeleted`, `trashDescendantCounts`, `listTagsByPrefix`, `listTagsForBlock`, `setProperty`, `deleteProperty`, `getProperties`, `getBatchProperties`, `listUnlinkedReferences`, `listPropertyKeys`, `createPropertyDef`, `listPropertyDefs`, `updatePropertyDefOptions`, `deletePropertyDef`). ~39 wrappers still on raw `invoke` (next: `createPageInSpace`, sync/pairing, gcal, drafts, compaction, bug-report/diagnostics, etc.). Public surface of `tauri.ts` unchanged. Pairs with MAINT-123 (typed `HANDLERS`). | M |
