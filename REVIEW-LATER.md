...
23 open items — 23 planned work (FEAT/MAINT/PERF/PUB). All frontend test-quality items closed. All five LOW backend cleanup batches (MAINT-148..152) closed. **All INFO/nits closed (last 5 in session 547). All UX-* items closed (last 3 in session 548). 17 backend Medium findings closed + 18 MAINT closed across sessions 549-566 (M-16, M-18, M-67, M-91 in 549; M-12, M-27, M-37, M-70, M-92 in 550; M-6 stale, M-26, M-71, M-84, M-94 in 551; M-17 partial tags+pages, M-28, MAINT-123 in 552; M-17 final projected_agenda+page_id, MAINT-114 spike-keep in 553; MAINT-137 in 554; MAINT-119, MAINT-122 in 555; MAINT-126, MAINT-132, MAINT-142 in 557; MAINT-120, MAINT-129, MAINT-139 in 558; MAINT-159, MAINT-164 in 559 + MAINT-130 partial 12-of-15; MAINT-116 + MAINT-130(a) in 560; MAINT-118 in 561; MAINT-130(b) + MAINT-127 page-blocks.ts portion in 562; MAINT-127 keyboard-config.ts portion in 563; MAINT-130(c) sub-tasks 1+2 in 564; MAINT-130(c) sub-task 3 + MAINT-165 in 565; MAINT-128 SettingsView + SortableBlock decompositions in 566 — MAINT-128 reduced from 9 to 7 components in scope; MAINT-127 still has 2 god-files in scope).**

Previously resolved: 814+ items across 533 sessions (per SESSION-LOG.md unique session count; latest is session 566).
...
| MAINT-128 | MAINT | God-component decomposition batch — 7 of 9 components remaining after session 566 closed `SettingsView.tsx` (656L → 198L; 5 new siblings under `src/components/settings/`) and `SortableBlock.tsx` (584L → 507L; 3 hooks extracted: `useAttachmentCount`, `usePropertyDefForEdit`, `useBlockContextMenu`). Remaining: `PageBrowser.tsx` (961L → ~300), `BlockTree.tsx` (899L, extract `useBlockLinkResolve` / `useBlockPropertiesBatch` / `useBlockNavigateToLink`), `TrashView.tsx` (788L), `ConflictList.tsx` (737L — also removes direct DOM-mutation anti-pattern at L287-300 where `item.setAttribute('role','option')` is called from a `useEffect`), `HistoryView.tsx` (528L → `useHistorySelection` + `useHistoryKeyboardNav` + `HistoryListView`), `PropertyRowEditor.tsx` (539L — dispatch by `def.value_type`; deletes the `biome-ignore lint/complexity/noExcessiveCognitiveComplexity` at L85), `backlink-filter/AddFilterRow.tsx` (556L, child 2.5× its parent). Do NOT try to land in one PR — stagger per file. **Stretch-target gaps for partially-closed components:** SettingsView gap to ≤150L closes via `useSettingsTab` hook for localStorage+URL persistence (~50L drop expected); SortableBlock gap to ≤250L closes via JSX extraction `<SortableBlockBody>` carving the swipe-content div. Both deferred. | L |
