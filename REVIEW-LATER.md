...
| MAINT-124 | MAINT | Collapse `src/App.tsx` — was 1444L, now 1139L after session 576's `useAppKeyboardShortcuts()` extraction (5 keyboard effects + 124L of dispatch tables/helpers → 1 hook call; 17 useEffect blocks → 11). 3 extractions remaining: `useAppDialogs()` (modal/dialog state), `<ViewDispatcher>` (currentView-switching component; partly already extracted as `ViewRouter`), `<AppShell>` (sidebar tree at L854-1089, was L1159-1394). Target ≤500L overall. The non-keyboard FEAT-12 quick-capture global hotkey (storage event + Tauri OS-level `registerGlobalShortcut`) and CLOSE_ALL_OVERLAYS_EVENT bridge stayed in App.tsx — out of scope for the keyboard-shortcuts category. | M |
| MAINT-125 | MAINT | `src/lib/tauri.ts` hand-writes `invoke('<str>', ...)` wrappers for commands already typed by `src/lib/bindings.ts` (auto-generated by Tauri Specta). Every new backend command is added in two places; renames drift silently. Migration in progress — **26 of ~80 wrappers migrated** via `unwrap` helper: batch 1 (session 574, 11 block-domain) + batch 2 (session 576, 15 list/agenda/property/page-meta: `listBlocks` w/ AgendaQuery shape, `listUndatedTasks`, `listProjectedAgenda`, `searchBlocks`, `queryByTags`, `queryByProperty`, `countAgendaBatch`, `countAgendaBatchBySource`, `listPageLinks`, `queryBacklinksFiltered`, `listBacklinksGrouped`, `setPageAliases`, `getPageAliases`, `resolvePageByAlias`, `exportPageMarkdown`). ~54 wrappers still on raw `invoke` (next: `createPageInSpace`, `listUnlinkedReferences`, sync, gcal, drafts, attachments mutation, peer/pairing, etc.). Public surface of `tauri.ts` unchanged across the migration. Pairs with MAINT-123 (typed `HANDLERS`). | M |
