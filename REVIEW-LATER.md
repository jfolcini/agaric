...
24 open items — 24 planned work (FEAT/MAINT/PERF/PUB). All frontend test-quality items closed. All five LOW backend cleanup batches (MAINT-148..152) closed. **All INFO/nits closed (last 5 in session 547). All UX-* items closed (last 3 in session 548). 17 backend Medium findings closed + 16 MAINT closed across sessions 549-564 (M-16, M-18, M-67, M-91 in 549; M-12, M-27, M-37, M-70, M-92 in 550; M-6 stale, M-26, M-71, M-84, M-94 in 551; M-17 partial tags+pages, M-28, MAINT-123 in 552; M-17 final projected_agenda+page_id, MAINT-114 spike-keep in 553; MAINT-137 in 554; MAINT-119, MAINT-122 in 555; MAINT-126, MAINT-132, MAINT-142 in 557; MAINT-120, MAINT-129, MAINT-139 in 558; MAINT-159, MAINT-164 in 559 + MAINT-130 partial 12-of-15; MAINT-116 + MAINT-130(a) in 560; MAINT-118 in 561; MAINT-130(b) + MAINT-127 page-blocks.ts portion in 562; MAINT-127 keyboard-config.ts portion in 563; MAINT-130(c) sub-tasks 1+2 in 564 — MAINT-130 reduced to sub-task 3 only; MAINT-127 still has 2 god-files; new MAINT-165 added in 564 for parallel ref-plumbing drift in 4 sibling pickers).**

Previously resolved: 810+ items across 531 sessions (per SESSION-LOG.md unique session count; latest is session 564).
...
| MAINT-127 | MAINT | God-file decomposition (libs + stores) — 2 of 4 god-files remaining after session 562 closed `page-blocks.ts` and session 563 closed `keyboard-config.ts`. `src/stores/navigation.ts` (543L) — tab engine + sidebar view interleaved; splitting requires cross-store coordination, NOT a clean move (rejected as in-scope for the simple-refactor pattern; needs design review before proceeding). `src/hooks/useGraphSimulation.ts` (716L) — 9 banner-comment sections at L39/58/112/278/325/361/516/603/617 mark the seams (`useGraphZoom`, `useGraphWorkerSimulation`, `useGraphRenderElements`); the only remaining mechanical extraction. | L |
...
| MAINT-130 | MAINT | Frontend cleanup follow-ups — sub-task 3 only after sessions 559-564. Factor out `createPickerPlugin(cfg)` for the ~50-70 LOC/file shared `addProseMirrorPlugins` shell across the 5 pickers (`at-tag-picker`, `block-link-picker`, `block-ref-picker`, `slash-command`, `property-picker`); keep per-picker `addInputRules` separate (they diverge per the 5-picker capability matrix). Architectural unification — best paired with MAINT-165 (parallel ref-plumbing fix for the 4 unfixed pickers) since both touch the same surface. | M |
| MAINT-165 | MAINT | **Parallel ref-plumbing drift in 4 sibling pickers** (latent bug discovered during MAINT-130(c) review in session 564). Same fragile-API-contract shape as the BlockRefPicker fix that just shipped — `AtTagPicker.configure({ items: searchTags })` (use-roving-editor.ts:325), `BlockLinkPicker.configure({ items: searchPages })` (line 333), `SlashCommand.configure({ items: searchSlashCommands })` (line 344), and `PropertyPicker.configure({ items: searchPropertyKeys })` (line 348) all capture their items callback at configure time. Currently NOT observable as a bug because all production callers (`useBlockResolve.ts:244-291`, etc.) wrap their search functions in `useCallback([])` with empty deps — the function reference is stable across renders. Apply the same wrapper-closure pattern (`items: (q) => searchTagsRef.current(q)`, etc.) + add 4 refs (`searchTagsRef`, `searchPagesRef`, `searchSlashCommandsRef`, `searchPropertyKeysRef`) mirroring the existing `searchBlockRefsRef` at line 280-281. Mechanically identical to sub-task 2 of MAINT-130(c). Cost: S. Risk: Low (additive, defensive). Impact: Low-to-medium (defends against future caller patterns that don't use `useCallback([])`). | S |
...
### MAINT-130 — Frontend cleanup follow-ups (sub-task 3 of (c) only after session 564)

**Context:** sessions 559 + 560 + 562 + 564 closed 16 of 17 original MAINT-130 sub-items: 12 in 559 (dead-code deletions, renames, convention-drift surgical fixes, `ConflictList` DOM-mutation refactor) + sub-item (a) in 560 (`<ConfirmDestructiveAction>` wrapper) + sub-item (b) in 562 (`useStarredPages` hook extraction) + sub-item (c) sub-tasks 1+2 in 564 (BlockRefPicker capability gap + `searchBlockRefsRef` ref-plumbing fix at use-roving-editor.ts:341).

**(c) sub-task 3 — `createPickerPlugin(cfg)` factory.** Factor out the shared `addProseMirrorPlugins` shell across all 5 pickers (~50-70 LOC/file of copied `try / await / logger.warn / command: { editor, range, props }` boilerplate) into a single `createPickerPlugin(cfg)` helper. Keep per-picker `addInputRules` separate (they diverge per the 5-picker capability matrix — see the BlockRefPicker work in session 564). Architectural unification across 5 extension files. **Best paired with MAINT-165** (parallel ref-plumbing fix for the 4 unfixed pickers) since both touch the same surface.

**Cost:** M.
**Risk:** Medium — the factor-out touches all 5 picker files; pair with MAINT-165 to land both as one consistent change.
**Impact:** S–M — unifies the picker plugin shell, removing ~250-350 LOC of duplication.

**Notes on closed sub-items:**

- **Sub-item (a) — session 560:** The new `ConfirmDestructiveAction` wrapper (i18n-key API + async-aware auto-close on success) sits alongside the existing `ConfirmDialog` (pre-translated strings + sync `onAction`). Both serve different purposes and coexist cleanly: ConfirmDialog for sync confirmations (used by 23 callsites today including TrashView, PageBrowser, HistoryView, `McpStatusSection`, `UnpairConfirmDialog`); ConfirmDestructiveAction for async operations needing auto-close (used by PairingDialog's close-guard).
- **Sub-item (b) — session 562:** `useStarredPages` hook extracted; `starredRevision` re-render counter eliminated at both `PageBrowser` and `PageHeader` callsites; cross-instance sync via `window.dispatchEvent('starred-pages-changed')` event broadcast on top of the existing `src/lib/starred-pages.ts` synchronous storage layer.
- **Sub-item (c) sub-tasks 1+2 — session 564:** BlockRefPicker now has `Commands` module augmentation + `addCommands()` (`resolveBlockRefFromSelection`) + `addInputRules()` for `((text))` auto-resolution (case-insensitive exact match against `item.label`; no `onCreate` path because block refs reference arbitrary mid-content blocks with no sensible "create" target). Ref-plumbing fix at `use-roving-editor.ts:341` migrated to wrapper-closure pattern matching `TagRef.configure` at L309-313. **MAINT-165 was opened during this work** for the parallel latent drift in 4 sibling pickers (AtTagPicker `searchTags`, BlockLinkPicker `searchPages`, SlashCommand `searchSlashCommands`, PropertyPicker `searchPropertyKeys`) — currently not observable because all production callers use `useCallback([])` but architecturally fragile.
