...
24 open items — 24 planned work (FEAT/MAINT/PERF/PUB). All frontend test-quality items closed. All five LOW backend cleanup batches (MAINT-148..152) closed. **All INFO/nits closed (last 5 in session 547). All UX-* items closed (last 3 in session 548). 17 backend Medium findings closed + 14 MAINT closed across sessions 549-560 (M-16, M-18, M-67, M-91 in 549; M-12, M-27, M-37, M-70, M-92 in 550; M-6 stale, M-26, M-71, M-84, M-94 in 551; M-17 partial tags+pages, M-28, MAINT-123 in 552; M-17 final projected_agenda+page_id, MAINT-114 spike-keep in 553; MAINT-137 in 554; MAINT-119, MAINT-122 in 555; MAINT-126, MAINT-132, MAINT-142 in 557; MAINT-120, MAINT-129, MAINT-139 in 558; MAINT-159, MAINT-164 in 559 + MAINT-130 partial 12-of-15; MAINT-116 + MAINT-130(a) in 560 — MAINT-130 now has 2 deferred sub-items).**

Previously resolved: 805+ items across 527 sessions (per SESSION-LOG.md unique session count; latest is session 560).
...
### MAINT-130 — Frontend cleanup follow-ups (2 deferred sub-items remaining after sessions 559+560)

**Context:** sessions 559 + 560 closed 13 of 15 original MAINT-130 sub-items: 12 in 559 (dead-code deletions, renames, convention-drift surgical fixes, the `ConflictList` DOM-mutation refactor) + sub-item (a) in 560 (the `<ConfirmDestructiveAction>` wrapper, scoped down to a coexisting alternative to `ConfirmDialog` after the original description's "AgentAccess/DeviceManagement have no confirmation" claim turned out to be incorrect — they already use `ConfirmDialog` via `McpStatusSection` / `UnpairConfirmDialog`).

**(b) `useStarredPages` hook extraction.** `PageBrowser` and `PageHeader` call `toggleStarred()` (localStorage-backed) directly with a `starredRevision` counter as a re-render workaround. Extract `useStarredPages` so the localStorage shape is owned in one place and consumers subscribe via the hook. Cost: S–M.

**(c) BlockRefPicker capability gap + `use-roving-editor` ref drift + `createPickerPlugin` factory.** `src/editor/extensions/block-ref-picker.ts` is the odd picker out among 5 (no `addInputRule`, no `onCreate`, no `Commands` module augmentation — users can type `[[foo]]` and auto-resolve via `BlockLinkPicker` but cannot do the same for `((foo))` even though the underlying node supports it). Add the missing three hooks. Same commit fixes `src/editor/use-roving-editor.ts:280-281 / 340-342` Pattern-C ref plumbing — `BlockRefPicker.configure({ items: searchBlockRefsRef.current })` reads `.current` at configure time, defeating the ref; migrate to the wrapper-closure pattern used by `TagRef.configure` at L309-313. Then factor out `createPickerPlugin(cfg)` for the shared `addProseMirrorPlugins` shell across all 5 pickers (~50-70 LOC/file of copied `try / await / logger.warn / command: { editor, range, props }` boilerplate); keep per-picker `addInputRules` separate (they diverge per the 5-picker capability matrix). Cost: M–L.

**Cost:** M overall.
**Risk:** Low for (b); Medium for (c) — the ref-plumbing fix touches editor hot paths, and the `createPickerPlugin` factory is an architectural unification across 5 extension files.
**Impact:** S–M — removes a re-render workaround (b); fixes a behavior mismatch among the 5 pickers (c).

**Note (sub-item (a) closed in 560):** The new `ConfirmDestructiveAction` wrapper added in session 560 (i18n-key API + async-aware auto-close on success) sits alongside the existing `ConfirmDialog` (pre-translated strings + sync `onAction`). Reviewer (`a60fdc93`) confirmed both serve different purposes and coexist cleanly: ConfirmDialog for sync confirmations (used by 23 callsites today including TrashView, PageBrowser, HistoryView, `McpStatusSection`, `UnpairConfirmDialog`); ConfirmDestructiveAction for async operations needing auto-close (used by PairingDialog's close-guard). Migrating other callsites is possible but not urgent — they already work correctly via the existing `ConfirmDialog` → Radix AlertDialog chain. The 3-way disconnect dialog in `GoogleCalendarSettingsTab` (cancel + keep + delete) genuinely doesn't fit a binary destructive wrapper; left as a documented exception.
