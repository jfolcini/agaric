...
23 open items — 23 planned work (FEAT/MAINT/PERF/PUB). All frontend test-quality items closed. All five LOW backend cleanup batches (MAINT-148..152) closed. **All INFO/nits closed (last 5 in session 547). All UX-* items closed (last 3 in session 548). 17 backend Medium findings closed + 21 MAINT closed (some partially) across sessions 549-574 (M-16, M-18, M-67, M-91 in 549; M-12, M-27, M-37, M-70, M-92 in 550; M-6 stale, M-26, M-71, M-84, M-94 in 551; M-17 partial tags+pages, M-28, MAINT-123 in 552; M-17 final projected_agenda+page_id, MAINT-114 spike-keep in 553; MAINT-137 in 554; MAINT-119, MAINT-122 in 555; MAINT-126, MAINT-132, MAINT-142 in 557; MAINT-120, MAINT-129, MAINT-139 in 558; MAINT-159, MAINT-164 in 559 + MAINT-130 partial 12-of-15; MAINT-116 + MAINT-130(a) in 560; MAINT-118 in 561; MAINT-130(b) + MAINT-127 page-blocks.ts portion in 562; MAINT-127 keyboard-config.ts portion in 563; MAINT-130(c) sub-tasks 1+2 in 564; MAINT-130(c) sub-task 3 + MAINT-165 in 565; MAINT-128 SettingsView + SortableBlock in 566; MAINT-128 HistoryView + ConflictList in 567; MAINT-128 TrashView + PageBrowser in 568; MAINT-128 BlockTree + MAINT-167 in 569; MAINT-127 useGraphSimulation in 570; MAINT-128 AddFilterRow in 571; MAINT-131 SortableBlock badge-count batch IPC in 572; MAINT-162 HistoryView+ConflictList+TrashView+TagFilterPanel grid flips in 573; MAINT-125 batch-1 (11 block-domain wrappers via `unwrap`) + MAINT-162 PageBrowser grid flip in 574 — **MAINT-128 reduced to 1 component remaining (PropertyRowEditor, rejected as design-heavy)**; MAINT-127 reduced to 1 god-file remaining (`navigation.ts`, blocked pending design); MAINT-131 reduced to 7 components remaining (was 8 — SortableBlock closed); MAINT-162 reduced to 1 component remaining (was 6 — only StaticBlock left, design-review-needed); **MAINT-125 progress: 11 of ~80 wrappers migrated, downgraded from L→M**).**

Previously resolved: 825+ items across 541 sessions (per SESSION-LOG.md unique session count; latest is session 574).
...
| MAINT-124 | MAINT | Collapse `src/App.tsx` (1436L) — 20+ effects for independent concerns, keyboard shortcut logic scattered across 5 separate effects (journal / global / space / tab / close-overlays), sidebar tree inlined at L1159-1394 (236L). Extract `useAppKeyboardShortcuts()` (5 effects → 1), `useAppDialogs()`, `<ViewDispatcher>`, `<AppShell>` (sidebar header / menu / footer + main content). Same commit standardizes 4 different async styles used in the file. Target ≤500 lines. **Silent `.catch(() => {})` triplet at App.tsx:935-947 noted in earlier description was already fixed** (each `.catch` calls `logger.warn` with `{ op: 'unminimize' / 'show' / 'setFocus' }` context); the `.catch(() => false)` at L935 is intentional default-to-false-on-failure, not a swallow. | L |
| MAINT-125 | MAINT | `src/lib/tauri.ts` hand-writes `invoke('<str>', ...)` wrappers for commands already typed by `src/lib/bindings.ts` (auto-generated by Tauri Specta). Every new backend command is added in two places; renames drift silently. Migration in progress — **batch 1 (11 block-domain wrappers: createBlock, editBlock, deleteBlock, restoreBlock, purgeBlock, getBlock, batchResolve, moveBlock, addTag, removeTag, getBacklinks) shipped via `unwrap` helper that delegates to `commands.*`**. ~70 wrappers still on raw `invoke` (next batches: list/agenda commands incl. `listBlocks` AgendaQuery shape, properties, sync, gcal, drafts, attachments, peer/pairing, etc.). Public surface of `tauri.ts` unchanged across the migration. Pairs with MAINT-123 (typed `HANDLERS`) for end-to-end compile-time mock-drift detection. | M |
...
| MAINT-131 | MAINT | Block-surface Tauri coupling — 7 presentational components remaining (was 8: SortableBlock badge-count IPC closed via `get_batch_attachment_counts` + `useBatchAttachmentCounts` provider mounted at BlockTree, `useAttachmentCount` hook deleted). Open: (a) StaticBlock per-row `useBlockAttachments` IPC needs a `list_attachments_batch` returning full `Vec<AttachmentRow>` plus cache-invalidation rework for add/delete mutations; (b) wrap remaining per-IPC calls in hooks (`useBlockReschedule` around setDueDate/setScheduledDate; `useLinkMetadata` around fetchLinkMetadata) to drop direct `lib/tauri` imports from the 7 remaining components. Prevents isolated rendering (Storybook, unit tests w/o full mocks) too. | M |
| MAINT-162 | MAINT | ARIA role re-evaluation for list views — 1 of 6 components remaining after the parallel grid-flip batches closed HistoryView, ConflictList, TrashView, TagFilterPanel (session 573) and PageBrowser (session 574; including the FEAT-14 hard `aria-required-children` violation at `PageBrowser.test.tsx:2197`). 14 axe-rule overrides removed cumulatively. Remaining: `StaticBlock` (7 `nested-interactive` overrides at `StaticBlock.test.tsx:360, 440, 1447, 1529, 1574, 1608, 1686`; **not a true list-row** — the outer `role="button"` is the underlying smell, not the listbox/option mismatch the other 5 components had). Closing requires removing the outer `role="button"` and treating the block as a passive container with focusable inner controls (no role flip, no grid wrapper). Needs design review before flipping. | M |
...
|---|---|---|
| `src/stores/navigation.ts` | 543 | Originally framed as "move tab engine to `src/stores/tabs.ts`", but session 563 inspection found `currentView` (sidebar view) is interleaved with the tab engine — `navigateToPage`, `goBack`, `switchTab`, etc. read AND write both `currentView` and `tabs[]`. A clean move would require cross-store coordination (a 2-store solution coordinating via subscribers, or moving `currentView` setting INTO tab actions). **Splitting this file is NOT mechanical refactor scope** — needs design review before proceeding. AGENTS.md "Architectural Stability" implies new Zustand stores need approval; splitting one store into two coordinating stores is functionally equivalent to that. Skipped pending discussion. |
| `src/hooks/useGraphSimulation.ts` | 716 | 9 banner-comment sections at L39 / L58 / L112 / L278 / L325 / L361 / L516 / L603 / L617 mark the seams. Extract `useGraphZoom`, `useGraphWorkerSimulation`, `useGraphMainThreadSim`, `useGraphRenderElements`. The only remaining mechanical extraction in MAINT-127. |

**Cost:** M (just `useGraphSimulation.ts` left as a tractable next step; `navigation.ts` blocked pending design discussion).
**Risk:** Medium — `useGraphSimulation.ts` is on the graph-rendering hot path; needs render-perf benchmark before/after.
**Impact:** M — graph simulation regains reviewability when split.
...
**What:** 7 presentational components in `src/components/` still import functions from `src/lib/tauri` directly (`StaticBlock`, `EditableBlock`, `BlockListItem`, `BlockPropertyEditor`, `BlockPropertyDrawer`, `ImageResizeToolbar`, `LinkEditPopover`). `SortableBlock` was already Tauri-import-free (its IPC went through `useAttachmentCount`, now removed). `AttachmentList` imports types only — acceptable. `BlockTree` is the orchestrator — acceptable.

**Remaining per-block IPC:** `StaticBlock.tsx` calls `useBlockAttachments(blockId)` which hits `listAttachments` at `useBlockAttachments.ts:35`. Because `StaticBlock` is the non-focused view for every block row on the page, every block fires its own `listAttachments` IPC on mount to derive `hasImageAttachments` for the inline preview decisions. At 50 blocks/page that's 50 `listAttachments` IPCs on mount. Closing this requires a second batch IPC `list_attachments_batch(block_ids) -> HashMap<String, Vec<AttachmentRow>>` returning the full attachment lists, plus reworking `useBlockAttachments` to accept either a per-block fetch or a context-provided slice. The mutation callbacks (`handleAddAttachment` / `handleDeleteAttachment`) need to invalidate the page-level cache when they fire.

**Remaining work:**

1. **Done (this session):** Backend `get_batch_attachment_counts(block_ids: Vec<String>) -> HashMap<String, u32>` mirrors the `json_each()`-backed batch patterns in `commands/blocks/queries.rs::batch_resolve_inner` and `fts/index.rs:228-235`. Empty input returns empty map (not error); blocks with 0 attachments OR missing from the DB are silently absent from the map (frontend defaults missing keys to 0). Frontend `useBatchAttachmentCounts` provider mounted at `BlockTree`; `SortableBlock` reads its count from `batchCounts?.get(blockId) ?? 0` instead of firing `useAttachmentCount(blockId)` per row. `useAttachmentCount.ts` and its test file were deleted (no remaining consumers).
2. **Open:** `list_attachments_batch(block_ids) -> HashMap<String, Vec<AttachmentRow>>` for the StaticBlock per-block IPC half. Same `json_each()` pattern; the only complication is the cache-invalidation contract for `handleAddAttachment` / `handleDeleteAttachment` in `useBlockAttachments`.
3. **Open:** Wrap remaining per-IPC calls in hooks (`usePropertySave` already exists; add `useBlockReschedule` around `setDueDate` / `setScheduledDate`; `useLinkMetadata` around `fetchLinkMetadata`). Components become pure JSX + hooks, no direct `lib/tauri` imports.

**Cost:** M — primary fix shipped; remaining work is 1 backend command (full-list batch) + 2 hooks (`useBlockReschedule`, `useLinkMetadata`) + refactor of 7 components (probably 2 PRs).
**Risk:** Low — additive IPC; the `useBlockAttachments` cache-invalidation rework is the trickiest piece because attachments mutations need to keep the parent cache in sync.
**Impact:** M — eliminates the remaining per-block IPC on every page mount AND enables isolated rendering (Storybook, unit tests w/o full mocks) for block-surface components.

### MAINT-162 — ARIA role re-evaluation for list views (`role="listbox"` → `role="grid"`)

**What:** 1 of 6 components remaining after sessions 573 and 574 closed HistoryView, ConflictList, TrashView, TagFilterPanel, and PageBrowser. 14 axe-rule overrides removed cumulatively (9 in session 573, 5 in session 574 including the FEAT-14 hard `aria-required-children` violation). Remaining:

- **`StaticBlock` (block rows in BlockTree):** 7 `nested-interactive` overrides at `StaticBlock.test.tsx:360, 440, 1447, 1529, 1574, 1608, 1686`. **Not a true list-row** — StaticBlock is the static rendering of a single block (used inside SortableBlock or stand-alone in some contexts). The MAINT-162 description recommended *"reconsider — `<li>` + plain children may be enough; `role="button"` outer with nested buttons is the underlying smell"*. The overrides probably go away by removing the outer `role="button"` and treating the block as a passive container with focusable inner controls (no role flip, no grid wrapper, no gridcell needed). Needs design review before flipping — the `role="button"` carries a lot of semantic and keyboard-handler weight in the existing tests.

**Done across sessions 573–574:** HistoryView (HistoryListView + HistoryListItem), ConflictList (ConflictList + ConflictListItem), TrashView (TrashListView + TrashRowItem), TagFilterPanel, and PageBrowser (PageBrowser + PageBrowserRowRenderer; section headers `role="group"` → `role="row"` + gridcell decision; FEAT-14 mixed-mode tree-button rows wrapped in `role="row"` + `role="gridcell"`). Pattern established: `<div role="grid">` outer + `<div role="row">` inner + `<div role="gridcell">` content/actions wrappers, with `// biome-ignore lint/a11y/useSemanticElements` and `// biome-ignore lint/a11y/useFocusableInteractive` comments. ARIA-Components matching `<table>`/`<tr>`/`<td>` semantic elements is rejected because these are non-tabular interactive grids per WAI-ARIA APG. Reference: `src/components/journal/MonthlyView.tsx` was the prior precedent for this pattern.

**Drift caught + fixed:** the original "minimum behavior-preserving change is just flipping the role attributes" claim was incorrect. Removing `nested-interactive` suppressions revealed `aria-required-children` violations because `role="row"` requires `role="gridcell"` children. Each row needs 1-3 `<div role="gridcell">` wrappers. Also: e2e specs (`starred-pages.spec.ts`, `spaces-coverage.spec.ts`) directly query `role="listbox"` / `role="option"` on the PageBrowser viewport — those needed concurrent updates to keep the e2e matrix green.

**Cost:** S–M — StaticBlock might end up being a 1-line removal (drop the outer `role="button"` + 7 axe overrides) if its tests can be adapted, OR it requires a meaningful keyboard-handler refactor.
**Risk:** Medium — StaticBlock's tests heavily depend on the current `role="button"` semantics. The decision is whether the outer wrapper element should be a generic container (no role) with focus delegated entirely to inner controls, or whether some subset of clicks (the whole-block-click-to-focus pattern) needs to be preserved.
**Impact:** M — closes the last 7 axe-suppression sites in MAINT-162.
