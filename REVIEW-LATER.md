...
24 open items — 24 planned work (FEAT/MAINT/PERF/PUB). All frontend test-quality items closed. All five LOW backend cleanup batches (MAINT-148..152) closed. **All INFO/nits closed (last 5 in session 547). All UX-* items closed (last 3 in session 548). 17 backend Medium findings closed + 18 MAINT closed across sessions 549-568 (M-16, M-18, M-67, M-91 in 549; M-12, M-27, M-37, M-70, M-92 in 550; M-6 stale, M-26, M-71, M-84, M-94 in 551; M-17 partial tags+pages, M-28, MAINT-123 in 552; M-17 final projected_agenda+page_id, MAINT-114 spike-keep in 553; MAINT-137 in 554; MAINT-119, MAINT-122 in 555; MAINT-126, MAINT-132, MAINT-142 in 557; MAINT-120, MAINT-129, MAINT-139 in 558; MAINT-159, MAINT-164 in 559 + MAINT-130 partial 12-of-15; MAINT-116 + MAINT-130(a) in 560; MAINT-118 in 561; MAINT-130(b) + MAINT-127 page-blocks.ts portion in 562; MAINT-127 keyboard-config.ts portion in 563; MAINT-130(c) sub-tasks 1+2 in 564; MAINT-130(c) sub-task 3 + MAINT-165 in 565; MAINT-128 SettingsView + SortableBlock decompositions in 566; MAINT-128 HistoryView + ConflictList decompositions in 567; MAINT-128 TrashView + PageBrowser decompositions in 568 — MAINT-128 reduced from 9 to 3 components in scope; MAINT-127 still has 2 god-files in scope; new MAINT-167 added in 568 for missing unit tests on the 3 TrashView beyond-spec hooks).**

Previously resolved: 818+ items across 535 sessions (per SESSION-LOG.md unique session count; latest is session 568).
...
| MAINT-128 | MAINT | God-component decomposition batch — 3 of 9 components remaining after sessions 566-568. Remaining: `BlockTree.tsx` (899L → ~500), extract `useBlockLinkResolve` / `useBlockPropertiesBatch` / `useBlockNavigateToLink`; `PropertyRowEditor.tsx` (539L — dispatch by `def.value_type`; deletes the `biome-ignore lint/complexity/noExcessiveCognitiveComplexity` at L85; **rejected in session 563 inspection as design-heavy** — 5 typed editors share `localValue`, date hook state, select-options state (3 fields), ref-picker state (4 fields), 10+ callbacks; splitting would re-create the prop-chain problem the biome-ignore acknowledges); `backlink-filter/AddFilterRow.tsx` (556L, child 2.5× its parent — 19 useState + 11 build functions; per-category split is mostly mechanical but state slots need to move with each category JSX). Do NOT try to land in one PR — stagger per file. **Stretch-target gaps for partially-closed components:** SettingsView gap to ≤150L closes via `useSettingsTab` hook for localStorage+URL persistence (~50L drop); SortableBlock gap to ≤250L closes via JSX extraction `<SortableBlockBody>`; HistoryView gap to ≤250L closes via `useHistoryEntries` data-loading hook (~30L drop); ConflictList gap to ≤450L closes via 3 additional extractions (`useConflictDeviceNames`, `useConflictActions`, `<ConflictFilterBar>`); TrashView is at the firm ≤450L target (≤350L stretch deferred — would require collapsing 6 IPC handlers into a single-use hook); PageBrowser gap to ≤300L is irreducible orchestrator glue (data loading + handleCreatePage + alias resolution + load-more + memos = ~127L). All deferred. | L |
| MAINT-167 | MAINT | **Unit tests for 3 TrashView beyond-spec hooks** (flagged by reviewer during MAINT-128 TrashView decomposition in session 568). The session 568 subagent extracted 3 cohesive hooks beyond MAINT-128's explicit description targets to hit the ≤450L line-count goal: `useTrashListShortcuts.ts` (118L — UX-275 keyboard shortcuts: Shift+R restore, Shift+Delete purge, Space toggle, Ctrl+A select-all, Escape clear), `useTrashBreadcrumbs.ts` (63L — parent-page resolution via `batchResolve` with cancellation), `useTrashDescendantCounts.ts` (39L — UX-243 cascade-count badges via `trashDescendantCounts` IPC). Each hook has single-responsibility scope and is currently covered by 76 existing TrashView integration tests but lacks dedicated unit tests. **Recommended:** Add `src/hooks/__tests__/{useTrashListShortcuts,useTrashBreadcrumbs,useTrashDescendantCounts}.test.ts(x)` with ~5 tests per hook (15 total) covering keyboard shortcut branches, IPC + cancellation, and error paths. Cost: S. Risk: Low. Impact: Low (defensive — integration tests already cover the behavior, but unit tests catch regressions faster). | S |
