...
| MAINT-162 | MAINT | ARIA role re-evaluation for list views — 2 of 6 components remaining after the grid-flip batch closed HistoryView, ConflictList, TrashView, and TagFilterPanel (9 `nested-interactive` suppressions removed). Remaining: (a) `PageBrowser` (FEAT-14 mixed-mode `role="listbox"` with both flat `role="option"` rows AND `PageTreeItem` namespace-branch buttons; 4 nested-interactive + 1 aria-required-children at `PageBrowser.test.tsx:1880, 1908, 2183, 2187, 2214`); (b) `StaticBlock` (7 nested-interactive overrides; not a true list-row, the outer `role="button"` is the underlying smell). Pattern established: `<div role="grid">` + `<div role="row">` + `<div role="gridcell">` wrappers with `useSemanticElements`/`useFocusableInteractive` biome-ignores; ARIA-Components-style `<table>`/`<tr>`/`<td>` elements rejected because these are non-tabular interactive grids per WAI-ARIA APG. | M |
...
**What:** 2 of 6 components remaining after the parallel grid-flip batch closed HistoryView, ConflictList, TrashView, and TagFilterPanel. Both remaining components have non-trivial structural concerns:

- **`PageBrowser` (FEAT-14 mixed-mode):** `role="listbox"` mixes flat-page `role="option"` rows with `PageTreeItem` namespace-branch button rows inside the same viewport. `aria-required-children: { enabled: false }` at `PageBrowser.test.tsx:2187` is a hard ARIA contract violation. The grid flip is straightforward in principle but PageBrowser's tree-button rows need to nest within `role="row"` correctly (probably as nested grids or as `role="rowgroup"` + nested `role="row"`); 4 `nested-interactive` overrides at `PageBrowser.test.tsx:1880, 1908, 2183, 2214` plus the 1 `aria-required-children` go away with this flip.
- **`StaticBlock` (block rows in BlockTree):** 7 `nested-interactive` overrides at `StaticBlock.test.tsx:360, 440, 1447, 1529, 1574, 1608, 1686`. The MAINT-162 description recommended *"reconsider — `<li>` + plain children may be enough; `role="button"` outer with nested buttons is the underlying smell"*. StaticBlock isn't a list-row in the listbox/grid sense — it's the static rendering of a block (used inside SortableBlock or stand-alone in some contexts). The overrides probably go away by removing the outer `role="button"` and treating the block as a passive container with focusable inner controls (no role flip, no grid wrapper). Needs design review before flipping.

**Done in the parallel batch:** HistoryView, ConflictList (including ConflictListItem), TrashView (including TrashRowItem), TagFilterPanel — all flipped to `role="grid"` + `role="row"` + `role="gridcell"`. Removed 9 `nested-interactive` suppressions across these 4 components (1 in HistoryListItem, 2 in ConflictList, 4 in TrashView, 2 in TagFilterPanel). Pattern established: `<div role="grid">` outer + `<div role="row">` inner + `<div role="gridcell">` content/actions wrappers, with `// biome-ignore lint/a11y/useSemanticElements` and `// biome-ignore lint/a11y/useFocusableInteractive` comments. ARIA-Components matching `<table>`/`<tr>`/`<td>` semantic elements is rejected because these are non-tabular interactive grids per WAI-ARIA APG. Reference: `src/components/journal/MonthlyView.tsx` was the prior precedent for this pattern.

**Drift caught + fixed:** the original "minimum behavior-preserving change is just flipping the role attributes" claim was incorrect. Removing `nested-interactive` suppressions revealed `aria-required-children` violations because `role="row"` requires `role="gridcell"` children. Each row needed at least one (typically two or three) `<div role="gridcell">` wrappers.

**Cost:** M — PageBrowser is non-trivial because of the FEAT-14 mixed-mode tree branches inside the listbox. StaticBlock might end up being a no-op (just remove the suppressions) if its `role="button"` outer can be relaxed to a passive container.
**Risk:** Medium — PageBrowser's keyboard handling already handles tree-branch vs flat-page interactions explicitly, so the grid flip won't regress that. StaticBlock's tests heavily depend on the current `role="button"` semantics.
**Impact:** M — closes the 12 remaining axe-suppression sites (4 + 1 PageBrowser + 7 StaticBlock) AND fixes the FEAT-14 hard ARIA violation.
