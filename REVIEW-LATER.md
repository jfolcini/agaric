...
| MAINT-131 | MAINT | Block-surface Tauri coupling — both per-block IPC halves now batched. **Done across sessions 572 and 575:** (a) SortableBlock badge-count via `get_batch_attachment_counts` + `useBatchAttachmentCounts` provider, `useAttachmentCount` hook deleted; (b) StaticBlock full-list via `list_attachments_batch` + `useBatchAttachments` provider with `{ get, loading, invalidate }` API, `useBlockAttachments.handleAdd/handleDelete` now call `invalidate(blockId)` to keep StaticBlock's batch-derived view consistent with AttachmentList drawer mutations. Open: wrap remaining per-IPC calls in hooks (`useBlockReschedule` around setDueDate/setScheduledDate — used by `BlockListItem.tsx`, `RescheduleDropZone.tsx`, `DateChipEditor.tsx`, `BlockPropertyDrawer.tsx`; `useLinkMetadata` around fetchLinkMetadata — used by `LinkEditPopover.tsx`) to drop direct `lib/tauri` imports from those components. Prevents isolated rendering (Storybook, unit tests w/o full mocks) too. | S |
