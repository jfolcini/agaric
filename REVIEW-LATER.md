...
28 open items — 28 planned work (FEAT/MAINT/PERF/PUB). All frontend test-quality items closed. All five LOW backend cleanup batches (MAINT-148..152) closed. **All INFO/nits closed (last 5 in session 547). All UX-* items closed (last 3 in session 548). 17 backend Medium findings closed + 11 MAINT closed across sessions 549-558 (M-16, M-18, M-67, M-91 in 549; M-12, M-27, M-37, M-70, M-92 in 550; M-6 stale, M-26, M-71, M-84, M-94 in 551; M-17 partial tags+pages, M-28, MAINT-123 in 552; M-17 final projected_agenda+page_id, MAINT-114 spike-keep in 553; MAINT-137 in 554; MAINT-119, MAINT-122 in 555; MAINT-126, MAINT-132, MAINT-142 in 557; MAINT-120, MAINT-129, MAINT-139 in 558). New: MAINT-164 filed in 558 (date-clock-dependent agenda test).**

Previously resolved: 801+ items across 525 sessions (per SESSION-LOG.md unique session count; latest is session 558).
...
| MAINT-164 | MAINT | `commands/agenda.rs:309` reads `chrono::Local::now().date_naive()` directly inside `list_projected_agenda_inner`, making every test that exercises the projected-agenda path date-clock-dependent. `projected_agenda_respects_repeat_count_end_condition` (filed `#[ignore]` in session 558) is the first symptom — its hardcoded April-2026 range stops matching once today moves past the range. Fix: thread a `today: NaiveDate` parameter (or a `Clock` trait à la `gcal_push::clock::Clock`) through `list_projected_agenda_inner` so callers from production stay on real-time and tests can pin a fake today. Re-enable the ignored test once the fix lands. Audit other tests in `agenda_cmd_tests.rs` for similar drift potential (any with hardcoded 2026-04-* dates is at risk as soon as today moves past the range). | S |
