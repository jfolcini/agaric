...
23 open items — 23 planned work (FEAT/MAINT/PERF/PUB). All frontend test-quality items closed. All five LOW backend cleanup batches (MAINT-148..152) closed. **All INFO/nits closed (last 5 in session 547). All UX-* items closed (last 3 in session 548). 17 backend Medium findings closed + 23 MAINT closed (some partially) across sessions 549-578 — see SESSION-LOG.md for the full session-by-session sequence. Latest progress (sessions 572-578): MAINT-131 fully reduced to residual cleanup; MAINT-162 reduced to 1 component remaining (StaticBlock); **MAINT-125 progress: 55 of ~80 wrappers migrated (~69%)**; **MAINT-124 progress: App.tsx 1444L → 872L (–572L, ~40% reduction), 1 extraction remaining (`<ViewDispatcher>`)**.

Previously resolved: 833+ items across 545 sessions (per SESSION-LOG.md unique session count; latest is session 578).
...
| MAINT-124 | MAINT | Collapse `src/App.tsx` — 1444L baseline → 1139L (576 useAppKeyboardShortcuts) → 907L (577 AppSidebar) → **872L** (578 useAppDialogs: 5 dialog useStates + BUG_REPORT_EVENT/CLOSE_ALL_OVERLAYS_EVENT listeners → `src/hooks/useAppDialogs.ts` 101L + 11 tests, –31L from App.tsx). 1 extraction remaining: `<ViewDispatcher>` (promote `ViewRouter` function at L174-303 + `useHeaderLabel` / `useConflictCount` / `useTrashCount` helper hooks + `ViewFallback` + `PageSelectHandler` type to a new file). Estimate after ViewDispatcher: ~730-740L. Target ≤500L will likely require either folding the lazy-imports block (~45L) into the ViewDispatcher batch OR a fifth extraction (e.g., the boot recovery / priority levels / space accent useEffect clusters, ~80L combined). The non-keyboard FEAT-12 quick-capture global hotkey (Tauri OS-level `registerGlobalShortcut`) stays in App.tsx — out of scope. | S–M |
| MAINT-125 | MAINT | `src/lib/tauri.ts` hand-writes `invoke('<str>', ...)` wrappers for commands already typed by `src/lib/bindings.ts`. Migration in progress — **55 of ~80 wrappers migrated (~69%)** via `unwrap` helper across 4 batches (sessions 574, 576, 577, 578). Batch-4 (session 578, 14 history/undo-redo/block-fixed-field wrappers): `getBlockHistory`, `getConflicts`, `getStatus`, `countBacklinksBatch`, `setTodoState`, `setPriority`, `setDueDate`, `setScheduledDate`, `listPageHistory`, `revertOps`, `restorePageToOp`, `undoPageOp`, `redoPageOp`, `computeEditDiff`. ~33 wrappers still on raw `invoke`, clustered as: sync/pairing (11), attachments (5), drafts (4), diagnostics/bug-report (4), spaces/pages (4), compaction (2), link-metadata (2), markdown-import (1). All mechanical. Public surface of `tauri.ts` unchanged. Pairs with MAINT-123 (typed `HANDLERS`). | M |
