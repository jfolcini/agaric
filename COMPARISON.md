...
| Tag inheritance in queries | 2.0: inherited class properties are queryable | **Unreachable.** `block_tag_inherited` is materialized, incrementally maintained across 5 propagation paths, and has a rebuild job. The resolver does read it — `src-tauri/agaric-store/src/tag_query/resolve.rs:35,123` branches on `include_inherited` — but **no production caller ever enables it** — the `query_by_tag*` sites pass `null` (`src/components/filters/TagFilterPanel.tsx:189,193`, `src/hooks/useQueryExecution.ts:63`) and the `filtered_blocks_query` tag filters omit the key entirely (`src/hooks/useQueryExecution.ts:185`, `src/lib/agenda-filters.ts:648`); a caller passing `true` would be honoured end to end, none does, and the Rust commands default `None` to `false` (`src-tauri/src/commands/tags.rs:549,599`). Only tests pass anything else. The shared filter vocabulary has no inherited-tag path at all. Inherited tags are *displayed* on a block and nothing more | **Dead** |
...
| Discovery | Cloud | mDNS `_agaricv1._udp.local.` via `iroh-mdns-address-lookup`, plus a `MulticastLock` on Android | Done |
...
| Pairing fallback | N/A | The QR payload is v2 (#4037): the passphrase plus the responder's `device_id`, `endpoint_id` and up to two `ip:port` candidates, so the joiner builds an `EndpointAddr` and dials without multicast — a first pair survives guest WiFi, AP isolation and a blocked multicast chain. A device with no bound endpoint to advertise emits the v1 passphrase-only shape instead, and mDNS owns discovery as before; mDNS is also the staleness fallback, since a DHCP lease that turns over between rendering the QR and scanning it invalidates every candidate in it | Done |
