...
**FIRST — before picking anything new, check CI on every open PR and fix all red.** Run `gh pr list --author @me --state open` then `gh pr checks <n>` for each. **Any red is yours to fix** — even a failure *inherited from `main`* (e.g. a lint/zizmor finding from a workflow that landed on `main` and now reds every PR's `validate / lint`). "Orthogonal / not from my diff" is NOT a reason to skip it: a red check blocks the merge of work that is otherwise green, so the loop stalls. Diagnose from `gh run view --job <id> --log` (or `gh api /repos/<owner>/<repo>/actions/jobs/<id>/logs`), fix the **underlying cause** (prefer a real fix — e.g. scope an over-broad token — over a suppression; suppress only with a justifying comment), commit, and push. If the cause lives on `main`, fix it in a dedicated small PR off `origin/main` so it can merge first and clear every other PR's inherited failure. Only once the open-PR board is green (or each red has a fix pushed) do you start a new batch.
...
- **Dismissing a red check as "not my diff"** — a CI failure inherited from `main` (a workflow lint/zizmor finding, a flaky-but-now-permanent failure) reds *every* open PR's check and blocks merging otherwise-green work. It is NOT "orthogonal" or "the maintainer's problem" — fix it. Per §1, check CI on all open PRs *before* starting a new batch and clear every red, fixing the underlying cause (e.g. scope an over-broad GitHub App token to satisfy zizmor `github-app`, rather than just suppressing). If the cause is on `main`, fix it in a dedicated small PR off `origin/main` so it merges first and unblocks the rest. (Session 907–911: parked a zizmor `github-app` finding as "orthogonal"; it silently red-lit four PRs — the correct move was to scope the token and ship the fix immediately.)
...
- **Parallel subagents + `git stash`/`reset`/`checkout` in a SHARED working tree = catastrophe** — `git stash` is a GLOBAL op: one subagent stashing sweeps up *every other concurrent subagent's* edits into the stash, scrambling the tree (session 889: 6 parallel agents over 28 files → only 12 files left modified, the rest stranded in `stash@{2}`; recovered by reconstructing from the stash + verify-gate). Subagents don't coordinate git state. **Rules:** (a) for multi-subagent batches that edit the tree, either run in isolated `git worktree`s (one per parallel *issue/track*) or run sequentially; (b) ALWAYS add "do NOT run any git command (stash/reset/checkout/add/commit); only edit files" to every parallel build-subagent prompt; (c) you CAN run two different *issues* truly in parallel — e.g. a frontend track in the main tree + a Rust track in a worktree (`git worktree add ../wt-x -b branch origin/main`) — since they touch disjoint files and separate `target/`. On scramble, don't trust stash provenance: reconstruct and let the full gate (oxlint + tsc + full test suite) be the arbiter.
- **`oxfmt --write` detaches `oxlint-disable-next-line` comments → re-run oxlint AFTER formatting** — formatting reflows JSX (e.g. splits a multi-attribute tag so each attribute lands on its own line), which moves the line a `-next-line` directive was meant to cover. Some oxlint rules anchor the diagnostic NOT at the element's opening-tag line but at the offending attribute's own line — notably **`jsx-a11y/prefer-tag-over-role` anchors at the `role=` line**. So a `// oxlint-disable-next-line … prefer-tag-over-role` placed above `<div` covers `<div` but misses the `role=` line one row below (it still suppresses sibling rules like `interactive-supports-focus` that DO anchor at the tag line). And `//` comments are not valid *between* JSX attributes. **Fix:** for sub-line-anchored rules, wrap the element in a block pair — `/* oxlint-disable jsx-a11y/prefer-tag-over-role -- reason */` … element … `/* oxlint-enable jsx-a11y/prefer-tag-over-role */` (plain `/* */` block comments, NOT `{/* */}` which is invalid in a parenthesized-expression/`return (` position). Block disables survive oxfmt reflow. ALWAYS run `npx oxlint` again after `oxfmt --write` on a batch and before committing — the pre-commit hook's `oxfmt --check` + the error-level rule will otherwise fail.
- **Semantic-tag / role swaps break tests that query the literal attribute** — swapping `<div role="status">` → `<output>` (or `role="region"`→`<section>`, etc.) changes the *implicit* role but removes the literal `role="…"` attribute. Tests using `container.querySelector('[role="status"]')` or `toHaveAttribute('role', 'status')` then fail (an `<output>`'s implicit role does not satisfy an attribute selector). Before any such swap, grep the matching test for `[role=` / `getByRole` / `toHaveAttribute('role'` — if the literal attribute is asserted, keep the role and disable-with-reason instead of swapping. This is exactly why a11y burndowns must run the FULL test suite (not just touched-file tests) as the gate.
- **Verify Rust batches with `cargo check --all-targets`, not just `--tests` — benchmarks break silently** — `cargo check --tests` / `cargo nextest run` do NOT compile `benches/*.rs`. A cross-cutting signature/type change (e.g. an IPC param `String`→`BlockId` newtype migration, session 896) leaves benchmark call sites passing the old type; the lib+tests build clean and a subagent reports "green", but `cargo check --all-targets` (or the CI bench build) fails later. After any such change the orchestrator must run `cd src-tauri && cargo check --all-targets` before committing and fix bench call sites (usually a trailing `.into()`). Related: a query-macro change (incl. an `as "col!: NewType"` override) needs `.sqlx` regen via a temp DB; a type-only `FromRow` *field* change does not.
- **The shared `src-tauri/dev.db` must match the CURRENT branch's migrations, or the pre-push clippy fails on a phantom type mismatch** — the pre-commit/pre-push `cargo clippy` runs sqlx in *online* mode against `DATABASE_URL` (`sqlite:dev.db`), so `query!`/`query_as!` macros are type-checked against the *live dev.db schema*, NOT the committed `.sqlx` offline cache. With branches whose migrations diverge (e.g. a schema batch adds `0075` flipping a column `TEXT`→`INTEGER`), `dev.db` ends up at whatever schema the *last* `sqlx migrate run` left it, and pushing a *different* branch then fails with a backwards trait error like `Option<String>: From<Option<i64>>` (session 901). `SQLX_OFFLINE=true cargo check` passes (uses the per-branch `.sqlx`), masking it until push. **Fix when switching between migration-divergent branches:** recreate dev.db from the branch you're about to push — `cd src-tauri && set -a && . ./.env && set +a && sqlx database drop -y && sqlx database create && sqlx migrate run --source migrations`. Don't reach for `SKIP_CI_VERIFY=1`.
- **Before migrating a timestamp/enum column, grep for cross-table comparisons — coupled columns must move together** — #109 Phase 2 (TEXT→INTEGER-ms timestamps) looked like ~10 independent per-table migrations, but `op_log.created_at`, `block_drafts.updated_at`, and `attachments.created_at` form a *coupled cluster*: recovery runs `… AND created_at > ?` binding `draft.updated_at` (a cross-table SQL comparison), and the materializer sets `attachments.created_at` *from* the op's `created_at`. Migrate only one side and the comparison silently breaks — SQLite ranks by storage class (INTEGER always < TEXT) regardless of value, and under `STRICT` a leftover-typed bind hard-fails. Self-generated + self-compared columns (`link_metadata.fetched_at`, `peer_refs.*`, `materializer_*`) are safe standalone. So before picking a "table N", grep the column name for `> ?` / `< ?` SQL predicates against *other* tables and for values flowing from one timestamp into another; migrate any coupled set in a single PR. (Backfill recipe that preserves ms precision: `CAST(ROUND((julianday(col) - 2440587.5) * 86400000.0) AS INTEGER)`, NULL-guarded for nullable columns.)
