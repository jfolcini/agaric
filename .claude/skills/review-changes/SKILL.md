...
Report blocking defects first (a concrete failure), then what the ladder would have skipped: helpers, options, abstractions, paragraphs the change did not need, and comments that narrate what a name could say. Both follow `AGENTS.md` § How we work: no speculative hardening, no tests for unreachable cases. Use `detail_level="minimal"`.
