...
// device-local preference is declared once (key, type, default,
// version, scope) with `getPref`/`setPref`/`getKeyedPref`/`setKeyedPref`
// accessors that never throw. A raw `localStorage.getItem(...)` /
// `.setItem(...)` bypass sidesteps that contract entirely — a typo'd
// key, an unvalidated shape, or a forgotten try/catch compiles clean and
// only fails at runtime (or silently corrupts a stored value). This
// guard forbids the raw pattern in NEW app code so the registry stays
// the single place a preference's storage key is declared.
//
// ─── Detection ──────────────────────────────────────────────────────
//
...
//     migrated in that pass (see `preferences.ts`'s file header,
//     "What's migrated vs. cataloged", for the per-file rationale —
//     mostly `useSyncExternalStore` + synthetic-`StorageEvent`-dispatch
//     hooks and modules with their own caching/migration-fallback
//     semantics that a mechanical migration would risk breaking).
//     Removing a file from this list as it migrates is expected and
//     welcome; ADDING one back requires the same "why not the registry"
//     justification as the original grandfather entry.
//
...
const EXEMPT_FILES = Object.freeze(
  new Set([
    ...
    // registry; several PREFS entries are read through it (see
    // DeadlineWarningSection.tsx, useAgendaPreferences.ts, etc.).
    'src/hooks/useLocalStoragePreference.ts',
    // useSyncExternalStore + synthetic same-tab StorageEvent dispatch
    // carrying the exact raw old/new string — getPref/setPref only
    // expose the typed value, not the raw string the dispatch needs.
    ...
    // getPref's default-collapsing would erase.
...
function runGuard() {
  ...
  if (violations.length > 0) {
    ...
    console.error('')
    console.error('Declare the preference in src/lib/preferences.ts (PREFS / PREF_CATALOG) and')
    console.error('use the typed accessors instead of a raw localStorage call:')
    console.error('')
    console.error("    import { getPref, PREFS, setPref } from '@/lib/preferences'")
    console.error('    const value = getPref(PREFS.myPreference)')
    console.error('    setPref(PREFS.myPreference, next)')
    console.error('')
    console.error("If a raw call is genuinely required (see preferences.ts's file header,")
    console.error('"What\'s migrated vs. cataloged", for the standing exemption categories),')
    console.error('add the file to EXEMPT_FILES in scripts/check-raw-local-storage.mjs with a')
