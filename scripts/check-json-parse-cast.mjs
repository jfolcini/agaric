...
// enforces the shape of values the program itself constructs — it cannot,
// and does not, re-check a value that came back out of storage. `JSON.parse(
// raw) as T` (or `(JSON.parse(raw) as unknown) !== false`, or any other
// spelling that immediately casts the call's result) type-ASSERTS the
// shape instead of checking it: a stale entry from an older schema, a
// hand-edited devtools value, or a future migration produces a
// well-formed-looking JS value of the WRONG shape, and the cast waves it
// through to every consumer typed as a valid `T`.
//
...
//      via a bracket-depth scan (handles nested calls/objects/strings), and
//      the text immediately after that close-paren is checked for a leading
//      `as` (word-boundary, so `assert`/`async` do not false-match).
//
 ...
 * `{ hits, scanError }` where `hits` is `[{ line }]` (1-based) and
...
export function analyzeSource(rawSrc) {
  try {
    ...
    while ((m = CALL_RE.exec(search))) {
      ...
      if (closeParenIdx === -1) continue // unbalanced call — not this guard's concern
 ...
 * scanErrors }`, where `details` maps an offender's repo-relative path to
 * its hit lines (message only, not baseline schema), `newOffenders` is
 * every file whose LIVE count exceeds its baselined count (0 if absent —
 * this also catches an ALREADY-baselined file that gained an additional,
 * brand-new cast, not just a file appearing for the first time), and
 * `staleBaseline` is every baseline entry whose live count is LOWER than
 * recorded (including a fully-fixed file at live count 0). `scanErrors` is
...
export function analyze({ root, srcDir, baseline }) {
  ...
  const staleBaseline = Object.keys(baselineCounts)
    .filter((f) => (details.get(f)?.length ?? 0) < baselineCounts[f])
    .toSorted()
  return { offenders, details, newOffenders, staleBaseline, scanned, scanErrors }
}
...
function runGuard() {
  ...
  const { offenders, details, newOffenders, staleBaseline, scanErrors } = analyze({
    root: ROOT,
    srcDir: SRC_DIR,
    baseline,
  })
  ...
  console.log(
    `OK: ${offenders.length} baselined JSON.parse(...)-as-cast offender(s), no new occurrences, no stale entries`,
  )
}
...
// #3881 boolean-idiom shape), a parenthesized-wrapper cast is still caught,
// a comment does not count, a string/template does not count, an unbalanced/
// unlexable file is reported as a scanError (not silently skipped), the
