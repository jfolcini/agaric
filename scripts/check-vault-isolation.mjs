...
//        node scripts/check-vault-isolation.mjs --self-test
...
// ─── self-test ───────────────────────────────────────────────────────
//
// Proves the guard is CAPABLE of failing. A guard that only ever passes
// is indistinguishable from a guard that was accidentally defanged.
...
function selfTest() {
  ...
  // Rust seam.
  check(
    'a direct resolve outside the seam is a violation',
    findDirectResolves('src-tauri/src/commands/x.rs', 'let d = app.path().app_data_dir()?;')
      .length === 1,
  )
  check(
    'a rustfmt-split chain is a violation',
    findDirectResolves('src-tauri/src/commands/x.rs', 'app\n  .path()\n  .app_data_dir()\n')
      .length === 1,
  )
  check(
  ...
  check(
    'a doc comment is not a violation',
    findDirectResolves('src-tauri/src/lib.rs', '/// resolved via `app.path().app_data_dir()`')
      .length === 0,
  )
  check(
    'a block comment is not a violation',
    findDirectResolves('src-tauri/src/lib.rs', '/* app.path().app_data_dir() */').length === 0,
  )
  check(
    'a different receiver is not a violation',
    findDirectResolves('src-tauri/src/recovery/boot.rs', 'materializer.app_data_dir()').length ===
      0,
  )

  // wdio config. The real file must satisfy every requirement, and
  // deleting any single one must be detected.
  const real = fs.readFileSync(path.join(ROOT, WDIO_CONF), 'utf8')
  check('the checked-in wdio.conf.ts passes', checkWdioConf(real).length === 0)
  check(
    'an empty config fails every requirement',
    checkWdioConf('').length === WDIO_REQUIREMENTS.length,
  )
  check(
    'dropping the env from the driver spawn is detected',
    checkWdioConf(real.replace(/\benv\s*:\s*sandboxEnv,?/, '')).some(
      (r) => r.id === 'env-reaches-the-app',
    ),
  )
  check(
    'dropping the runtime verification is detected',
    checkWdioConf(real.replace(/notes\.db/g, 'x.db')).some((r) => r.id === 'runtime-verification'),
  )
  check(
    'dropping the sandbox flag is detected',
    checkWdioConf(real.replace(/AGARIC_E2E_SANDBOX/g, 'X')).some((r) => r.id === 'sandbox-flag'),
  )

  if (failures.length > 0) {
    console.error('check-vault-isolation --self-test FAILED:')
    for (const f of failures) console.error(`  - ${f}`)
    return 2
  }
  console.log('check-vault-isolation --self-test: ok')
  return 0
}
