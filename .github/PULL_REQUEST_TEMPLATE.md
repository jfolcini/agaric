...
- [ ] If a `.sql` migration or an inline `sqlx::query!` was added or changed: **`just gen-sqlx`** re-run and **all four** regenerated `.sqlx/` caches (root + `agaric-store`/`agaric-engine`/`agaric-sync`) committed in this PR — never the bare `cargo sqlx prepare`, which silently prunes the member crates' entries (AGENTS.md § Key Architectural Invariants #6)
