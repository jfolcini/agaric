...
- [ ] If a `.sql` migration or an inline `sqlx::query!` was added or changed: **`just gen-sqlx`** re-run and **all four** regenerated `.sqlx/` caches (root + `agaric-store`/`agaric-engine`/`agaric-sync`) committed in this PR — never the bare `cargo sqlx prepare`, which prunes root-cache entries covering the members' test-only queries *and* leaves the three member caches stale (AGENTS.md § Key Architectural Invariants #6)
