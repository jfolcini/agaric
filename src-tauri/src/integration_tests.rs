...
async fn create_edit_delete_restore_produces_sequential_ops_with_valid_hashes() {
    ...
    let block = get_block_inner(&pool, block_id.into()).await.unwrap();
    assert!(
...
async fn mixed_operations_produce_consistent_op_log_and_block_state() {
    ...
    let fetched_b0 = get_block_inner(&pool, b0.id.clone()).await.unwrap();
    assert_eq!(
        fetched_b0.content,
        Some("edited 0".into()),
    ...
    let fetched_b2 = get_block_inner(&pool, b2.id.clone()).await.unwrap();
...
async fn purge_removes_block_tags_properties_and_attachments() {
    ...
    delete_block_inner(&pool, DEV, &mat, bid.clone().into())
        .await
        .unwrap();
    settle_bg_tasks(&mat).await;

    // Purge
    let purge = purge_block_inner(&pool, DEV, &mat, bid.clone().into())
        .await
        .unwrap();
...
async fn materializer_processes_background_tasks_after_page_create() {
    ...
    // 5 background tasks via `dispatch::enqueue_background_tasks`:
    ...
    // which the worker also counts as a processed task — total 6.
    assert_eq!(
        bg, 6,
        "page creation dispatches exactly 5 bg tasks + 1 flush barrier, got {bg}"
    );
}
