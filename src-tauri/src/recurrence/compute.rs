...
pub(crate) async fn handle_recurrence_in_tx(
    ...
    // transaction (a `CommandTx` derefs to `sqlx::Transaction`) to the
    // neutral core, then enqueues the returned op records on the tx so the
    // caller still drives commit + background dispatch exactly as before.
    // `tx` is `&mut CommandTx`, which `DerefMut`s to `sqlx::Transaction`; the
    // deref coercion happens at the call site so we pass `tx` directly rather
    // than an explicit `&mut **tx` (clippy::explicit_auto_deref).
    let (created, ops) =
        agaric_engine::recurrence::build_recurrence_sibling_in_tx(tx, state, device_id, block_id)
            .await?;
    for op in ops {
