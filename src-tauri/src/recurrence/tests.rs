...
// The pure `shift_date` / `shift_date_once` / `days_in_month` date-math unit
// tests moved down with the recurrence core into
// `agaric_engine::recurrence` (#2621 THE INVERSION). The `++`-arm explicit
// failure-mode tests below still exercise the re-exported `shift_date`, and the
// `project_block_dates` projection + T-36 integration suites remain here because
// they drive the app-layer command / materializer plumbing.
