...
async fn cleanup_orphaned_attachments_uses_read_pool() {
    ...
    crate::materializer::cleanup_orphaned_attachments(&write_pool, Some(&read_pool), dir.path())
        .await
        .unwrap();
...
async fn cleanup_orphaned_attachments_unlink_error_is_non_fatal() {
    ...
    let result = crate::materializer::cleanup_orphaned_attachments(&pool, None, dir.path()).await;
...
async fn cleanup_orphaned_attachments_restores_bytes_referenced_inside_the_unlink_window_3519() {
    ...
    *crate::materializer::GC_RACE_RENDEZVOUS.lock().unwrap() = Some(hook_tx);

    let gc = tokio::spawn({
        let pool = pool.clone();
        let root = dir.path().to_path_buf();
        async move { crate::materializer::cleanup_orphaned_attachments(&pool, None, &root).await }
    });
    ...
    *crate::materializer::GC_RACE_RENDEZVOUS.lock().unwrap() = None;
