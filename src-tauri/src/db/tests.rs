...
async fn reserved_key_set_matches_db_check_constraint_589() {
    ...
    let check_start = ddl
        .find("key_not_reserved")
        .expect("block_properties DDL must contain the key_not_reserved CHECK (migration 0088)");
...
async fn blocks_rebuild_self_fk_allows_drop() {
    ...
    let sql: (String,) =
        sqlx::query_as("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'blocks'")
            .fetch_one(&pool)
            .await
            .unwrap();
    assert!(
...
async fn text_deleted_at_wedges_0085_rerun_integer_ms_passes_618() {
    ...
    sqlx::query("INSERT INTO blocks (id, content, deleted_at) VALUES ('B618', 'x', 1600000000000)")
        .execute(&pool)
        .await
        .unwrap();
...
async fn init_pool_recovery_skips_dangling_fk_refs_73() {
    ...
    let ghost_prop: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM block_properties WHERE block_id = 'GHOST_BLOCK'")
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn attachment_blobs_backfill_dedups_pre_0094_rows_1993() {
    ...
    let pa: String = sqlx::query_scalar("SELECT fs_path FROM attachments WHERE id = 'ATT1993A'")
        .fetch_one(&pool)
        .await
        .unwrap();
    let pb: String = sqlx::query_scalar("SELECT fs_path FROM attachments WHERE id = 'ATT1993B'")
        .fetch_one(&pool)
        .await
        .unwrap();
    ...
    let pc: String = sqlx::query_scalar("SELECT fs_path FROM attachments WHERE id = 'ATT1993C'")
        .fetch_one(&pool)
        .await
        .unwrap();
...
async fn spaces_0089_trigger_registers_and_fk_rejects_non_spaces_708() {
    ...
    .await
    .unwrap();
    let registered: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM spaces WHERE id = 'SREG708'")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(
...
async fn blocks_deleted_at_ms_backfill_handles_null_and_value_376() {
    ...
    sqlx::query("INSERT INTO blocks (id, block_type, content) VALUES ('LIVE376', 'content', 'x')")
        .execute(&pool)
        .await
        .unwrap();
    ...
    let expected = DateTime::parse_from_rfc3339("2025-08-15T12:00:00.123Z")
        .unwrap()
        .timestamp_millis();
    let del: Option<i64> = sqlx::query_scalar("SELECT deleted_at FROM blocks WHERE id = 'DEL376'")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(
