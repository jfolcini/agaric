...
pub struct Materializer {
    ...
    /// the app's `run` setup (also registered as Tauri managed state);
    ...
    /// #4502 — the runtime [`Self::spawn_task`] binds to when the caller is
    /// not inside one. Tauri's `setup` callback runs outside any runtime, so
    /// the app passes its `async_runtime` handle; the test constructors take
    /// the current one.
...
impl Materializer {
    ...
    ///
    /// `loro` is the engine state constructed at the top of the app's `run`
    ...
    fn build(
        ...
        Self::spawn_task(
            &mat.tasks,
            &mat.runtime,
            Self::inbound_rebuild_debounce_loop(mat.clone()),
        );
    ...
    /// runtime. To keep the abort-on-shutdown semantics while not
    /// panicking in `setup`, we bind the task explicitly via
    /// `JoinSet::spawn_on` to either:
    ///   - the current Tokio runtime when one exists (tests under
    ///     `#[tokio::test]`, async command handlers, async tasks); or
    ///   - `runtime`, the handle the constructor was given, as a fallback
    ///     (the non-async `setup` callback path, where the app passes its
    ///     `async_runtime` handle; #4502).
    ///
    ...
    async fn cached_op_log_count(&self) -> Option<i64> {
        let now = u64::try_from(agaric_store::db::now_ms()).unwrap_or(0);
        let last_at = self
            .metrics
