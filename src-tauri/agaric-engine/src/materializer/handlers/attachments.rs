...
// `crate::apply::attachments`; `apply_op_tx` (also moved) calls them
...
#[cfg(any(test, feature = "test-util"))]
pub static GC_RACE_RENDEZVOUS: std::sync::Mutex<Option<GcRaceRendezvous>> =
...
pub async fn cleanup_orphaned_attachments(
    ...
    let Some(referenced_paths) = load_referenced_paths(lookup_pool).await else {
        return Ok(());
    };

    let (files, exempted_temps) = walk_attachment_files(&attachments_root).await;
    ...
    let pruned_blobs = prune_unreferenced_blob_rows(pool).await;
    ...
    log_cleanup_summary(&counters);

    Ok(())
}

/// Collects every regular file under `attachments_root` as an unlink
/// candidate, returning them with the count of in-flight transfer temps the
/// walk exempted (#3325).
async fn walk_attachment_files(attachments_root: &Path) -> (Vec<PathBuf>, u64) {
    ...
    let mut dir_stack: Vec<PathBuf> = vec![attachments_root.to_path_buf()];
    ...
    (files, exempted_temps)
}

/// Loads every `attachments.fs_path` as the referenced-path set. `None` means
/// the load failed and the caller must abort the pass.
async fn load_referenced_paths(
    lookup_pool: &SqlitePool,
) -> Option<std::collections::HashSet<String>> {
    match sqlx::query_scalar!(r#"SELECT fs_path as "fs_path!: String" FROM attachments"#)
...
/// Deletes every `attachment_blobs` row whose `on_disk_path` no longer has an
/// `attachments` row, returning the number of rows removed.
async fn prune_unreferenced_blob_rows(pool: &SqlitePool) -> u64 {
...
/// Per-pass tallies for [`cleanup_orphaned_attachments`]. A struct rather than
/// seven same-typed `u64` locals threaded through the helpers below, where a
/// positional swap would compile and silently mis-report the pass.
#[derive(Default)]
...
/// Emits the one-line per-pass summary.
fn log_cleanup_summary(counters: &CleanupCounters) {
    let &CleanupCounters {
        scanned,
        unlinked,
        errors,
...
/// Walks the candidate list in `CLEANUP_BATCH_SIZE` chunks, yielding the
/// runtime between chunks.
async fn sweep_walked_attachments(
    pool: &SqlitePool,
    app_data_dir: &Path,
    files: &[PathBuf],
    referenced_paths: &std::collections::HashSet<String>,
    undoable_deleted: &std::collections::HashSet<String>,
    counters: &mut CleanupCounters,
) {
    for chunk in files.chunks(CLEANUP_BATCH_SIZE) {
        for full_path in chunk {
            counters.scanned += 1;
            process_walked_attachment(
                pool,
                full_path,
                app_data_dir,
                referenced_paths,
...
/// Evaluates one walked file and, if it is a confirmed orphan, destroys it.
async fn process_walked_attachment(
    pool: &SqlitePool,
    full_path: &Path,
    app_data_dir: &Path,
    referenced_paths: &std::collections::HashSet<String>,
    ...
    if !prune_blob_row_for_path(pool, &relative_str, full_path, counters).await {
        return;
    }

    if !write_pool_confirms_orphan(pool, &relative_str, full_path, counters).await {
        return;
    }
    ...
    let Some(quarantine) = quarantine_orphan(full_path, counters).await else {
        return;
    };

    if !confirm_orphan_after_quarantine(pool, &relative_str, full_path, counters).await {
        restore_quarantined_bytes(&quarantine, full_path, counters).await;
        return;
    }

    remove_quarantined_orphan(&quarantine, full_path, counters).await;
}

/// Deletes the `attachment_blobs` mapping for one unlink candidate, guarded
/// against a concurrent add. Returns `false` when the delete failed, in which
/// case the file must be left alone this pass.
async fn prune_blob_row_for_path(
    pool: &SqlitePool,
    relative_str: &str,
    full_path: &Path,
    counters: &mut CleanupCounters,
    ...
    .await
    {
        Ok(r) => counters.pruned_blobs += r.rows_affected(),
        Err(e) => {
...
/// Re-asks the write pool whether anything references this path. `false`
/// means a reference exists or the check was indeterminate — either way the
/// file must not be unlinked.
async fn write_pool_confirms_orphan(
    pool: &SqlitePool,
    relative_str: &str,
    full_path: &Path,
    counters: &mut CleanupCounters,
...
/// Renames an orphan candidate aside under the GC quarantine name. `None`
/// means the bytes could not be sheltered and must be left where they are.
async fn quarantine_orphan(full_path: &Path, counters: &mut CleanupCounters) -> Option<PathBuf> {
...
/// Final orphan confirmation, taken once the bytes are already sheltered.
async fn confirm_orphan_after_quarantine(
    pool: &SqlitePool,
    relative_str: &str,
    full_path: &Path,
    counters: &mut CleanupCounters,
...
/// Hands quarantined bytes back to their original name after a reference
/// appeared during the orphan decision (#3519).
async fn restore_quarantined_bytes(
    quarantine: &Path,
    full_path: &Path,
    counters: &mut CleanupCounters,
) {
    match tokio::fs::rename(quarantine, full_path).await {
        Ok(()) => counters.restored += 1,
        Err(e) => {
...
/// Destroys the quarantined bytes of a confirmed orphan.
async fn remove_quarantined_orphan(
    quarantine: &Path,
    full_path: &Path,
    counters: &mut CleanupCounters,
) {
