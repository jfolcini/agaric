...
// #4250 test-only: the undo-retention window the GC honours, so a test can age
// a `delete_attachment` op past the exact bound production uses rather than
// restating it as a literal. Production reads it inside `attachments`.
...
#[cfg(any(test, feature = "test-util"))]
pub use attachments::GC_RACE_RENDEZVOUS;
...
pub use apply::apply_op_projected;
