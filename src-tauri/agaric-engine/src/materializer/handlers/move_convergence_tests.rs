...
//! `crate::loro::shared`'s module docs (the canonical statement,
...
async fn seed_engine_world(db_name: &str) -> (TempDir, SqlitePool, crate::loro::shared::LoroState) {
    let dir = TempDir::new().expect("tempdir");
...
/// `seed_delete_op` / `seed_move_op` fixtures in the app crate's `db::recovery` tests.
...
fn engine_deleted_at(state: &crate::loro::shared::LoroState, block_id: &str) -> Option<String> {
    let space = agaric_store::space::SpaceId::from_trusted(SPACE_ID);
    let mut guard = state
        .registry
        .for_space(&space, DEVICE_ID)
...
///   this way. [`crate::loro::projection::project_block_full_to_sql`]
...
async fn import_snapshot_into(
    dst_pool: &SqlitePool,
    src_pool: &SqlitePool,
    src_state: &crate::loro::shared::LoroState,
    reproject_order: &[&str],
) {
...
fn engine_has_block(state: &crate::loro::shared::LoroState, block_id: &str) -> bool {
    let space = agaric_store::space::SpaceId::from_trusted(SPACE_ID);
    let mut guard = state
        .registry
        .for_space(&space, DEVICE_ID)
...
/// `crate::loro::shared`'s module docs, #3983); `before` is sampled
