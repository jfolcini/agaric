...
/// their write transaction via [`agaric_store::db::begin_immediate_logged`],
...
fn apply_tx_uses_begin_immediate_not_deferred() {
    ...
    assert!(
        all_prod
            .contains("agaric_store::db::begin_immediate_logged(pool, \"materializer_apply_op\")"),
        "apply_op must open its write tx via `begin_immediate_logged` with the \
         `materializer_apply_op` label — see SQL-review .",
    );
    assert!(
        all_prod.contains(
            "agaric_store::db::begin_immediate_logged(pool, \"materializer_apply_batch\")"
        ),
