...
/// + underscores).
pub const GLOBAL_TASK_SENTINEL: &str = "__GLOBAL__";
...
pub const SEED_UNRESOLVED_LINK_LAST_ERROR: &str =
...
fn give_up_reason(row: &DueRow) -> Option<&'static str> {
    ...
    let age_ms = agaric_store::db::now_ms() - row.created_at;
...
pub(crate) async fn lease_entry(
    ...
    let next_attempt_at =
        agaric_store::db::now_ms() + backoff_delay_for(attempts, class).num_milliseconds();
    sqlx::query!(
...
pub fn spawn_sweeper(
    ...
    let _handle = materializer.runtime.clone().spawn(async move {
...
mod tests {
    ...
    async fn fetch_due_skips_future_entries() {
        let (pool, _dir) = test_pool().await;
        let future = agaric_store::db::now_ms() + 3_600_000;
        sqlx::query!(
            "INSERT INTO materializer_retry_queue \
                 (block_id, task_kind, attempts, next_attempt_at) \
             VALUES (?, ?, ?, ?)",
    ...
    async fn task_from_row_roundtrip() {
        let row = DueRow {
            ...
            attempts: 0,
            created_at: agaric_store::db::now_ms(),
            last_error: None,
        };
    ...
    async fn lease_entry_defers_row_without_touching_attempts_378() {
        let (pool, _dir) = test_pool().await;
        let past = agaric_store::db::now_ms() - 5 * 60_000;
        let created = agaric_store::db::now_ms() - 1_000;
        sqlx::query!(
            "INSERT INTO materializer_retry_queue \
    ...
    async fn sweep_once_gives_up_on_max_attempts_157_d() {
        ...
        let past = agaric_store::db::now_ms() - 5 * 60_000;
        let recent_created = agaric_store::db::now_ms();
        sqlx::query!(
            "INSERT INTO materializer_retry_queue \
                 (block_id, task_kind, attempts, created_at, next_attempt_at) \
             VALUES (?, ?, ?, ?, ?)",
    ...
    async fn sweep_once_gives_up_on_age_exceeded_157_d() {
        ...
        let past = agaric_store::db::now_ms() - 5 * 60_000;
        let stale_created = agaric_store::db::now_ms() - (GIVE_UP_AGE_DAYS + 1) * 86_400_000;
        sqlx::query!(
            "INSERT INTO materializer_retry_queue \
                 (block_id, task_kind, attempts, created_at, next_attempt_at) \
             VALUES (?, ?, ?, ?, ?)",
    ...
    fn give_up_reason_exempts_shed_rows_from_max_attempts_2541() {
        ...
        let stale_shed = DueRow {
            ...
            task_kind: "UpdateFtsBlock".into(),
            attempts: MAX_ATTEMPTS,
            created_at: agaric_store::db::now_ms() - (GIVE_UP_AGE_DAYS + 1) * 86_400_000,
            last_error: Some(SHED_LAST_ERROR.into()),
        };
        assert_eq!(
    ...
    async fn sweep_shed_not_counted_and_backoff_not_rewound_2541() {
        ...
        let hostage = agaric_store::db::begin_immediate_logged(&pool, "test_2541_hostage")
            .await
            .unwrap();
        ...
        let min_expected = agaric_store::db::now_ms() + 90_000;
        let n = sweep_once(&pool, &pool, &mat).await.unwrap();
        assert_eq!(
            n, 0,
    ...
    async fn sweep_once_below_thresholds_still_reenqueues_157_d() {
        ...
        let past = agaric_store::db::now_ms() - 5 * 60_000;
        let recent_created = agaric_store::db::now_ms() - (GIVE_UP_AGE_DAYS - 1) * 86_400_000;
        sqlx::query!(
            "INSERT INTO materializer_retry_queue \
                 (block_id, task_kind, attempts, created_at, next_attempt_at) \
             VALUES (?, ?, ?, ?, ?)",
    ...
    async fn sweep_retires_apply_op_superseded_by_purge_621() {
        ...
        let t0 = agaric_store::db::now_ms() - 60_000;
    ...
    async fn sweep_reenqueues_slot_op_that_wins_canonical_order_on_seq_4402() {
        ...
        let t = agaric_store::db::now_ms() - 60_000;
