...
pub struct LoroState {
    ...
    // #2896 — boot-replay reprojection suppression is NO LONGER ambient state
    // on `LoroState`. The retired boot-replay suppression flag (an `AtomicBool`)
    // and its companion dirty-parent set (a `Mutex<HashSet<…>>`) — plus the
    // load-bearing quiescence invariant they relied on — were replaced by an EXPLICIT
    // `agaric_engine::apply::kernel::ApplyMode` threaded through `apply_op_tx`:
    // the boot-replay driver owns an `ApplyMode::ReplaySuppressed(ReplayDirtyParents)`
    // sink and passes it down per op, while every other applier passes
    // `ApplyMode::Normal`. A concurrent non-replay applier therefore cannot
    // observe replay's mode by construction — no global, no invariant to police.
