...
pub async fn recompute_pages_cache_counts_for_pages(
...
/// depth-bounded).
pub async fn resolve_owning_page(
    conn: &mut sqlx::SqliteConnection,
    block_id: &str,
...
pub async fn pre_diff_target_pages(
...
pub async fn refresh_inbound_counts_after_reindex(
    pool: &sqlx::SqlitePool,
    block_id: &str,
    pre: &[String],
) -> Result<(), AppError> {
