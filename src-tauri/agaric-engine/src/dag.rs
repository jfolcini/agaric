...
/// never run unbounded.
pub const MAX_LCA_STEPS: usize = 10_000;
...
// ---------------------------------------------------------------------------
//
// NOTE (#2621 inversion): the `#[cfg(test)]` CTE-oracle reference
// implementations (`extract_prev_edit`, `fetch_prev_edit_oracle`,
// `walk_edit_chain_oracle`, `find_lca_oracle`) live app-side in
// `crate::dag` alongside the tests that exercise them, since a `#[cfg(test)]`
// item in this crate is not visible to the app crate's test build. They call
// into the `pub` production surface here (`walk_edit_chain`, `WalkOutcome`,
// `MAX_LCA_STEPS`, `parse_parent_seqs_canonical`).
...
pub fn parse_parent_seqs_canonical(parent_seqs_json: &str) -> Result<Vec<(String, i64)>, AppError> {
...
/// #2481 phase 1 — ingest an already-decoded replicated op record as
/// append-only, hash-verified **audit metadata**.
///
/// This is the engine-side core of audit-only op-log replication
/// (`SyncMessage::OpLogBatch`). The app-level `crate::dag::insert_replicated_op`
/// shim decodes the wire `OpTransfer` (extracting the transfer-carried
/// `origin`, which `OpRecord` does not surface) and calls into here. It reuses
...
/// * the caller-supplied `origin` attribution is preserved verbatim.
///
...
pub async fn ingest_replicated_record(
    pool: &SqlitePool,
    record: &OpRecord,
    origin: &str,
...
async fn ingest_remote_record(
    ...
    // payload through `agaric_store::op::validate_set_property` (key format,
...
/// `snapshot::compact_op_log`, the chain walk will encounter
