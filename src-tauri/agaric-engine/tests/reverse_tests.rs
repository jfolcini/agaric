...
///
/// Goes through the real ingest core (`dag::ingest_replicated_record`, the
/// function `agaric_sync::sync_protocol::insert_replicated_op` wraps; this
/// crate cannot name the sync crate) so the denormalized `block_id` column is
/// populated and the `is_replicated = 1` stamp is set exactly as a synced
/// audit row would be. Authored by a distinct
...
async fn append_replicated_op(
    ...
    let record = agaric_store::op_log::OpRecord {
    ...
    agaric_engine::dag::ingest_replicated_record(pool, &record, "agent:codex")
        .await
        .expect("replicated audit op must ingest");
    agaric_store::op_log::get_op_by_seq(&agaric_store::db::ReadPool(pool.clone()), device_id, seq)
        .await
