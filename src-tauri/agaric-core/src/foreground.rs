...
//! dependencies): the shared [`LifecycleHooks`] flag/wake pair and the pure
//! [`derive_app_state`] discrimination logic are consumed by lower layers
//! (the sync daemon, the materializer) as well as the app shell.
