...
//! family of commands. They originated in the app's command layer, then moved
//! into a neutral `domain` layer, and now live in `agaric-store` (#2621, wave
//! S4d) alongside the `fts` / `filters` search cluster that consumes them. The
//! app re-exports this module (`pub use agaric_store::search_types;` behind
//! `crate::domain::search_types`) so command-internal callers and the
//! `tauri-specta` collection resolve unchanged.
//!
//! The definitions are unchanged — same fields, same derives (`serde`,
//! `specta::Type`), same serde renames — so the IPC wire format and the
//! generated TS bindings stay byte-for-byte identical.
...
// The self-contained leaf date-filter enums moved DOWN into `agaric-core`
// (#2633): `DateFilter` / `NamedDateRange` / `DateOp` derive only
// `serde` + `specta::Type` and reference no store/engine type, so they belong
// at the bottom of the DAG. The composite `SearchFilter` below stays here
// because it couples to `SpaceScope` (engine) and `LastEditedSpec`. Re-exported
// so every `crate::search_types::DateFilter` / `agaric_store::search_types::…`
...
pub use agaric_core::date_filter::{DateFilter, DateOp, NamedDateRange};
