...
async fn query_by_property_rejects_both_value_filters() {
    ...
    match err {
        agaric_core::error::AppError::Validation { message: msg, .. } => {
            assert!(
    ...
    assert!(matches!(
        err,
        agaric_core::error::AppError::Validation { .. }
    ));
}
...
// reports — they had previously diverged on NULL handling), a parity walk
// asserts each inlined keyset WHERE / ORDER BY fragment normalises to the
// single canonical shape in `pagination::mod`. Since the three copies now span
// both crates (S4b, #2621), that source-tree walk (Test B) lives in the app's
// `pagination_app_tests::position_keyset_drift` where it can reach both trees;
// only the canonical-const self-consistency check (Test A) remains here.
