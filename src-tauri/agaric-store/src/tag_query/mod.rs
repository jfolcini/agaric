...
pub use resolve::{resolve_tag_leaves, resolve_tag_prefix_leaves};
...
impl TagExpr {
    ...
    /// ```
    pub fn validate_depth(&self) -> Result<(), agaric_core::error::AppError> {
        self.check_depth(0)
    }

    fn check_depth(&self, depth: usize) -> Result<(), agaric_core::error::AppError> {
