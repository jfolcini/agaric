...
//! pool from `agaric_lib::db::init_pool`, and the coupled modules from `agaric_lib::mcp` /
//! `agaric_lib::dag`. The store-resident op_log tests keep everything that only needs
//! the migrated schema.
...
async fn test_pool() -> (SqlitePool, TempDir) {
    ...
    let pool = agaric_lib::db::init_pool(&dir.path().join("test.db"))
        .await
        .unwrap();
    (pool, dir)
}
