...
//! app-only command inner functions (`agaric_lib::commands::{
...
async fn read_page_links_sorted(pool: &SqlitePool) -> Vec<(String, String, i64)> {
    let mut rows: Vec<(String, String, i64)> = agaric_lib::commands::list_page_links_inner(
        pool,
...
async fn projected_agenda_cache_oracle_matches_on_the_fly() {
    ...
    let on_the_fly = agaric_lib::commands::list_projected_agenda_on_the_fly(
...
async fn page_link_cache_read_path_matches_legacy_query() {
    ...
    let mut new_rows = agaric_lib::commands::list_page_links_inner(
        &pool,
        &agaric_store::space::SpaceScope::Global,
        None,
    )
