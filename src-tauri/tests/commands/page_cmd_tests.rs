...
impl agaric_lib::import::ImportProgressSink for RecordingImportSink {
...
async fn import_markdown_multi_chunk_tree_matches_single_chunk() {
    use agaric_lib::commands::pages::markdown::IMPORT_CHUNK_BLOCKS;
...
async fn list_page_links_split_lazy_rebuild_on_readonly_pool_c1() {
    ...
    let links =
        agaric_lib::commands::list_page_links_inner_split(&write, &read, &SpaceScope::Global, None)
            .await
...
async fn list_page_links_optimized_matches_oracle() {
    ...
    mat.enqueue_background(agaric_lib::materializer::MaterializeTask::RebuildPageLinkCache)
        .await
        .unwrap();
    mat.flush_background().await.unwrap();
...
async fn load_page_subtree_returns_active_descendants_excluding_root() {
    ...
    assert!(
        ...
        agaric_lib::commands::pages::listing::PAGE_SUBTREE_MAX_BLOCKS,
    );
    assert_eq!(
        subtree.total, 3,
...
async fn load_page_subtree_reports_truncation_over_cap() {
    use agaric_lib::commands::pages::listing::PAGE_SUBTREE_MAX_BLOCKS;
...
fn folder_path_to_namespace_title_maps_folders_to_namespace_1446() {
    use agaric_lib::commands::pages::markdown::folder_path_to_namespace_title;
    assert_eq!(
...
async fn import_markdown_rejects_excess_attachment_file_count_2724() {
    ...
    let vault: Vec<VaultFile> = (0..=agaric_lib::commands::MAX_ATTACHMENT_FILE_COUNT)
        .map(|i| VaultFile {
...
fn ingest_read_counts_counts_occurrences_not_distinct_refs_2724() {
    ...
    use agaric_lib::commands::pages::markdown::ingest_read_counts;
