...
async fn query_by_tags_inner_rejects_oversized_tag_ids() {
    ...
    let over_cap: Vec<String> = (0..=agaric_lib::commands::tags::MAX_FILTER_TAG_IDS)
...
async fn count_backlinks_batch_large_input_beyond_sqlite_param_limit() {
    ...
    // IDs (the `MAX_BATCH_BLOCK_IDS` cap ceiling) — still well beyond the old
    // ~999 bind-parameter limit that the `IN (?, ?, …)` approach hit.
...
async fn list_page_links_returns_only_current_space_edges_feat3p4() {
    ...
    let scoped_a = agaric_lib::commands::list_page_links_inner(
        &pool,
        &SpaceScope::Active(SpaceId::from_trusted(TEST_SPACE_ID)),
        None,
    )
...
async fn list_page_links_with_none_space_id_returns_all_edges_feat3p4() {
    ...
    let unscoped = agaric_lib::commands::list_page_links_inner(&pool, &SpaceScope::Global, None)
        .await
        .unwrap()
        .edges;
    assert_eq!(
...
async fn list_page_links_with_nonexistent_space_id_returns_empty_feat3p4() {
    ...
    let resp = agaric_lib::commands::list_page_links_inner(
        &pool,
        &SpaceScope::Active(SpaceId::from_trusted("01NONEXISTENT0000000000000")),
        None,
    )
...
async fn list_page_links_disjointness_feat3p4() {
    ...
    let to_set = |v: Vec<agaric_lib::commands::PageLink>| {
        v.into_iter()
            .map(|l| (l.source_id.into(), l.target_id.into()))
            .collect::<std::collections::HashSet<(String, String)>>()
    };
    let a = to_set(
        agaric_lib::commands::list_page_links_inner(
            &pool,
            &SpaceScope::Active(SpaceId::from_trusted(TEST_SPACE_ID)),
            None,
        )
...
async fn list_page_links_cte_parity_with_inlined_subquery_pend20f() {
    ...
    let scope_a = agaric_lib::commands::list_page_links_inner(
        &pool,
        &SpaceScope::Active(SpaceId::from_trusted(TEST_SPACE_ID)),
        None,
    )
    ...
    .unwrap()
    .edges;
    let scope_b = agaric_lib::commands::list_page_links_inner(
        &pool,
        &SpaceScope::Active(SpaceId::from_trusted(TEST_SPACE_B_ID)),
        None,
    )
...
/// #2542 — empty input is a bulk-read no-op: `get_blocks_inner` returns an
/// empty `Vec` (not a Validation error), matching the rest of the
/// batch-read family. Over-cap input still rejects with Validation.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_blocks_empty_returns_empty_and_rejects_oversize() {
    let (pool, _dir) = test_pool().await;
