...
async fn batch_properties_returns_all_for_multiple_blocks() {
    ...
    assert!(result.contains_key(b2.id.as_str()), "b2 must be in result");
    assert_eq!(
        result[b1.id.as_str()].len(),
        1,
        "b1 should have one property"
    );
    assert_eq!(
        result[b1.id.as_str()][0].key,
        "importance",
        "b1 property key should be 'importance'"
    );
    assert_eq!(
        result[b2.id.as_str()].len(),
        1,
        "b2 should have one property"
    );
    assert_eq!(
        result[b2.id.as_str()][0].key,
        "status",
        "b2 property key should be 'status'"
    );
}

/// #2542 — bulk reads return the empty collection on empty input (they do
/// not reject like the bulk-write family). An empty `block_ids` list is a
/// legitimate state (a page with zero blocks), so `get_batch_properties_inner`
/// returns an empty map rather than a Validation error.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
async fn batch_properties_omits_blocks_without_properties() {
    ...
    assert!(
        !result.contains_key(block.id.as_str()),
...
async fn batch_properties_returns_multiple_props_per_block() {
    ...
    assert!(
        result.contains_key(block.id.as_str()),
        "block must be in result"
    );
    let props = &result[block.id.as_str()];
...
/// #2542 — bulk reads return the empty collection on empty input. An empty
/// `ids` list is a legitimate state, so `batch_resolve_inner` returns an
/// empty `Vec` rather than a Validation error (matches the rest of the
/// batch-read family).
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
async fn set_todo_state_clears_value() {
    ...
    let result = set_todo_state_inner(&pool, DEV, &mat, block.id.as_str().into(), None)
        .await
        .unwrap();
...
async fn set_priority_sets_and_clears() {
    ...
    // Set priority
    let result = set_priority_inner(&pool, DEV, &mat, block.id.as_str().into(), Some("2".into()))
        .await
        .unwrap();
    assert_eq!(
        result.priority,
    ...
    // Clear priority
    let result = set_priority_inner(&pool, DEV, &mat, block.id.as_str().into(), None)
        .await
        .unwrap();
...
async fn set_due_date_sets_and_clears() {
    ...
    // Clear due date
    let result = set_due_date_inner(&pool, DEV, &mat, block.id.as_str().into(), None)
        .await
        .unwrap();
...
async fn set_priority_writes_op_log_entry() {
    ...
    set_priority_inner(&pool, DEV, &mat, block.id.as_str().into(), Some("1".into()))
        .await
        .unwrap();
...
async fn set_scheduled_date_sets_and_clears() {
    ...
    let result = set_scheduled_date_inner(&pool, DEV, &mat, block.id.as_str().into(), None)
        .await
        .unwrap();
    assert_eq!(
...
async fn todo_state_auto_todo_to_null_clears_both_timestamps() {
    ...
    set_todo_state_inner(&pool, DEV, &mat, block.id.as_str().into(), None)
        .await
        .unwrap();
    mat.flush_background().await.unwrap();
...
async fn seed_legacy_repeat_property(
    ...
    let mut tx = agaric_lib::db::CommandTx::begin_immediate(pool, "seed_legacy_repeat")
        .await
        .unwrap();
    tx.arm_engine_rollback(mat.loro_state());
    let (_row, op_record) = agaric_engine::block_ops::set_property_in_tx(
        &mut tx,
        mat.loro_state(),
        device_id,
        block_id.to_string(),
...
async fn recurrence_daily_creates_next_occurrence() {
    ...
    set_repeat_property(&pool, DEV, &mat, block.id.as_str(), "daily").await;
    mat.flush_background().await.unwrap();
...
async fn recurrence_weekly_shifts_by_7_days() {
    ...
    set_repeat_property(&pool, DEV, &mat, block.id.as_str(), "weekly").await;
    mat.flush_background().await.unwrap();
...
async fn recurrence_monthly_handles_month_end() {
    ...
    set_repeat_property(&pool, DEV, &mat, block.id.as_str(), "monthly").await;
    mat.flush_background().await.unwrap();
...
async fn recurrence_custom_plus_3d() {
    ...
    set_repeat_property(&pool, DEV, &mat, block.id.as_str(), "+3d").await;
    mat.flush_background().await.unwrap();
...
async fn m20_set_priority_accepts_user_extended_options() {
    // / docs/architecture/data-and-events.md § Property values: the priority property is a
    ...
    let result = set_priority_inner(&pool, DEV, &mat, block.id.as_str().into(), Some("A".into()))
        .await
        .unwrap();
    assert_eq!(
    ...
    set_priority_inner(&pool, DEV, &mat, block.id.as_str().into(), Some("2".into()))
        .await
        .unwrap();
...
async fn todo_state_and_priority_fallback_defaults_match_seeded_options() {
    ...
    use agaric_lib::commands::properties::{
        PRIORITY_FALLBACK_DEFAULTS, TODO_STATE_FALLBACK_DEFAULTS,
    };
...
async fn get_property_normalizes_lowercase_block_id() {
    ...
    let lower = block.id.as_str().to_ascii_lowercase();
    assert_ne!(
        lower, block.id,
