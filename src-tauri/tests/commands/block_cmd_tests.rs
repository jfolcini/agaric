...
async fn create_block_with_page_and_space_id_emits_two_ops_atomically() {
    ...
    let block = create_block_inner_with_space(
        ...
        &SpaceScope::Active(SpaceId::from_trusted(
            agaric_lib::spaces::bootstrap::SPACE_PERSONAL_ULID,
        )),
        // #2849 PR2: server-generated id (test).
        None,
    )
    ...
    assert_eq!(
        space_ref.as_deref(),
        Some(agaric_lib::spaces::bootstrap::SPACE_PERSONAL_ULID),
...
// The documented limit (docs/architecture/data-and-events.md § Everything is a block). closed the asymmetry on
...
async fn verify_written_attachment_missing_file_returns_io_error() {
    ...
    let result = agaric_lib::commands::attachments::verify_written_attachment(&missing, 1024).await;
...
async fn verify_written_attachment_size_mismatch_returns_validation_error() {
    ...
    let result = agaric_lib::commands::attachments::verify_written_attachment(&path, 64).await;
...
async fn verify_written_attachment_accepts_matching_size_and_keeps_bytes() {
    ...
    agaric_lib::commands::attachments::verify_written_attachment(&path, 32)
        .await
...
async fn add_attachment_with_bytes_verifies_stored_size_against_disk() {
    ...
    agaric_lib::commands::attachments::verify_written_attachment(
...
async fn add_attachment_with_bytes_rejects_when_stored_length_diverges() {
    ...
    agaric_lib::commands::attachments::write_fault::arm_truncate(_dir.path());
...
async fn add_attachment_with_bytes_rejects_when_stored_bytes_vanish() {
    ...
    agaric_lib::commands::attachments::write_fault::arm_unlink(_dir.path());
...
async fn list_attachments_batch_returns_full_lists_per_block() {
    ...
    // attachment ids are generated with the non-monotonic `Ulid::generate()`, so two
...
async fn flush_all_drafts_skips_oversized_draft_and_flushes_the_rest_3262() {
    ...
    let oversized = "x".repeat(agaric_engine::block_ops::MAX_CONTENT_LENGTH + 1);
    sqlx::query(
...
async fn create_blocks_batch_inserts_n_blocks_in_one_tx() {
    ...
    let n: usize = 5;
    let specs: Vec<agaric_lib::commands::CreateBlockSpec> = (0..n)
        .map(|i| agaric_lib::commands::CreateBlockSpec {
    ...
    let created = agaric_lib::commands::create_blocks_batch_inner(&pool, DEV, &mat, specs)
        .await
        .unwrap();
...
async fn create_blocks_batch_atomic_rollback() {
    ...
    let specs = vec![
        agaric_lib::commands::CreateBlockSpec {
            block_type: "content".into(),
    ...
    let result = agaric_lib::commands::create_blocks_batch_inner(&pool, DEV, &mat, specs).await;
    assert!(
        matches!(result, Err(AppError::Validation { .. })),
...
async fn create_blocks_batch_rejects_empty_oversize() {
    ...
    let empty = agaric_lib::commands::create_blocks_batch_inner(&pool, DEV, &mat, vec![]).await;
    assert!(
        matches!(empty, Err(AppError::Validation { .. })),
        "empty list must reject with Validation, got {empty:?}"
    );

    let oversize: Vec<agaric_lib::commands::CreateBlockSpec> = (0
        ...
        .collect();
    let result = agaric_lib::commands::create_blocks_batch_inner(&pool, DEV, &mat, oversize).await;
    assert!(
        matches!(result, Err(AppError::Validation { .. })),
        "oversize list must reject with Validation, got {result:?}"
    );
...
async fn create_blocks_batch_with_properties() {
    ...
    // accepts `1` / `2` / `3` (see `agaric_lib::commands::properties` defaults),
...
async fn create_blocks_batch_with_due_date_stores_as_date() {
    ...
    let specs = vec![agaric_lib::commands::CreateBlockSpec {
        block_type: "content".into(),
