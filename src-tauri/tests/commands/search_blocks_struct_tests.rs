...
use agaric_lib::commands::{MatchOffset, SearchBlockRow, SearchFilter};
...
fn search_filter_pend53_due_filter_named_today() {
    ...
    use agaric_lib::commands::queries::{DateFilter, NamedDateRange};
    match df {
...
fn search_filter_pend53_due_filter_op_form() {
    ...
    .unwrap();
    let df = filter.due_filter.expect("dueFilter must deserialise");
    use agaric_lib::commands::queries::{DateFilter, DateOp};
    match df {
