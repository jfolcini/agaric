...
async fn restore_page_to_op_skips_delete_attachment() {
    ...
    let att_ts = agaric_lib::db::now_ms();
    sqlx::query(
    ...
    let del_ts = agaric_lib::db::now_ms();
    op_log::append_local_op_at(
        &pool,
        DEV,
...
async fn undo_produced_delete_cohorts_are_monotonic_and_match_op_created_at() {
    ...
    let mut floor = 0;
    for _ in 0..2000 {
        floor = agaric_lib::db::next_delete_ms();
    }
...
/// #2549 supersedes #2481 phase 2 here: explicit History-view revert of a
/// replicated audit ref is now REJECTED. #2481 had left `revert_ops`
/// deliberately unfiltered on `is_replicated`, but a replicated audit row
/// (migration 0099, #2495) was ingested for provenance only and NEVER applied
/// to local state, so appending its inverse would corrupt local state by
/// "undoing" a forward effect that never happened here. `revert_ops_in_tx`'s
/// `reject_replicated_targets` guard refuses the batch before any reverse is
...
async fn revert_ops_rejects_replicated_ref_explicitly_2549() {
    ...
    .await
    .expect_err("#2549: explicit revert of a replicated audit ref must be rejected");
    assert!(
        matches!(err, AppError::Validation { .. }),
        "#2549: reverting a replicated audit op must surface AppError::Validation, got {err:?}"
    );

    // The guard aborts before any reverse op is appended: no new undo-flagged
    // op exists and the block content is untouched by the rejected revert.
    let reverse_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM op_log WHERE is_undo = 1")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(
        reverse_count.0, 0,
        "no reverse op must be appended when the batch is rejected"
    );
    mat.shutdown();
}
