...
async fn restore_all_deleted_restores_all_soft_deleted_blocks() {
    ...
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "RA_DEL1")
        .await
        .unwrap();
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "RA_DEL2")
        .await
        .unwrap();
...
async fn restore_all_deleted_handles_cascade_deleted_blocks() {
    ...
    let (ts, count) = soft_delete::cascade_soft_delete(&pool, &mat, DEV, "RA_PAR")
        .await
        .unwrap();
...
async fn purge_all_deleted_removes_all_soft_deleted_blocks() {
    ...
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "PA_DEL1")
        .await
        .unwrap();
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "PA_DEL2")
        .await
        .unwrap();
...
async fn purge_all_deleted_cleans_dependent_tables() {
    ...
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "PA_REL")
        .await
        .unwrap();
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "PA_TAG")
        .await
        .unwrap();
...
async fn purge_all_deleted_preserves_non_deleted_blocks() {
    ...
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "PA5_DEL1")
        .await
        .unwrap();
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "PA5_DEL2")
        .await
        .unwrap();
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "PA5_DEL3")
        .await
        .unwrap();
...
async fn list_trash_with_cascade_deleted_page_returns_only_root_and_restores_descendants() {
    ...
    let (_ts, count) = soft_delete::cascade_soft_delete(&pool, &mat, DEV, "UX243_PG")
        .await
        .unwrap();
...
async fn purge_root_from_trash_removes_descendants() {
    ...
    soft_delete::cascade_soft_delete(&pool, &mat, DEV, "UX243_PP")
        .await
        .unwrap();
...
async fn purge_all_deleted_succeeds_when_tag_is_deleted_but_still_inherited_by_live_blocks() {
    ...
    let (_ts, cnt) = soft_delete::cascade_soft_delete(&pool, &mat, DEV, "BUG46_T")
        .await
        .unwrap();
