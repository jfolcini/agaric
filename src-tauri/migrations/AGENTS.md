...
**Guard:** the `check-migration-mock-contract` prek hook (`scripts/check-migration-mock-contract.py`) mechanically enforces the rule above. It carries an explicit CONTRACT map (backend table → the mock's in-memory store symbol + the `src/lib/tauri-mock/` files that model it) and, for every **new** migration (one not grandfathered in `src-tauri/migrations-mock-ack-baseline.txt`) that touches a mapped table, fails unless EITHER (a) a modeling mock file is changed in the same PR, OR (b) the migration carries a literal `-- mock-unaffected: <reason>` annotation line (the escape hatch for index-only / derived-cache-only changes the mock does not model). The baseline makes `prek run --all-files`, pre-commit, and pre-push behave identically with no git dependence; after landing a new migration, grandfather it with `python3 scripts/check-migration-mock-contract.py --update-baseline`. A `--self-test` companion hook pins the CONTRACT map against rot (asserts every mapped file exists and still mentions its store symbol).
