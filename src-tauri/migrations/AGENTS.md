...
Runs the per-migration round-trip / data-preservation tests. These live in `db::tests` (`src-tauri/src/db/tests.rs`) — and a few siblings under `snapshot`/`spaces` — and follow two overlapping naming conventions. The older `_376`/`_606`/`_708` batch (#376/PR #394 round-trip + cascade-preservation harness, #606 satellite-preservation, #708 spaces-registry) predates the migration-number convention below, so those three issue numbers stay pinned in the filter by suffix (`_(376|606|708)$`) rather than being rediscovered by pattern. Every migration test since follows `<table>_<NNNN>_<what>_<issue>` (e.g. `peer_refs_0107_endpoint_id_add_preserves_existing_rows_3464`, `peer_refs_0111_streamed_at_add_preserves_existing_rows_4084`), embedding the zero-padded migration number — the `_0[0-9]{3}_` half of the filter matches that shape wherever it appears in the name, so a new migration test is caught automatically as long as its name embeds its migration number this way; no filter edit needed *while migration numbers stay below `1000`* (currently at `0111`). `_0[0-9]{3}_` only spans `0000`–`0999` by construction — the leading literal `0` is what lets it avoid matching unrelated 4-digit numbers (issue/PR references, years, sizes) elsewhere in test names; verified by running `test(/_(376|606|708)$|_[0-9]{4}_/)` against this tree, which drops the leading-zero anchor and pulls in 17 unrelated tests (issue/PR-numbered tests, `f14_page_size_1000_...`, `rmcp_2026_...`) for zero new migration tests, so that broadening is not a safe substitute today. When migration numbers reach `1000` this filter will need revisiting — sooner if the naming convention ever grows an extra digit. (Previously the filter was a bare `_(376|606|708)$` alternation, which is why every migration test added after #708 — `_0107`, `_0108`, `_0111`, etc. — went silently unselected; verified against the tree, the old filter matched 15 tests and this one matches 27, a strict superset.) There is no module or test named `migration_tests`; the regex filter is what actually selects them. A new migration needs a corresponding test fixture that inserts representative data + reads it back (name it with the `_<NNNN>_` migration number so this filter keeps catching it); lock the schema's external contract early.
