...
use crate::transport::SecretKey;
use crate::transport::session::{recv_sync_message, send_sync_message};
use crate::transport::test_support::{ServiceHarness, TestSide, quic_pair};
...
async fn try_sync_with_peer_emits_error_event_on_connection_failure() {
    ...
    let result = tokio::time::timeout(
        ...
        try_sync_with_peer(&ctx, &peer, &refs, None),
    )
    .await;
...
async fn the_repeat_report_suppression_is_wired_into_try_sync_with_peer_4120() {
    ...
    async fn cycle(
        ...
        let apply_host: Arc<dyn crate::apply_host::ApplyHost> = Arc::new(materializer.clone());
        let ctx = SyncSessionContext {
            pool: &pool,
            device_id: "LOCAL_DEV",
...
async fn try_sync_with_peer_skips_a_peer_with_no_device_id() {
    ...
    let apply_host: std::sync::Arc<dyn crate::apply_host::ApplyHost> =
        std::sync::Arc::new(materializer.clone());
    let ctx = SyncSessionContext {
        pool: &pool,
        device_id: "LOCAL",
...
async fn inbound_is_refused_while_outbound_holds_the_same_peer_3511() {
    ...
    let proof = crate::pairing::pairing_proof("the pairing passphrase");
    ...
    let outbound = tokio::spawn({
        ...
        let peer = peer.clone();
        async move {
            let apply_host: Arc<dyn crate::apply_host::ApplyHost> = Arc::new(materializer);
            ...
            try_sync_with_peer(&ctx, &peer, &[], None).await
        }
    });
...
async fn handle_incoming_sync_admits_pairing_with_correct_proof_855() {
    let right = crate::pairing::pairing_proof("the real passphrase");
...
async fn pairing_proof_from_two_device_command_flow_is_admitted_3463() {
    ...
    let host_passphrase = crate::pairing::start_pairing_armed(
        &pool,
...
// `crate::transport::identity`, where its tests live.
            ...
            send_sync_message(
                ...
                &SyncMessage::LoroSync {
                    msg: crate::sync_protocol::loro_sync_types::LoroSyncMessage::Snapshot {
                        protocol_version:
                            crate::sync_protocol::loro_sync_types::LORO_SYNC_PROTOCOL_VERSION,
...
async fn drive_two_device_pairing_windowed_3507(
    ...
    let host_passphrase =
        crate::pairing::start_pairing_armed(&host_pool, &host_slot, &host_sched, HOST_DEV_3507)
            .await
    ...
    let joiner_apply: Arc<dyn crate::apply_host::ApplyHost> =
    ...
    let was_cancelled = tokio::time::timeout(
        std::time::Duration::from_secs(30),
        try_sync_with_peer(&ctx, &peer, &joiner_refs, None),
    )
    .await
...
async fn drive_pairing_claim_4230(
    ...
    let proof = crate::pairing::pairing_proof("the pairing passphrase 4230");
...
async fn seed_replicated_op_4380(
    ...
    crate::sync_protocol::insert_replicated_op(
        pool,
        &crate::sync_protocol::types::OpTransfer {
...
async fn drive_pairing_bind_4380(foreign_device: &str) -> BindRun4380 {
    ...
    let advertised: Vec<String> = crate::sync_protocol::get_local_heads(&joiner_pool)
        .await
        .unwrap()
        .into_iter()
        .map(|h| h.device_id)
    ...
    let proof = crate::pairing::pairing_proof(PASSPHRASE_4380);
    peer_refs::set_pending_pairing(&joiner_pool, &proof)
        .await
        .unwrap();
    peer_refs::set_pending_pairing(&host_pool, &proof)
...
async fn a_legacy_joiners_unusable_head_is_not_bound_4451() {
    ...
    let over_cap = "d".repeat(crate::sync_protocol::MAX_DEVICE_ID_CHARS + 1);
