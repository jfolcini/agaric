...
/// [`peer_refs::update_on_sync`](agaric_store::peer_refs::update_on_sync) (or
...
/// with [`agaric_engine::loro::snapshot::reload_registry_from_db`] (drop every
...
pub async fn apply_snapshot<R: std::io::Read>(
    ...
    let mut tx = agaric_store::db::begin_immediate_logged(pool, "snapshot_restore").await?;
    ...
    let new_peer_epoch = agaric_engine::loro::peer_epoch::bump_peer_epoch(&mut tx).await?;
    tracing::info!(
        new_peer_epoch,
        "apply_snapshot: peer-id epoch bumped (#792); post-reset engines \
         will mint ops under a fresh Loro PeerID"
    );
    let cursor_reset_at = agaric_store::db::now_ms();
    sqlx::query!(
    ...
    let repaired_block_properties: Vec<_> = data
        ...
        .filter(|bp| {
            ...
            if agaric_store::op::is_column_backed_property_key(&bp.key) {
...
#[cfg(test)]
mod wipe_inventory_tests {
    /// #3328 — the snapshot RESET wipe inventory and the post-restore rebuild
    /// task set must stay in step.
    ///
    /// [`CACHE_TABLES`] (the wipe half) and the materializer's
    /// `POST_SNAPSHOT_CACHE_REBUILDS` (the refill half) live in two crates with no
    /// mechanical link between them; this crate sees both, so the pairing is
    /// pinned here (#4502 moved it down from the app crate with the materializer).
    ///
    ...
    fn snapshot_wipe_inventory_and_post_snapshot_rebuilds_agree() {
        use super::CACHE_TABLES;
        use agaric_engine::materializer::POST_SNAPSHOT_CACHE_REBUILDS;
