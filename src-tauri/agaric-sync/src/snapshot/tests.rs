...
async fn cleanup_spares_recent_pending_but_deletes_old_pending() {
    ...
    let recent_pending =
        ulid::Ulid::from_parts(u64::try_from(agaric_store::db::now_ms()).unwrap(), 7).to_string();
...
async fn collect_tables_runs_on_deferred_connection_2470() {
    ...
    let write_tx = agaric_store::db::begin_immediate_logged(&pool, "test_hold")
        .await
        .unwrap();
...
/// survived the reset. #3487 deleted the wire path that could re-ship
/// them, but the rows still mislead two local readers: `dag::find_lca`
/// reads any `status = 'complete'` row as "compaction has occurred", and
/// `compact_op_log` takes the latest as its retention floor. This pins
/// the wipe against both.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
async fn measure_apply_snapshot_write_lock_hold_2470() {
    ...
    let writer_task = tokio::spawn(async move {
        ...
        loop {
            ...
            attempts += 1;
            let start = std::time::Instant::now();
            let result = agaric_store::db::begin_immediate_logged(
                &writer_pool,
                "concurrent_writer_probe_2470",
            )
            .await;
            let elapsed = start.elapsed();
