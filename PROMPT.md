...
- **Improvement opportunities found mid-task go to GitHub issues, not `pending/`.** When you spot something worth doing that isn't part of the current scope, the choice is binary: either (a) fix it drive-by in the same PR if it's a 1–3 line change with no design questions and no test-fixture sprawl, or (b) file a GitHub issue (`gh issue create --label plan` if it's multi-phase / needs Open Qs, no label otherwise). This applies **even when the opportunity needs further exploration** — "I'm not sure yet" is exactly why a GitHub issue exists (discussion happens in comments). Do NOT create new `pending/PEND-NN-*.md` files; that pattern was retired during the GitHub-issue migration (2026-05-27). The only files that should grow in `pending/` are `IDEAS.md` (long-running ideation backlog) and `REVIEW-LATER.md` (small CR-* / MAINT-* tickets that don't warrant their own issue).
